...
- `/projects/blockwise` on `main` is the sole maintained source; production serves that revision. Never deploy from a feature checkout, and never merge a feature or `archive/` branch wholesale. Every other checkout, including `/root/work/blockwise` and each `/projects/<task>` copy, is a dated snapshot whose rule and doc files never override the canonical ones.
