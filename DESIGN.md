...
The approved three-screen explainer uses one reversible Motion value (Choose, Customise, Review, approved), not independent entrance timers. All layers remain mounted; edit and review crossfade inside the same fixed-size panel. Complete headline layers occupy the same reserved space; no letter-by-letter rendering or text reflow. The marketing-only focal travel lasts 640ms; panel and approval crossfades use 350ms. Autoplay holds start after movement finishes, pause out of view, and yield to screen selection. Reduced motion applies the selected state immediately. Existing imagery and Motion dependency are reused; no video or added animation engine.
