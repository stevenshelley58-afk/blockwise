...
- **One system, two themes.** Ad Builder is the dark theme of this same system, not a separate design system. Only the colour roles change there; the type roles, spacing scale, shape roles, controls, state behaviour and action hierarchy are shared.
...
- **Keep customer navigation permanent.** Self-serve customer routes keep the five mobile destinations: Home, Ads, Performance, Leads and More. Ads owns the full Ad Builder subtree. Never expose an unavailable tool or invent a destination to fill the menu.
...
- **Reduced motion is mandatory.** Every transform is gated: useReducedMotion for motion/react, the motion-reduce variant for CSS. There is exactly one confetti moment, the first-run handoff into Ad Builder; it is brief, never loops, and is skipped entirely under reduced motion.
...
- **The weekly metrics row is Home's only card row, and Performance opens with the same one.** The four figures — spend, link clicks, cost per link click and leads — each render in the shared KPI card surface: `rounded-(--r-card)`, `border-(--line)`, `bg-card`, `shadow-card`, with that figure's sparkline and prior-period comparison inside its own card. Two cards across on a phone, one row of four from `lg`, because the sidebar takes its width before that. Leads, local ads and everything else on Home stay flat: sections, rows and hairlines. Performance leads with the same four cards over the range it is showing and puts every other figure it reports inside More reporting details.
...
- **One figure card, used everywhere.** That card is `src/components/ui/metric-card.tsx`: label, figure, its own line, its comparison, and the money and count formats both bands share. Home and Performance both build their figure rows from it, so a surface never hand-rolls a second stat card. A figure with no daily series draws no line, and a missing figure renders the unavailable mark rather than a zero, with no comparison beside it.
- **One period control per surface, next to the data it slices.** Performance states its range once, in the chart card above the line it changes; the heading keeps the title, the sync state and Refresh. Do not repeat the same period as chips or a second selector, and choose the chart's own metric from a menu rather than a row of buttons. That one control slices the whole page, the figure row included: the row's heading names the window the figures on screen actually cover — the payload's range, never the range a customer just picked while the fetch is still in flight — and the customer's own span says its dates rather than a label that would leave them guessing which dates it meant.
...
- **One notice bar, used everywhere.** That bar is `src/components/ui/notice-bar.tsx`: the amber preview marker, the note, and the product's own call to action. Home's weekly figures, Home's example leads and Performance's example report all close on it with the same sentence, so a preview is labelled the same way wherever it appears.
...
- **Ad Radar feeds Home's local ads, keyed on the workspace's explicit default postcode.** A verified signup email may seed that value from the private location projection only while it is missing; a customer save always wins. Without a postcode, Home asks an owner or admin for it instead of substituting a generic area. Read the selected postcode first, then a bounded set of nearby postcode areas, and include only ads with structured ad-area postcode evidence.
...
The native editor starts on the actual edited ad preview. Template headings wrap at words, not arbitrary letters. Initial text is fitted using its actual font, box, line limit and existing overflow policy; saved customer typography is never refitted on reopen. Photos, Words and Adjust design occupy a slim secondary left tool column on desktop, and one equal-width bottom row on mobile. These tool buttons are quiet ghost controls with visible icon-and-text labels, and carry no decorative arrow; the main customer navigation stays separate. Review & publish is primary; Save and its truthful status remain available. Adjust design reveals the stock full-width native canvas, and Done designing returns to the ad. These are two views of one continuously mounted native editor, not two implementations or a conversion. Preserve all freeform edits on transition and keep hidden design tools keyboard-inert. Copy, optional AI help and photo choices use shared Sheets rather than permanent sidebars. Secondary copy settings are disclosed when needed. Feed and Story dimensions remain fixed; native file imports and cloud services are not exposed. Original ads and the old editor remain available. See docs/architecture/adbuilder-vue-editor.md for alternatives and trade-offs.
...
- 2026-09-14: **one design system, applied to the whole website.** Owner decisions
  ...
  *Ad Builder.* Ad Builder is now the dark theme of this same system, not a separate
  ...
  *Scope.* The whole website is in scope: customer, Ad Builder, authentication, legal,
...
- 2026-09-12: Performance's chart menu says "Leads over time" rather than "Enquiries
...
- 2026-09-12: a demo is labelled one way everywhere. The one notice bar says
  ...
  example leads and now under Performance's figures, which is where the demo
  statement belongs rather than above the numbers it labels. Performance's example
...
- 2026-09-12: Performance now leads with Home's four figure cards — spend, link
  ...
  workspaces. Rewrote three rules: the card row now belongs to Performance too, a
...
- `/ad-builder/publish-history` preserves the recovered 12 September four-stage
