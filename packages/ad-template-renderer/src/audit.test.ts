...
test("audit receipt preserves every aggregate text preflight violation", async () => {
  const template = templateFixture();
  const headline = template.feedLayout.layers.find((layer) => layer.layerId === "feed-headline");
  assert.ok(headline?.type === "text");
  headline.geometry.width = 8;
  template.textInputs.push({ key: "phone", label: "Phone", placeholder: "+61 400 000 000", maxLength: 40 });
  template.storyLayout.layers.push({
    type: "text",
    layerId: "story-phone-text",
    inputKey: "phone",
    font: { file: "manrope-800.woff2" },
    fontSize: 36,
    lineHeight: 1,
    ...
    colourRole: "mainText",
    overflowBehaviour: "scale_down",
    geometry: { x: 80, y: 80, width: 8, height: 80 },
  });
  const photo = imageBytes();
  const result = await auditTemplateArtifact({
    artifactBytes: artifactBytes(template, [{
  ...
  assert.equal(result.receipt.verdict, "fail");
  assert.equal(result.outputs, undefined);
  const rerender = result.receipt.checks.find((check) => check.id === "renderer_rerender");
  assert.equal(rerender?.status, "fail");
  assert.equal(typeof rerender?.details, "string");
  const details = rerender!.details as string;
  assert.ok(details.startsWith("AD_TEMPLATE_TEXT_PREFLIGHT_FAILED "));
  assert.ok(details.indexOf("feed-headline") < details.indexOf("story-phone-text"));
  assert.match(details, /feed text layer feed-headline cannot fit at the 24px readability floor/);
  assert.match(details, /story text layer story-phone-text cannot fit at the 32px readability floor/);
});
