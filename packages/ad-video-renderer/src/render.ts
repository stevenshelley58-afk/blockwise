...
import { renderBeatFrame } from "./frame.js";
...
export async function renderVideoProject(request: RenderRequest, options: RenderOptions): Promise<RenderResult> {
  ...
  try {
    ...
    for (const scene of plan.scenes) { const frame = await renderBeatFrame(project, scene, resolved); const path = join(workDir, `beat-${scene.index}.png`); await writeFile(path, frame); framePaths.push(path); }
    ...
    if (options.executeFfmpeg) {
      ...
      await encodeMp4(framePaths, project.durationSeconds, fps, manifest.composition.beats.map((beat) => beat.transition), mp4Path, options.ffmpegPath ?? "ffmpeg", musicPath, clipPaths);
...
async function encodeMp4(frames: string[], duration: number, fps: number, transitions: string[], output: string, ffmpeg: string, musicPath?: string, clipPaths = new Map<number, string>()): Promise<void> {
  const args: string[] = ["-hide_banner", "-loglevel", "error", "-y"];
  for (let i = 0; i < frames.length; i++) {
    const clip = clipPaths.get(i);
    if (clip) args.push("-stream_loop", "-1", "-t", String(duration / frames.length), "-i", clip);
    else args.push("-loop", "1", "-framerate", String(fps), "-t", String(duration / frames.length), "-i", frames[i]!);
  }
  ...
  if (transition === "hard_cut") filters.push(`${frames.map((_, i) => `[v${i}]`).join("")}concat=n=${frames.length}:v=1:a=0[vout]`);
    ...
    const xfade = transition === "brand_wipe" ? "wipeleft" : transition === "smart_push" ? "smoothleft" : "fade"; const overlap = 0.25; let current = "v0";
    for (let i = 1; i < frames.length; i++) { const out = `vx${i}`; const offset = Math.max(0, i * duration / frames.length - overlap); filters.push(`[${current}][v${i}]xfade=transition=${xfade}:duration=${overlap}:offset=${offset.toFixed(3)}[${out}]`); current = out; }
    videoLabel = current;
  }
  filters.push(`[${frames.length}:a]volume=${musicPath ? "0.18" : "1.0"}[aout]`);
  ...
  try { await execFileAsync(ffmpeg, args, { maxBuffer: 2 * 1024 * 1024 }); } catch (error) { const message = error instanceof Error ? error.message : String(error); throw new Error(`FFmpeg render failed: ${message.slice(0, 500)}`); }
}
