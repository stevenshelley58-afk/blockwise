...
export interface LegacyGenerationReview { process: "exact-clone"; sourcePlacement: Placement; targetPlacement: Placement; likenessThreshold: number; comparator: { overall: number; geometry: number; colourEffects: number; compositionCrop: number; typography: number; decision: "revise" | "ready" }; finalReviewers: { id: string; route: string; overall: number; minimum: number; decision: "pass" | "fail" }[]; warnings: string[]; fontSubstitution: null | { source: string; used: string; reason: string }; }
export interface SectionGenerationReview { policy: "section-95-font-exempt-no-obvious-errors-v1"; process: "exact-clone"; sourcePlacement: Placement; targetPlacement: Placement; sectionThreshold: number; fontMatchRequired: false; comparator: { geometry: number; colourEffects: number; compositionCrop: number; typography: number; details: number; decision: "revise" | "ready" }; finalReviewers: { id: string; route: string; minimum: number; decision: "pass" | "fail" }[]; overallCheck: { noObviousErrors: true }; warnings: string[]; fontSubstitution: null | { source: string; used: string; reason: string }; }
export interface FrozenSection98GenerationReview { policy: "section-98-font-exempt-no-obvious-errors-v1"; process: "exact-clone"; sourcePlacement: Placement; targetPlacement: Placement; sectionThreshold: number; fontMatchRequired: false; comparator: { geometry: number; colourEffects: number; compositionCrop: number; typography: number; details: number; decision: "revise" | "ready" }; finalReviewers: { id: string; route: string; minimum: number; decision: "pass" | "fail" }[]; overallCheck: { noObviousErrors: true }; warnings: string[]; fontSubstitution: null | { source: string; used: string; reason: string }; }
export type GenerationReview = SectionGenerationReview | FrozenSection98GenerationReview | LegacyGenerationReview;
