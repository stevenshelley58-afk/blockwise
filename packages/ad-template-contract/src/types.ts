...
export interface SectionGenerationReview { policy: "section-98-font-exempt-no-obvious-errors-v1"; process: "exact-clone"; sourcePlacement: Placement; targetPlacement: Placement; sectionThreshold: number; fontMatchRequired: false; comparator: { geometry: number; colourEffects: number; compositionCrop: number; typography: number; details: number; decision: "revise" | "ready" }; finalReviewers: { id: string; route: string; minimum: number; decision: "pass" | "fail" }[]; overallCheck: { noObviousErrors: true }; warnings: string[]; fontSubstitution: null | { source: string; used: string; reason: string }; }
export type GenerationReview = SectionGenerationReview | LegacyGenerationReview;
