...
import { templatePackSchema, adDocumentSchema, type TemplatePackParsed as TemplatePack } from "./schema.ts";
...
describe("TemplatePack schema", () => {
  ...
  it("rejects invalid layer geometry (zero width)", () => {
    const bad = structuredClone(goldenPack);
    ((bad.feedLayout.layers[0] as { geometry: { width: number } }).geometry.width = 0);
    const result = templatePackSchema.safeParse(bad);
    assert.equal(result.success, false);
  });
