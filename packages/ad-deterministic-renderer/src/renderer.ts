...
export async function renderPlacement(input: RenderInput, placement: Placement): Promise<RenderOutput> {
  registerPackFonts(input.pack, input.fontValues);
...
/** Accept both v1 pixel rectangles and v2 normalized rectangles at the boundary. */
...
function registerPackFonts(pack: TemplatePack, fontValues: Record<string, Buffer> = {}): void {
  for (const font of pack.fonts) {
    const fileName = basename(font.file);
    const registrationKey = `${fileName}:${font.sha256}`;
    if (registeredFontFiles.has(registrationKey)) continue;
    const imported = fontValues[font.file] ?? fontValues[`font:${font.file}`];
    const importedPath = imported ? join(tmpdir(), `blockwise-adstudio-${font.sha256.slice(0, 16)}-${fileName}`) : null;
    if (imported && importedPath && !existsSync(importedPath)) writeFileSync(importedPath, imported);
    const absolute = importedPath ?? join(process.cwd(), "public", "fonts", "adstudio", fileName);
...
type RenderTextLayer = TextLayer & {
  /** Optional v2 authoring metadata carried through a pack adapter. */
  sizeRatio?: number;
  fontFamily?: string;
...
function renderText(ctx: SKRSContext2D, layer: TextLayer, input: RenderInput, dims: CanvasDimensions): void {
  ...
  // The editor resolves the face by filename stem. Accept a v2 family alias
