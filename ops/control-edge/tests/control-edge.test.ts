...
import { ExecutorError, runMaintenance, runOneAction } from "../src/executor.ts";
...
class FakeRepository implements ActionRepository {
  nonces = new Set<string>(); actions = new Map<string, ActionStatus>(); reaps = 0; failures: Array<{ retryable: boolean; error: string }> = [];
  ...
  async reap(): Promise<number> { this.reaps += 1; return 0; }
  ...
  async fail(_id: string, _lease: string, error: string, retryable: boolean): Promise<string | null> { this.failures.push({ error, retryable }); return null; }
}
...
test("maintenance always reaps expired leases before claiming", async () => {
  const repo = new FakeRepository(); let claimed = false;
  repo.claim = async () => { claimed = true; return null; };
  await runMaintenance(repo, {} as never);
  assert.equal(repo.reaps, 1); assert.equal(claimed, true);
});
test("executor distinguishes permanent domain failures from transient failures", async () => {
  const action = { id: "1", action_id: "1", idempotency_key: "k", created_at: "2026-09-04T15:00:00.000Z", workspace_id: "2", customer_id: "2", actor_operator_id: "3", actor_role: "support", action_type: "billing_reconcile", target_type: "billing", target_id: "2", expected_version: 1, reason: "r", payload: {}, attempts: 1, max_attempts: 3, expires_at: "2026-09-04T16:00:00.000Z", lease_token: "4" } as unknown as ClaimedAction;
  const permanent = new FakeRepository(); permanent.claim = async () => action; permanent.heartbeat = async () => true;
  await runOneAction(permanent, { execute: async () => { throw new ExecutorError("billing_subscription_workspace_mismatch", false); } } as never);
  assert.deepEqual(permanent.failures, [{ error: "billing_subscription_workspace_mismatch", retryable: false }]);
  const transient = new FakeRepository(); transient.claim = async () => action; transient.heartbeat = async () => true;
  await runOneAction(transient, { execute: async () => { throw new ExecutorError("executor_http_503", true); } } as never);
  assert.deepEqual(transient.failures, [{ error: "executor_http_503", retryable: true }]);
});
