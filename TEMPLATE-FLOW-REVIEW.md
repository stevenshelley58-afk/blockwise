...
1b. **The user's brief never reaches the ad (template mode).** The dialog requires a "Short description," but the clone request sends only images + brandHex (`new-ad-dialog.tsx:1237-1249` — no `copy`, no description), so `resolveCloneCopy` bakes the template's **sample-copy defaults** into the generated image; AI copy enrichment is deliberately skipped in template mode (`campaigns/route.ts:181-187`), so the Meta feed text comes from the generic offer library too. The description only influences offer inference (`generator.ts:95,279`). Net: a required field the product then ignores — the single biggest "it feels broken" contributor after 1.
...
1. **In-place editing = targeted regeneration (the Stitch model).** For template ads, replace the Fabric editor with click-on-the-image inline editing: after each generation, a vision pass returns bounding boxes for every editable element (headline, CTA, price badge, agent photo); the UI overlays hit-targets on the flat image, so clicking the headline opens an inline input right on top of it. On confirm, run a **targeted edit anchored on the current image** — "change only the headline text to X, keep everything else identical," optionally masked with the element's box (gpt-image-2 edits accepts masks; the Gemini flash-image class is strong at consistent localized instruction edits) — on the **fast tier** (seconds, cents) as the working preview. "Finalize" runs the **quality tier** (gpt-image-2 high, ~$0.21) once. The template sample is the reference for the *first* generation only; every edit re-anchors on the current creative, which is what keeps the design stable across edits. Regenerate-on-edit only felt unviable because the clone route is locked to a single 60–110s provider today.
2. **Vision-QA + region pass — the make-or-break piece.** Baked-in text means the model can typo the user's headline, suburb, or phone number, and today nothing checks. One vision call after every generation does double duty: verify the exact copy strings rendered (auto-reroll on failure) AND return the editable-region boxes that power in-place editing (§1). The reroll machinery already exists as dead code: `creative-qa.ts` vision QA + the `bulk-cell.ts` pattern (generate up to N rolls, keep the first that passes QA). This converts clone-first's biggest weakness into a solved problem.
3. **Consistency on re-edits.** Edits use the current accepted image as the base input (§1), never a fresh re-clone from the template sample — so an edit can't reshuffle the whole design. QA additionally checks that non-targeted regions didn't drift.
...
6. **One brief, one copy model, three surfaces.** The user supplies photos + a single short brief — nothing else. One AI copy pass generates the complete copy set from brief + brand kit + photo: (a) the on-image overlay fields the template declares (`copyFields` from `deriveTemplateBrief`, with the sample copy as tone/shape reference — not as the final text), (b) the Meta feed text (primary 125 / headline 40 / description 90), and (c) the CTA as the actual Meta enum (`LEARN_MORE`/`SIGN_UP`/`DOWNLOAD`/`CONTACT_US` — lead-gen product, button opens the lead form), chosen directly instead of keyword-sniffed from a free-text label (two divergent keyword maps exist today: `use-copy.ts:54` vs `generator.ts:1419`). The clone then runs **with** that copy so the image carries the user's actual message — `generate-clone` already accepts a `copy` param the dialog never sends. Editing respects each surface's nature: feed text = instant plain-text edits; on-image text = click-in-place targeted regen (§1); CTA = dropdown of the real enum. The review screen shows the ad as Meta renders it — image, feed text, and button chrome together.
...
- **T0.7** Make the brief reach the ad (finding 1b): generate the template's `copyFields` + Meta feed copy from the user's description via the existing copy generator (sample copy as tone reference, not final text), and pass `copy` into the clone request — the API already accepts it. Highest quality-per-effort fix in the list. *Accept:* the generated image contains the user's message, not the sample's.
...
- **T1.2** Vision-QA + region pass around every clone (Part 4 §2): one vision call verifies the exact copy strings rendered (headline, suburb, phone, CTA) AND returns editable-element bounding boxes; auto-reroll up to N using the `bulk-cell.ts` roll pattern + `creative-qa.ts` vision QA. *Accept:* a fixture clone with a deliberately hard headline never ships with mangled text, and the response includes usable boxes for each declared copy field.
- **T1.3** Rebuild the edit surface for template ads as in-place editing (Part 4 §1): hit-targets from the T1.2 boxes overlaid on the flat image, inline input on click, fast-tier targeted edit anchored on the current image, "Finalize" for the quality pass. Retire the Fabric editor and the role-translation tables for clone creatives; `browser-creative-renderer.ts` goes with it (export = the finalized image itself). *Accept:* click headline → type → preview updates in seconds; the rest of the design is pixel-stable.
