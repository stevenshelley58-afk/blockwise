...
Pick **one** creative architecture: **hybrid — AI prepares the photo, canvas owns the text.**

- The image model's job: fit/outpaint/restyle the customer's photo(s) into the template's photo slots (scene, lighting, composition — **no text in the image**).
- The canvas's job: render headline/subheadline/CTA/logo/badges as layers with brand-kit fonts and colors, exactly as `generator.ts` + the Fabric editor already do.

Why hybrid beats full reference-clone (current flagship):
- **Editing works.** Copy changes are instant, free, and typo-proof — no $0.21 + 60s + rate-limit regeneration per headline tweak, no gpt-image-2 text artifacts, no fake phone numbers rendered into pixels.
- **Brand fidelity is guaranteed** (real fonts, exact hex), which an image model cannot promise.
- **Cost/speed collapse**: text fidelity — the main reason to pay for the top image model — stops mattering; photo prep can run on a cheaper/faster model tier and gets cached per photo, while every copy iteration costs zero.
- It's what SKILL.md already envisions (vision + outpaint photo-fit; `smart-crop.ts` as fallback), and the deterministic compositor + editor + QA already exist and work.

Keep reference-clone, demoted to an explicit **"AI remix" variant generator** clearly marked non-editable (regenerate-to-change), or drop it until clone outputs can be decomposed back into layers. Do not leave it as the silent default that disables editing.

Consolidate the clone/photo-prep provider path on the existing direct OpenAI `images/edits` adapter (`ai-providers.ts`) with the OpenRouter Gemini image fallback — the superior multi-provider runner (`generateMixedImageVariantsInParallel`) is already written and dead. Retire the fal indirection (or keep fal keyed as one more candidate, but with cascade + provider-run logging + documented env).
...
3. **Customer ad image — two-tier by purpose.** Drafts/previews and photo-slot prep: the fast/cheap image tier (Gemini flash image class — already your OpenRouter default). Final render / photoreal remix: gpt-image-2 class at high quality. Under the hybrid architecture the expensive model is needed far less often, because copy iterations no longer trigger image generation, and a prepared photo is cached per photo+slot rather than per edit.
...
- **T0.2** Persist `fabricJson` on draft save (size-cap it, or persist a layout-delta) — or, if layout editing is deliberately out of scope, remove drag/resize from the editor so the UI doesn't lie. *Accept:* reload preserves what the user saw, or the user can't produce state that won't survive.
...
- **T1.1** Ship the hybrid path (Part 4): photo-prep provider cascade (direct OpenAI edits + Gemini image fallback, provider-run logging, compliance gate) feeding template photo slots; canvas text layers stay authoritative. Reference-clone becomes the opt-in "AI remix" variant.
- **T1.2** One renderer: derive gallery previews AND exports from the same canvas render code (server-side SVG → raster), delete `browser-creative-renderer.ts`. *Accept:* pixel-identical preview/editor/export for a fixture campaign, asserted in e2e.
- **T1.3** Unify the role vocabulary: one shared `CreativeRole` module used by CopyState, SelectedElement, canvas objects, and slot derivation; delete the scattered translation tables. Fix the `templates.ts` legacy types (`sampleCopy` free-form, delete the fixed-role `editableImage` enum and library stubs) so the `as unknown as` cast goes away.
- **T1.4** Move generation async: wire `adstudio_creative_jobs` (or a trigger.dev task) for image work; the route enqueues and the client polls/streams progress. Kills the 120s ceiling and enables >1 clone variant.
