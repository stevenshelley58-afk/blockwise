...
import { adoptWorkspaceAsset, AdoptAssetError } from "../src/lib/adbuilder/adopt-workspace-asset.ts";
...
function accessClient(row: unknown, error: unknown = null) {
  ...
  return { from() { return query; } } as unknown as SupabaseClient;
}

test("asset adoption denies cross-workspace sources", async () => {
  await assert.rejects(
    () => adoptWorkspaceAsset({ accessSupabase: accessClient(null), serviceSupabase: {} as unknown as SupabaseClient, workspaceId: "workspace-a", adId: "ad-a", sourceAssetId: "asset-b" }),
...
test("asset adoption distinguishes source lookup failures", async () => {
  await assert.rejects(
    () => adoptWorkspaceAsset({ accessSupabase: accessClient(null, { message: "db unavailable" }), serviceSupabase: {} as unknown as SupabaseClient, workspaceId: "workspace-a", adId: "ad-a", sourceAssetId: "asset-a" }),
...
test("asset adoption rejects invalid source bytes before ledger calls", async () => {
  ...
  await assert.rejects(
    () => adoptWorkspaceAsset({ accessSupabase: accessClient({ id: "asset-a", workspace_id: "workspace-a", storage_path: "workspace-a/kit/a.png" }), serviceSupabase: service as unknown as SupabaseClient, workspaceId: "workspace-a", adId: "ad-a", sourceAssetId: "asset-a" }),
...
test("valid adoption follows prepare, upload, verify, claim, finalize", async () => {
  ...
  const result = await adoptWorkspaceAsset({ accessSupabase: accessClient({ id: "asset-a", workspace_id: "workspace-a", storage_path: "workspace-a/kit/a.png" }), serviceSupabase: service as unknown as SupabaseClient, workspaceId: "workspace-a", adId: "ad-a", sourceAssetId: "asset-a" });
  assert.match(result.ref, /^\/api\/adbuilder\/customer-media\?/);
  assert.deepEqual(calls, ["workspace-artifacts:download", "rpc:adbuilder_prepare_customer_image_upload", "adbuilder-customer-images:upload", "adbuilder-customer-images:info", "adbuilder-customer-images:download", "rpc:adbuilder_claim_customer_image_finalize", "rpc:adbuilder_finalize_customer_image_upload"]);
});
...
test("failed discard claim never removes the customer object", async () => {
  ...
  const module = await import("../src/lib/adbuilder/adopt-workspace-asset.ts");
  const { discardCustomerImageUpload: discard } = module as typeof import("../src/lib/adbuilder/adopt-workspace-asset");
  await discard({ serviceSupabase: service }, "reservation-1", "workspace-a", "ad-a", "workspace-a/adbuilder/ads/ad-a/images/hash.png");
  assert.deepEqual(calls, ["adbuilder_discard_customer_image_upload"]);
});
