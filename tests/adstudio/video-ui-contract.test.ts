...
test("Ad Studio uses one private workbench shell", () => {
  ...
  const navigation = source("src", "components", "adstudio", "studio-navigation.tsx");
...
test("video flow keeps the guided contract, honest API states, and touch targets", () => {
  const flow = source("src", "components", "adstudio", "video", "video-new-flow.tsx");
  const player = source("src", "components", "adstudio", "video", "video-preview-player.tsx");
