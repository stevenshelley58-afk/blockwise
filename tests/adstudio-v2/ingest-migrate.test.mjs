...
test("migrate-v1 is idempotent: running it twice yields identical output", () => {
  ...
  try {
    ...
    const env = {
      ...
      ADSTUDIO_PRIVATE_V2: join(scratch, "private"),
    };
...
test("ingest has no compatibility path that can write source-derived assets under public", () => {
  const ingest = readFileSync(join("scripts", "adstudio", "v2", "ingest.mjs"), "utf8");
  assert.doesNotMatch(ingest, /ADSTUDIO_PUBLIC_V2/);
  assert.match(ingest, /ADSTUDIO_PRIVATE_V2/);
});
