...
test("hash-pinned template resolver uses a matching history snapshot, never a mutated current doc", () => {
  ...
  try {
    ...
    const source = {
      current(id: string) {
        return JSON.parse(readFileSync(join(temp, id, "template.json"), "utf8"));
      },
      history(id: string) {
        return [{
          value: JSON.parse(readFileSync(join(temp, id, "history", "before-qa.json"), "utf8")),
          strict: false,
        }];
      },
    };
    assert.equal(loadTemplateV2ByHash(templateId, currentHash, source)?.name, current.name);
    assert.equal(loadTemplateV2ByHash(templateId, historicalHash, source)?.name, historical.name);
    assert.equal(loadTemplateV2ByHash(templateId, "f".repeat(64), source), null);
  } finally {
...
test("canonical render URLs are immutable and change exactly when the pixels change", async () => {
  const uploads: Array<{ path: string; bytes: Uint8Array; options: { contentType: string; upsert: boolean } }> = [];
  const supabase = {
    storage: {
      from: () => ({
  ...
  const first = await persistAdDocRender({
    supabase,
    workspaceId,
    bytes: new Uint8Array([1, 2, 3]),
    name: "creative-feed",
  });
  const retry = await persistAdDocRender({
    supabase,
    workspaceId,
    bytes: new Uint8Array([1, 2, 3]),
    name: "creative-feed",
  });
  const edited = await persistAdDocRender({
    supabase,
    workspaceId,
  ...
  assert.match(first, new RegExp(`^${workspaceId}/adstudio/renders/creative-feed-[0-9a-f]{64}\\.png$`));
  assert.ok(uploads.every((upload) => upload.options.contentType === "image/png" && upload.options.upsert));
  await assert.rejects(
    persistAdDocRender({ supabase, workspaceId, bytes: new Uint8Array([1]), name: "../escape" }),
    /render name is invalid/,
  );
});
