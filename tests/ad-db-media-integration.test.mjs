...
test('media collector uses verified archive without legacy overwrite or AI follow-up',async()=>{
  ...
  const fn=load('handleMediaCollector','findCapturedMediaAssetByStorage',{rest:async()=>[{id:asset,observed_ad_id:ad}],captureMediaAsset:async a=>captures.push(a.id),patchMediaAsset:async(...a)=>patches.push(a),enqueueFollowUp:()=>{throw Error('AI must not be queued')},refreshCreativeStoredMedia:()=>{throw Error('legacy public URL path must not run')}});
...
test('media collector rejects invalid scope without querying or spawning',async()=>{
 const fn=load('handleMediaCollector','findCapturedMediaAssetByStorage',{rest:()=>{throw Error('must not query')}});
...
test('capture actually starts archive CLI and verifies ad-scoped archive result',async()=>{
 ...
 const fn=load('captureMediaAsset','findMediaBlob',context);
 ...
 await assert.rejects(()=>load('captureMediaAsset','findMediaBlob',context)({id:asset,observed_ad_id:ad}),/verified object/);
});
