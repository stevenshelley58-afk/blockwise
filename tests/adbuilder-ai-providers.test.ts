...
} from "../src/lib/adbuilder/ai-providers.ts";
import { ProviderRequestError, validateProviderJsonOutput } from "../src/lib/adbuilder/providers.ts";
...
test("template analysis uses its own strict provider schema", () => {
  const valid = validateProviderJsonOutput({
    schemaName: "adBuilderTemplateAnalysis",
...
test("clone QA accepts strict numeric score strings without weakening invalid-score rejection", () => {
  ...
  const valid = validateProviderJsonOutput({
    schemaName: "adBuilderCloneQualityReview",
    rawText: JSON.stringify({
      ...base,
...
test("production exports expose only explicitly priced provider candidates", () => {
  const adapters = readFileSync("src/lib/adbuilder/ai-providers.ts", "utf8");
  const publicApi = readFileSync("src/lib/adbuilder/index.ts", "utf8");
  const googleAdapter = readFileSync("src/lib/adbuilder/google-image-provider.ts", "utf8");
  const styleProfile = readFileSync("src/lib/adbuilder/style-profile.ts", "utf8");
...
test("GPT Image 2 generations use the exact AdBuilder canvas instead of a crop-prone native portrait", async () => {
...
test("every GPT Image 2 point release keeps the exact AdBuilder canvas size", async () => {
