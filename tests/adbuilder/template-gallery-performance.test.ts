...
/**
 * Regression guard for the Ad Builder template gallery render storm.
 *
 ...
 * lazy loading. One visit to /ad-builder/templates with 61 templates issued 61
...
describe("Ad Builder template gallery stays cheap to render", () => {
  it("keeps gallery samples lazy and bounded in size", () => {
    const gallery = read("src/components/adbuilder/template-gallery.tsx");
  ...
  it("lets one gallery view render without tripping its own rate limit", () => {
    const route = read("src/app/api/adbuilder/templates/[templateId]/sample/route.ts");
