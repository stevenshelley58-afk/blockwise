...
} from "../../src/app/(customer)/ad-builder/templates/[templateId]/publish/publish-controls.ts";
...
describe("explicit Meta publish setup", () => {
  ...
  it("loads workspace-scoped saved campaign locations into the publish UI", () => {
    ...
    const page = readFileSync("src/app/(customer)/ad-builder/templates/[templateId]/publish/page.tsx", "utf8");
    const flow = readFileSync("src/app/(customer)/ad-builder/templates/[templateId]/publish/publish-flow.tsx", "utf8");
    assert.match(page, /\.from\("adbuilder_campaigns"\)/);
