...
import { needsLogoImportRecovery } from "../../src/components/adbuilder/brand-studio.tsx";

const publishFlow = readFileSync("src/app/(customer)/ad-builder/templates/[templateId]/publish/publish-flow.tsx", "utf8");
const brandStudio = readFileSync("src/components/adbuilder/brand-studio.tsx", "utf8");
...
test("legacy external logo URLs give a safe existing-flow recovery", () => {
  ...
  assert.equal(needsLogoImportRecovery("/api/adbuilder/media?path=workspace%2Flogo.png"), false);
  assert.equal(needsLogoImportRecovery("https://blockwise.sale/api/adbuilder/media?path=workspace%2Flogo.png"), false);
