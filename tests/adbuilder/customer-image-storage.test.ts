...
} from "../../src/lib/adbuilder/customer-image-storage.ts";
import { buildCustomerImageRef } from "../../src/lib/adbuilder/customer-image-ref.ts";
import { imageSha256 } from "../../src/lib/adbuilder/customer-image-hash.server.ts";
...
function fakeSupabase(initial: Map<string, Buffer> = new Map(), failUpload = false) {
  ...
  const storage = {
    from: (bucket: string) => {
      assert.equal(bucket, "adbuilder-customer-images");
      return {
...
test("uploads valid customer PNGs to the exact content-addressed workspace/ad path", async () => {
  ...
  const expectedPath = `${workspaceId}/adbuilder/ads/${adId}/images/${hash}.png`;
...
test("existing refs download and verify bytes without uploading", async () => {
  ...
  const fake = fakeSupabase(new Map([[`${workspaceId}/adbuilder/ads/${adId}/images/${hash}.png`, png]]));
...
test("cross-scope and tampered refs reject with a generic storage error", async () => {
  const hash = imageSha256(png);
  const validPath = `${workspaceId}/adbuilder/ads/${adId}/images/${hash}.png`;
