...
} from "../../src/components/adbuilder/editor/canonical-preview.ts";
...
describe("canonical editor preview handoff", () => {
  ...
  it("posts the active placement and hands the server PNG to the editor", async () => {
    ...
    controller.request({
      endpoint: "/api/adbuilder/ads/ad-1/preview?workspaceId=workspace-1",
      document,
      placement: "feed",
      debounceMs: 0,
    ...
    assert.equal(calls[0].endpoint, "/api/adbuilder/ads/ad-1/preview?workspaceId=workspace-1");
