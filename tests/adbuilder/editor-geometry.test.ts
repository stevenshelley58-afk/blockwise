...
describe("Ad Builder editor geometry contract", () => {
  it("keeps the absolute neutral just-listed feed bounds top-left anchored", async () => {
    ...
    const { resolveGeometry, fabricRectGeometry, fabricCircleGeometry } = await import("../../src/components/adbuilder/editor/layer-geometry.ts");
  ...
  it("resolves normalized pack rectangles with the same dimensions as the server renderer", async () => {
    const { resolveGeometry, effectiveTextFontSize, fabricCharSpacing, fabricPathPosition } = await import("../../src/components/adbuilder/editor/layer-geometry.ts");
  ...
  it("anchors Fabric line and icon paths using their normalized command bounds", async () => {
    ...
    const { fabricPathPosition } = await import("../../src/components/adbuilder/editor/layer-geometry.ts");
  ...
  it("maps every supported semantic icon and both divider orientations", async () => {
    const { fabricIconPathData, fabricLinePathData, resolveIconShape } = await import("../../src/components/adbuilder/editor/layer-geometry.ts");
  ...
  it("keeps rounded image mask corners at the canonical 16px radius", async () => {
    const { imageMaskRadius } = await import("../../src/components/adbuilder/editor/layer-geometry.ts");
