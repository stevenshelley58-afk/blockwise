...
import {
  ...
  resolveColourMap,
  templateCopyValues,
} from "../../src/components/adbuilder/editor/use-editor-state.ts";
import { adDocumentSchema } from "../../packages/ad-template-contract/src/schema.ts";
import { InvalidActiveRevisionError } from "../../src/lib/adbuilder/create-customer-ad.ts";
import { editorTextInputs } from "../../src/components/adbuilder/editor/use-editor-state.ts";
...
describe("new ads start with placeholders, not template copy", () => {
  ...
  it("restoring a saved ad never re-seeds template copy or flips the checkbox on", () => {
    ...
    const source = readFileSync("src/components/adbuilder/editor/use-editor-state.ts", "utf8");
...
describe("business-name override (Brand Pack default, per-ad override)", () => {
  ...
  it("buildAdDocument serializes the override only when set", async () => {
    const { buildAdDocument } = await import("../../src/components/adbuilder/editor/use-editor-state.ts");
    const pack = makeTemplate();
...
describe("malformed saved documents fail closed", () => {
  ...
  it("the stable ad route preserves the document, blocks saving, and offers read-only recovery", () => {
    const page = readFileSync("src/app/(customer)/ad-builder/ads/[id]/page.tsx", "utf8");
...
describe("Meta Feed and Story previews", () => {
  const source = readFileSync("src/components/adbuilder/editor/meta-previews.tsx", "utf8");
...
describe("publish reports active state honestly with safe retry", () => {
  const flow = readFileSync("src/app/(customer)/ad-builder/templates/[templateId]/publish/publish-flow.tsx", "utf8");
