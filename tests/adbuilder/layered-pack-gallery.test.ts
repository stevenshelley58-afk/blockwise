...
import {
  ...
  templateLeadType,
} from "../../src/lib/adbuilder/pack-gallery.ts";
...
describe("Ad Builder direct layered template gallery", () => {
  ...
  it("builds only canonical same-origin asset URLs", () => {
    assert.equal(templateAssetProxyUrl("layered-template-01", "feed-plate"), "/api/adbuilder/templates/layered-template-01/assets/feed-plate");
    assert.equal(templateAssetProxyUrl("layered-template-01", "photo:hero"), "/api/adbuilder/templates/layered-template-01/assets/photo%3Ahero");
    assert.equal(
      templateAssetProxyUrl("layered-template-01", "feed-plate", "8dc1234c-88d2-4d51-aebd-2c84a36fa8cf"),
      "/api/adbuilder/templates/layered-template-01/assets/feed-plate?adId=8dc1234c-88d2-4d51-aebd-2c84a36fa8cf",
    );
    ...
    assert.equal(
      ...
      "/api/adbuilder/templates/layered-template-01/sample?placement=feed&adId=8dc1234c-88d2-4d51-aebd-2c84a36fa8cf",
    );
    ...
    assert.equal(
      ...
      "/api/adbuilder/templates/layered-template-01/sample?placement=feed&adId=8dc1234c-88d2-4d51-aebd-2c84a36fa8cf&w=384",
    );
    assert.equal(
      gallerySampleProxyUrl("layered-template-01", "story", undefined, 384),
      "/api/adbuilder/templates/layered-template-01/sample?placement=story&w=384",
    );
    ...
    assert.equal(
      ...
      "/api/adbuilder/templates/layered-template-01/sample?placement=feed",
    );
  });
  ...
  it("uses exact component encoding and real layered previews", () => {
    ...
    const sampleRoute = readFileSync("src/app/api/adbuilder/templates/[templateId]/sample/route.ts", "utf8");
  ...
  it("keeps saved-ad routes ownership-gated and service-backed", () => {
    const editorRoute = readFileSync("src/app/(customer)/ad-builder/ads/[id]/page.tsx", "utf8");
    const publishPage = readFileSync("src/app/(customer)/ad-builder/templates/[templateId]/publish/page.tsx", "utf8");
    const assetRoute = readFileSync("src/app/api/adbuilder/templates/[templateId]/assets/[assetKey]/route.ts", "utf8");
    const sampleRoute = readFileSync("src/app/api/adbuilder/templates/[templateId]/sample/route.ts", "utf8");
    const saveRoute = readFileSync("src/app/api/adbuilder/ads/[id]/save/route.ts", "utf8");
    const renderAssets = readFileSync("src/lib/adbuilder/render-assets.ts", "utf8");
    const copyRoute = readFileSync("src/app/api/adbuilder/ads/[id]/copy-proposal/route.ts", "utf8");
    const formRoute = readFileSync("src/app/api/adbuilder/ads/[id]/instant-form/route.ts", "utf8");
