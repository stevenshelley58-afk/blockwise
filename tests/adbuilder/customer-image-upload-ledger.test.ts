...
const genericMediaRoute = readFileSync("src/app/api/adbuilder/media/route.ts", "utf8");
const nextConfig = readFileSync("next.config.ts", "utf8");

test("customer image migration creates a private, constrained bucket", () => {
  assert.match(migration, /'adbuilder-customer-images'[\s\S]*false[\s\S]*10485760/);
  assert.match(migration, /array\['image\/png', 'image\/jpeg', 'image\/webp'\]/);
  assert.match(migration, /enable row level security/);
  assert.match(migration, /revoke all on table public\.adbuilder_customer_image_uploads from public, anon, authenticated/);
  assert.match(migration, /grant all on table public\.adbuilder_customer_image_uploads to service_role/);
});
...
test("customer image ledger has bounded workspace quotas and stale cleanup", () => {
  ...
  assert.match(migration, /adbuilder_prepare_customer_image_upload/);
  assert.match(migration, /adbuilder_finalize_customer_image_upload/);
  assert.match(migration, /adbuilder_claim_customer_image_finalize/);
  assert.match(migration, /adbuilder_discard_customer_image_upload/);
  assert.match(migration, /status in \('pending', 'finalizing', 'deleting', 'finalized'\)/);
  assert.match(migration, /adbuilder_customer_image_uploads_status_check[\s\S]*add constraint adbuilder_customer_image_uploads_status_check[\s\S]*check \(status in \('pending', 'finalizing', 'deleting', 'finalized'\)\)/);
  assert.match(migration, /with claimed as \([\s\S]*update public\.adbuilder_customer_image_uploads[\s\S]*workspace_id = p_workspace_id[\s\S]*returning id, object_path[\s\S]*from claimed/);
  ...
  assert.match(migration, /adbuilder_complete_customer_image_stale_cleanup/);
...
test("customer image status hardening is idempotent for already-migrated databases", () => {
  assert.match(hardeningMigration, /if not exists \([\s\S]*adbuilder_customer_image_uploads_status_check/);
  assert.match(hardeningMigration, /add constraint adbuilder_customer_image_uploads_status_check/);
...
test("finalize checks object metadata before downloading bytes", () => {
  ...
  assert.match(mediaRoute, /adbuilder_claim_customer_image_finalize/);
  assert.ok(mediaRoute.indexOf("adbuilder_claim_customer_image_finalize") < mediaRoute.indexOf("await bucket.info(parsed.path)"));
  assert.match(mediaRoute, /ledgerResult\.status === "finalized"/);
  assert.match(mediaRoute, /adbuilder_complete_customer_image_stale_cleanup/);
  ...
  assert.match(readFileSync("src/components/adbuilder/editor/customer-image-upload.ts", "utf8"), /operation: "discard"/);
  const preparedTokenIndex = mediaRoute.indexOf("createSignedUploadUrl");
  assert.ok(preparedTokenIndex > 0);
  assert.match(readFileSync("src/components/adbuilder/editor/customer-image-upload.ts", "utf8"), /upsert: false/);
  assert.match(mediaRoute, /from\(CUSTOMER_IMAGE_BUCKET\)/);
  assert.match(proxyRoute, /from\(CUSTOMER_IMAGE_BUCKET\)/);
  assert.match(proxyRoute, /adbuilder_customer_image_uploads/);
  assert.match(proxyRoute, /status", "finalized"/);
  assert.match(genericMediaRoute, /workspace-artifacts/);
  assert.doesNotMatch(genericMediaRoute, /adbuilder_customer_image_uploads/);
});

test("AdBuilder image routes trace Sharp's Linux runtime on Vercel", () => {
  assert.match(nextConfig, /outputFileTracingIncludes/);
  assert.match(nextConfig, /"\/api\/adbuilder\/ads\/\*\/media"/);
  assert.match(nextConfig, /"\/api\/adbuilder\/customer-media"/);
