...
import { ingestTemplateArtifact } from "../../src/lib/adbuilder/ingest-artifact.ts";
...
describe("direct template route and migration contract", () => {
  it("uses the sole shared schema and maps conflicts to HTTP 409", () => {
    const ingest = readFileSync("src/lib/adbuilder/ingest-artifact.ts", "utf8");
    const route = readFileSync("src/app/api/internal/adbuilder/template-artifacts/route.ts", "utf8");
  ...
  it("verifies internal HMAC requests and rejects unauthenticated calls", () => {
    const route = readFileSync("src/app/api/internal/adbuilder/template-artifacts/route.ts", "utf8");
    assert.match(route, /verifyInternalRequest\(request, "adbuilder\.templates"/);
  ...
  it("keeps legacy customer data while allowing direct-template customer inserts", () => {
    ...
    const createCustomerAd = readFileSync("src/lib/adbuilder/create-customer-ad.ts", "utf8");
