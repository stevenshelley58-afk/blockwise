...
import { applyTextValuesToScenes, convertTemplateToFabricScenes, nativeTemplateFonts, readVueNativeEditor, textValuesFromScenes } from "../../src/components/adbuilder/vue-editor/fabric-scene.ts";
...
test("template conversion creates full-size Fabric scenes with an editor workspace", () => {
  ...
  assert.match(String(image?.src), /^\/api\/adbuilder\/templates\/fixture-minimal\/assets\/photo\?adId=ad-1$/);
});
...
test("conversion refuses external image sources instead of claiming editable fidelity", () => {
 ...
 test("native editor resolves both declared and bundled fonts instead of silently substituting", () => {
  const template = pack();
  template.fonts.push({ file: "/fonts/adbuilder/manrope-600.woff2" });
  const fonts = nativeTemplateFonts(template, "ad-1");
  assert.equal(fonts.length, 2);
  assert.match(fonts[0].url, /assets\/font\?adId=ad-1$/);
  assert.equal(fonts[1].url, "/fonts/adbuilder/manrope-600.woff2");
  assert.equal(fonts[1].family, "Blockwise_fixture-minimal_%2Ffonts%2Fadbuilder%2Fmanrope-600.woff2");
});
