...
import { documentToken } from "../../src/lib/adbuilder/document-token.ts";
import { loadNativeTrialIdentity, nativeTrialCreationKey } from "../../src/lib/adbuilder/vue-native-copy.ts";
import { NativeSaveError, saveNativeAd } from "../../src/lib/adbuilder/vue-native-save.ts";
import {
  NATIVE_RENDERER_VERSION,
  NativeEditorValidationError,
  validateNativeEditorDocument,
  validateNativePngExport,
} from "../../src/lib/adbuilder/vue-native-validation.ts";
...
describe("Vue native scene and export validation", () => {
  ...
  it("rejects arbitrary scene URLs and malformed or wrong-sized PNG exports", async () => {
    ...
    assert.doesNotThrow(() => validateNativeEditorDocument({ nativeEditor: { ...native, feed: scene(1080, 1350, { type: "image", src: `/api/adbuilder/templates/${templateId}/assets/photo%3Ahero?adId=${trialAdId}` }) }, workspaceId, adId: trialAdId, templateId }));
...
describe("Vue native revision persistence", () => {
  it("stores exact native scenes and browser PNG hashes in the active-revision transaction", async () => {
    ...
    assert.match(revision.feed_png_path, new RegExp(`^${workspaceId}/adbuilder/renders/${trialAdId}/feed-`));
