...
import { applyTextValuesToScenes, nativeImageSlots, replaceNativeImageInScenes, type FabricObject, type FabricScene } from "../../src/components/adbuilder/vue-editor/fabric-scene.ts";
...
const photo = (extra: FabricObject = {}): FabricObject => ({
  type: "image", id: "photo", inputKey: "photo", src: "/api/adbuilder/media/original",
...
test("simple photos list only surviving bound slots, including groups and empty placeholders", () => {
  ...
  assert.deepEqual(nativeImageSlots(input), [
    { key: "photo", src: "/api/adbuilder/media/original", placements: ["feed", "story"] },
...
test("photo replacement center-crops without distortion, preserving each format's affine layout", () => {
  ...
  const next = replaceNativeImageInScenes(input, "photo", "/api/adbuilder/media/new", { width: 1600, height: 900 });
  for (const placement of ["feed", "story"] as const) {
    ...
    assert.equal(current.src, "/api/adbuilder/media/new");
  }
...
test("a removed slot is not recreated and unrelated objects stay unchanged", () => {
  ...
  const next = replaceNativeImageInScenes(input, "photo", "/api/adbuilder/media/new", { width: 1200, height: 1200 });
