...
describe("customer Ad Builder workbench contract", () => {
  it("keeps the layered preview behind one progressive inspector", () => {
    const source = readFileSync("src/components/adbuilder/editor/editor-shell.tsx", "utf8");
  ...
  it("uses the route workbench flow without fixed viewport shells", () => {
    const editorRoute = readFileSync("src/app/(customer)/ad-builder/templates/[templateId]/page.tsx", "utf8");
    const stableEditorRoute = readFileSync("src/app/(customer)/ad-builder/ads/[id]/page.tsx", "utf8");
    const publishRoute = readFileSync("src/app/(customer)/ad-builder/templates/[templateId]/publish/page.tsx", "utf8");
  ...
  it("keeps Fabric geometry aligned with the pack x/y contract", () => {
    const canvas = readFileSync("src/components/adbuilder/editor/layered-canvas.tsx", "utf8");
    const geometry = readFileSync("src/components/adbuilder/editor/layer-geometry.ts", "utf8");
    const editor = readFileSync("src/components/adbuilder/editor/editor-shell.tsx", "utf8");
  ...
  it("keeps the compact Ads hub and exact edit/review destinations", () => {
    ...
    const command = readFileSync("src/components/adbuilder/home-command.tsx", "utf8");
    ...
    assert.match(command, /aria-label="Ad Builder links"/);
  ...
  it("keeps editor inputs rounded, labelled, and progressive", () => {
    const inputs = readFileSync("src/components/adbuilder/editor/inputs-panel.tsx", "utf8");
    const copy = readFileSync("src/components/adbuilder/editor/meta-copy-panel.tsx", "utf8");
    const shell = readFileSync("src/components/adbuilder/editor/editor-shell.tsx", "utf8");
    const colours = readFileSync("src/components/adbuilder/editor/colour-toggle.tsx", "utf8");
  ...
  it("links the clicked ad area and the field that edits it", () => {
    ...
    const previews = readFileSync("src/components/adbuilder/editor/meta-previews.tsx", "utf8");
  ...
  it("shows template defaults, recovers stale saves, and keeps publishing choices explicit", () => {
    const shell = readFileSync("src/components/adbuilder/editor/editor-shell.tsx", "utf8");
    const state = readFileSync("src/components/adbuilder/editor/use-editor-state.ts", "utf8");
    const inputs = readFileSync("src/components/adbuilder/editor/inputs-panel.tsx", "utf8");
    const publish = readFileSync("src/app/(customer)/ad-builder/templates/[templateId]/publish/publish-flow.tsx", "utf8");
    const instantForm = readFileSync("src/components/adbuilder/instant-form-editor.tsx", "utf8");
    ...
    assert.match(publish, /variantIds: variants/);
    assert.match(publish, /hidden=\{stage !== 1\}/);
    assert.match(publish, /captureReady/);
    assert.match(publish, /build.controls/);
    assert.match(publish, /Edit ad/);
    assert.match(publish, /This ad promises an offer, guide or result/);
    assert.match(publish, /fulfilmentRequired: publishRequirements\.fulfilmentRequired/);
    assert.match(publish, /Offer delivery URL/);
    assert.match(publish, /Approve & publish/);
    assert.match(publish, /Why use this campaign setup/);
