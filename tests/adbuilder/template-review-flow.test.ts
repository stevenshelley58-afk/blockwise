...
test("legacy review metadata remains readable", () => {
  ...
  assert.equal(adTemplateSchema.safeParse(legacy).success, true);
});
...
test("review endpoint is run-bound, import stays quarantined, and product migration applies it", () => {
  const route = readFileSync("src/app/api/internal/adbuilder/template-artifacts/[templateId]/review/route.ts", "utf8");
  ...
  assert.match(route, /adbuilder\.templates\.review/);
...
test("customer view defaults to the complete Meta ad while artwork remains inspectable", () => {
  const detail = readFileSync("src/components/adbuilder/template-detail-preview.tsx", "utf8");
