...
import { normaliseAdDbRow } from "../src/lib/research/ad-db.ts";
...
test("ad DB only emits locally verified archive media routes", () => {
  const record = normaliseAdDbRow({
    ...
    media: [
      { id: "asset-ok", kind: "image", storageBucket: "research-ad-creatives", objectKey: "sha256/" + "a".repeat(64), sha256: "a".repeat(64), byteSize: 4, mimeType: "image/jpeg", width: 1, height: 1 },
