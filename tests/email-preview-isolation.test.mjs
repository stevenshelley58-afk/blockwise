...
test("email preview allows only read-only design review and never calls a provider", async () => {
  ...
  try {
    for (const [path, method, expected] of [
      ...
      ["/email-assets/sample-seller-appraisal.jpg", "GET", 200],
      ["/email-assets/sample-buyer-demand.jpg", "HEAD", 200],
      ["/email-assets/sample-seller-appraisal.jpg", "POST", 405],
      ["/email-assets/private.json", "GET", 404],
      ["/api/health", "GET", 404],
