...
const page = "src/app/(customer)/ad-builder/video/page.tsx";
const component = "src/components/adbuilder/video/video-home.tsx";
const shell = "src/components/adbuilder/studio-shell.tsx";

test("the Video surface is reachable from Ad Builder navigation", () => {
  const nav = read(shell);
  assert.match(nav, /href: "\/ad-builder\/video"/, "Studio nav must link to the video route");
...
test("the page reads the customer's own records, so RLS applies", () => {
  const source = read(page);
  assert.match(source, /requirePageSurfaceAccess\("adbuilder"\)/);
