...
test("clone route runs cascade + QA reroll and never ships an unverified clone silently", () => {
  ...
  assert.match(route, /resolveRuntimeModelProfile\(tier === "preview" \? "image_draft" : "image_final"\)/);
  assert.match(route, /createImageProviderForCandidate/);
  assert.match(route, /createOpenAiImageProvider\(\)/);
  ...
  assert.match(route, /recordAdStudioProviderRun/);
