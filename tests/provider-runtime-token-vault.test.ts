...
test("Google runtime credentials use the same encrypted service-role-only vault lane", { concurrency: false }, async () => {
  const previousKey = process.env.TOKEN_ENCRYPTION_KEY;
  process.env.TOKEN_ENCRYPTION_KEY = encryptionKey;
  let stored: Record<string, unknown> | undefined;
  const service = {
    async rpc(name: string, args: Record<string, unknown>) {
      ...
      if (name === "runtime_provider_token_vault_get") {
        return { data: stored ? [{ encrypted_access_token: stored.p_encrypted_access_token, token_nonce: stored.p_token_nonce }] : [], error: null };
      }
      throw new Error(`Unexpected RPC ${name}`);
    },
  } as unknown as Parameters<typeof loadRuntimeProviderToken>[0];
  try {
    await upsertRuntimeProviderToken({ serviceSupabase: service, provider: "google", accessToken: "google-runtime-1234" });
    assert.equal(await loadRuntimeProviderToken(service, "google"), "google-runtime-1234");
    assert.equal(stored?.p_runtime_provider, "google");
    assert.notEqual(stored?.p_encrypted_access_token, "google-runtime-1234");
  } finally {
    if (previousKey === undefined) delete process.env.TOKEN_ENCRYPTION_KEY;
    else process.env.TOKEN_ENCRYPTION_KEY = previousKey;
  }
