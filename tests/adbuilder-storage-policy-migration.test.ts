...
test("workspace artifact reads use the canonical workspace access helper", () => {
  ...
  assert.match(sql, /private\.adbuilder_has_workspace_access\(/i);
  assert.match(sql, /private\.workspace_id_from_storage_path\(name\)/i);
  assert.doesNotMatch(sql, /public\.(?:is_workspace_member|has_workspace_role|adbuilder_has_workspace_access)\(/i);
  assert.equal(
