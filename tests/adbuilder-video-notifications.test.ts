...
} from "../src/lib/adbuilder/video-notifications.ts";
...
test("every message links to the authenticated surface instead", async () => {
  for (const kind of KINDS) {
    ...
    const payload = outbox.queued[0].payload as Record<string, unknown>;
    assert.match(String(payload.html), /\/ad-builder\/video/, `${kind} must link to the video page`);
    assert.match(String(payload.text), /\/ad-builder\/video/);
  }
});
