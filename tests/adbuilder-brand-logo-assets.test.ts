...
import { storeBrandKitLogoAssets } from "../src/lib/adbuilder/brand-logo-assets.server.ts";
import { buildAdBuilderFallbackBrandKit } from "../src/lib/adbuilder/trial-brand-kit.ts";
...
function brandKit() {
  const kit = buildAdBuilderFallbackBrandKit({
...
test("stores an extracted logo behind the authenticated same-origin media route", async () => {
  ...
  assert.match(result.brandKit.logos.primaryLogoUrl ?? "", /^\/api\/adbuilder\/media\?path=/);
