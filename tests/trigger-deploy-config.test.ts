...
test("GitHub deploys Trigger.dev tasks after main branch checks pass", () => {
  ...
  assert.match(keySyncAction, /env add TRIGGER_SECRET_KEY "\$environment" ""/);
  ...
  assert.match(keySyncAction, /--value "\$value"/);
  assert.match(keySyncAction, /env add "\$variable" "\$environment"[\s\S]*--yes/);
