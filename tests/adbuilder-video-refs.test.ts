...
import {
  ...
  versionObjectPath,
  visibilityForKind,
} from "../src/lib/adbuilder/video-refs.ts";
...
test("media references are API routes, never signed urls", () => {
  ...
  assert.ok(ref.startsWith("/api/adbuilder/videos/media?"));
...
test("a forged media reference cannot smuggle another workspace path", () => {
  ...
  const forged = `/api/adbuilder/videos/media?workspaceId=${WS}&assetId=${ASSET}&path=${encodeURIComponent(foreign)}`;
