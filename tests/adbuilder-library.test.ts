...
import {
  ...
  filterAndSortAssets,
  filterAndSortAds,
} from "../src/lib/adbuilder/library-contract.ts";
import { loadAdBuilderLibraryPage } from "../src/lib/adbuilder/library-read-model.ts";
...
test("library read model stays workspace-scoped and exposes asset metadata", async () => {
  ...
  const page = await loadAdBuilderLibraryPage({ supabase: client, workspaceId: "workspace-1", kind: "assets", limit: 24 });
...
test("the Library route unifies ads and assets without retiring their feature routes", () => {
  const page = readFileSync("src/app/(customer)/ad-builder/library/page.tsx", "utf8");
  const shell = readFileSync("src/components/adbuilder/studio-library.tsx", "utf8");
  const ads = readFileSync("src/components/adbuilder/ads-library.tsx", "utf8");
  const assets = readFileSync("src/components/adbuilder/media-library.tsx", "utf8");
