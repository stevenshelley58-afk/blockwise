...
function normaliseHostedMetaAd(raw: Record<string, unknown>, pageId: string): MetaAdLibraryAd {
  ...
  const videoUrls = collectStrings(
    ...
    pick(snapshot, "videos", "video_urls", "ad_creative_videos"),
  ...
  return {
    adArchiveID: adId,
    id: adId,
    pageID: String(pick(raw, "pageID", "pageId", "page_id") ?? pageId),
    pageName: firstString(pick(raw, "pageName", "page_name"), pick(snapshot, "pageName", "page_name")),
