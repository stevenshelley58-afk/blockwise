...
for (const audience of ['first_ad', 'returning'] as const) {
  test(`${audience} shows the preview fixture with correct template targets`, async ({ page }, info) => {
    ...
    const safe = { creativeSuggestions: { status: 'ready', audience, items: [
      { templateId: 'qa-home-a', name: 'A fresh perspective', previewUrl: '/api/home-preview-fixture/a', href: '/ad-builder/templates/qa-home-a' },
      { templateId: 'qa-home-b', name: 'Your next conversation', previewUrl: '/api/home-preview-fixture/b', href: '/ad-builder/templates/qa-home-b' },
    ] } };
    ...
    await expect(use).toHaveAttribute('href', /^\/ad-builder\/templates\/[^/]+$/);
    ...
    await expect(page).toHaveURL(/\/ad-builder\/templates/);
