...
const storageStatePath = process.env.ADBUILDER_E2E_STORAGE_STATE ?? "e2e/.auth/adbuilder-test.storage-state.json";
const workspaceId = process.env.ADBUILDER_E2E_WORKSPACE_ID;
const templateId = process.env.ADBUILDER_E2E_TEMPLATE_ID;
const baseUrl = process.env.PLAYWRIGHT_BASE_URL;
const loginBaseUrl = process.env.ADBUILDER_E2E_LOGIN_URL || baseUrl;
const email = process.env.ADBUILDER_E2E_EMAIL;
const password = process.env.ADBUILDER_E2E_PASSWORD;
...
const allowDryRunPublish = process.env.ADBUILDER_E2E_PUBLISH_TEST_MODE === "dry-run";
...
const missingPreconditions = [
  ...
  loginBaseUrl && !loginBaseUrlIsAllowed && "ADBUILDER_E2E_LOGIN_URL (must be https://blockwise.sale or this project's Vercel deployment)",
  !hasWorkspace && "ADBUILDER_E2E_WORKSPACE_ID",
  !hasTemplate && "ADBUILDER_E2E_TEMPLATE_ID (must identify an available template)",
  !hasAuthState && `authenticated storage state at ${storageStatePath}`,
].filter(Boolean);
if ((!canRun || !canLogin) && process.env.CI) {
  test("Ad Builder real-loop preconditions are present in CI", () => {
    throw new Error(`Ad Builder E2E preconditions missing or invalid: ${[...missingPreconditions, !email && "ADBUILDER_E2E_EMAIL", !password && "ADBUILDER_E2E_PASSWORD"].filter(Boolean).join(", ")}.`);
  });
}

test.describe("Ad Builder login", () => {
  test.skip(!canLoginAndCreate, "Set PLAYWRIGHT_BASE_URL, ADBUILDER_E2E_EMAIL, ADBUILDER_E2E_PASSWORD, ADBUILDER_E2E_WORKSPACE_ID and ADBUILDER_E2E_TEMPLATE_ID.");
  ...
  test("logs in and creates an ad in one authenticated flow", async ({ page }) => {
    ...
    if (loginUsesDifferentOrigin) {
      await page.goto(`${baseUrl!.replace(/\/+$/, "")}/ad-builder?workspaceId=${encodeURIComponent(workspaceId!)}`);
...
test.describe("Ad Builder authenticated real loop", () => {
  test.skip(!canRun, "Set PLAYWRIGHT_BASE_URL, ADBUILDER_E2E_WORKSPACE_ID, ADBUILDER_E2E_TEMPLATE_ID and authenticated storage state.");
  ...
  for (const viewport of [{ width: 1440, height: 900 }, { width: 768, height: 1024 }, { width: 390, height: 844 }, { width: 320, height: 844 }]) {
    test(`opens the gallery without overflow at ${viewport.width}x${viewport.height}`, async ({ page }) => {
      ...
      const templateHref = `/ad-builder/templates/${encodeURIComponent(templateId!)}`;
      const templateCard = page.locator(`a[href="${templateHref}"]`);
      await expect(templateCard, `ADBUILDER_E2E_TEMPLATE_ID=${templateId} is not visible in the gallery at ${viewport.width}x${viewport.height}`).toBeVisible();
...
async function runEditorFlow(page: Page) {
  ...
  await page.addInitScript(() => localStorage.setItem("bw-consent", "essential"));
  await page.goto(`/ad-builder?workspaceId=${encodeURIComponent(workspaceId!)}`);
  const template = page.locator(`a[href="/ad-builder/templates/${encodeURIComponent(templateId!)}"]`);
  await expect(template, `ADBUILDER_E2E_TEMPLATE_ID=${templateId} is not present in the template gallery`).toHaveCount(1);
  await expect(template).toBeVisible();
  await template.click();
  const editor = page.getByRole("region", { name: "Ad Builder editor" });
  ...
  for (const ref of Object.values(firstDocument?.sharedImageValues ?? {})) {
    expect(ref).toContain("/api/adbuilder/customer-media?");
  ...
  await expect(page).toHaveURL(/\/ad-builder\/templates\/[^/]+\/publish(?:\?|$)/);
  ...
  for (const frozenImage of [frozenFeed, frozenStory]) {
    ...
    expect(src, "frozen creative should expose a same-origin media export").toMatch(/^\/api\/adbuilder\/media\?path=/);
  ...
  if (allowDryRunPublish) {
    ...
    const publishRequest = page.waitForRequest(request => request.url().includes("/api/adbuilder/ads/") && request.url().includes("/publish?") && request.method() === "POST");
    const publishResponse = page.waitForResponse(response => response.url().includes("/api/adbuilder/ads/") && response.url().includes("/publish?") && response.request().method() === "POST");
  ...
  await page.goto(`/ad-builder/templates/${encodeURIComponent(templateId!)}`);
...
async function saveEditor(page: Page): Promise<SaveResult> {
  const responsePromise = page.waitForResponse(response => response.url().includes("/api/adbuilder/ads/") && response.url().includes("/save?") && response.request().method() === "POST");
  const requestPromise = page.waitForRequest(request => request.url().includes("/api/adbuilder/ads/") && request.url().includes("/save?") && request.method() === "POST");
