...
test.use({
  storageState: process.env.ADBUILDER_E2E_STORAGE_STATE ?? "/srv/blockwise/secrets/adbuilder-e2e.storage-state.json",
...
test("canonical PNG follows edits and every editor preview mode without saving", async ({ page }) => {
  ...
  await page.goto("/ad-builder/ads/" + adId + "?workspaceId=" + workspaceId);
