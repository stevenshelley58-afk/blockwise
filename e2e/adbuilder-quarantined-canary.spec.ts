...
const adId = process.env.ADBUILDER_E2E_AD_ID;
const storageStatePath = process.env.ADBUILDER_E2E_STORAGE_STATE ?? "e2e/.auth/adbuilder-test.storage-state.json";
const dryRunEnabled = process.env.ADBUILDER_E2E_PUBLISH_TEST_MODE === "dry-run";
...
test.describe("quarantined 006 production editor canary", () => {
  test.skip(!canRun, "Requires the dedicated storage state, workspace, quarantined template/ad IDs, and ADBUILDER_E2E_PUBLISH_TEST_MODE=dry-run.");
  ...
  test("is hidden from discovery but opens through its owned test-ad route", async ({ page }) => {
    ...
    await expect(page.locator(`a[href="/ad-builder/templates/${encodeURIComponent(templateId!)}"]`)).toHaveCount(0);
    ...
    await expect(page).toHaveURL(new RegExp(`/ad-builder/ads/${adId}$`));
  ...
  test("edits, reviews, persists, and drafts a provider-write-disabled PAUSED plan", async ({ page }) => {
    ...
    const proposalResponse = page.waitForResponse(response => response.url().includes(`/api/adbuilder/ads/${adId}/copy-proposal?`) && response.request().method() === "POST");
    const proposalRequest = page.waitForRequest(request => request.url().includes(`/api/adbuilder/ads/${adId}/copy-proposal?`) && request.method() === "POST");
    ...
    const saveResponse = page.waitForResponse(response => response.url().includes(`/api/adbuilder/ads/${adId}/save?`) && response.request().method() === "POST");
    const saveRequest = page.waitForRequest(request => request.url().includes(`/api/adbuilder/ads/${adId}/save?`) && request.method() === "POST");
    ...
    await expect(page).toHaveURL(new RegExp(`/ad-builder/templates/${templateId}/publish\\?adId=${adId}$`));
    ...
    const publishResponse = page.waitForResponse(response => response.url().includes(`/api/adbuilder/ads/${adId}/publish?`) && response.request().method() === "POST");
    await freeze.click();
...
function editorUrl() {
  return `/ad-builder/ads/${encodeURIComponent(adId!)}`;
}
