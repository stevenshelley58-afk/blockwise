...
const output = '/root/work/adstudio-vue-host-20260912';
...
const scene = height => ({ version:'5.3.0', width:1080, height, objects:[
  ...
  { type:'rect', id:'shape', width:920, height:650, left:80, top:90, fill:'#23483e', originX:'left', originY:'top' },
...
const props = () => ({ pack:template, adId:'trial-ad', workspaceId:'workspace-1', initialDocument:document, initialRevision:revision, sourceAdId:'source-ad', businessName:'Synthetic test agency', logoUrl:null, libraryAssets:[] });
...
try{
  ...
  const save=page.getByRole('button',{name:'Save',exact:true});await save.waitFor();
  ...
  await page.getByRole('button',{name:'Copy & AI',exact:true}).click();
  await page.getByLabel('On-image headline',{exact:true}).fill('Edited copy');
  await page.getByRole('button',{name:'Generate proposal'}).click();
  await page.getByText('Proposed headline',{exact:false}).waitFor();
  assert.equal(await page.getByLabel('On-image headline',{exact:true}).inputValue(),'Edited copy');
  await page.getByRole('button',{name:'Apply proposal'}).click();
  assert.equal(await page.getByLabel('On-image headline',{exact:true}).inputValue(),'Proposed headline');
  await page.getByRole('button',{name:'Close',exact:true}).click();
  await save.click();await page.getByRole('status').filter({hasText:/^Saved$/}).waitFor();
  assert.equal(saves,1);assert.equal(proposals,1);assert.equal(document.nativeEditor.feed.objects.find(object=>object.id==='headline').text,'Proposed headline');
  assert.equal(document.nativeEditor.story.objects.find(object=>object.id==='headline').text,'Proposed headline');
  await page.reload();await page.waitForFunction(()=>[...document.querySelectorAll('button')].some(button=>button.textContent==='Save'&&!button.disabled));
  await page.getByRole('button',{name:'Ad preview',exact:true}).click();
  ...
  await writeFile(resolve(output,'receipt.json'),JSON.stringify({status:'passed',scope:'mocked component harness',saves,proposals,checks:['copy proposal is explicit','text updates both sizes','real bridge snapshot meets backend validators','save and reopen','ad preview','publish review navigation','desktop and mobile']},null,2));
  console.log('Host harness passed: copy, AI proposal/apply, native snapshot/save/reopen, ad preview, publish review navigation, desktop/mobile.');
