// Seeds the dedicated Ad Builder e2e user + workspace (idempotent).
//
// Used by the AdBuilder E2E (Vercel Preview) workflow before running
...
//      ADBUILDER_E2E_EMAIL (default adbuilder-e2e@blockwise.test),
//      ADBUILDER_E2E_PASSWORD (required, >= 16 chars),
//      ADBUILDER_E2E_TEMPLATE_ID (optional quarantined template to attach to a
//      deterministic test ad; the script prints ADBUILDER_E2E_AD_ID).
...
export const ADBUILDER_E2E_WORKSPACE_ID = "00000000-0000-4000-8000-0000000000e2";
...
const email = (cleanEnv(process.env.ADBUILDER_E2E_EMAIL) ?? "adbuilder-e2e@blockwise.test").toLowerCase();
const password = cleanEnv(process.env.ADBUILDER_E2E_PASSWORD);
const quarantinedTemplateId = cleanEnv(process.env.ADBUILDER_E2E_TEMPLATE_ID);
...
if (!password || password.length < 16) {
  throw new Error("Set ADBUILDER_E2E_PASSWORD to a unique password of at least 16 characters.");
}
...
const attributes = {
  password,
  email_confirm: true,
  user_metadata: { full_name: "AdBuilder E2E" },
};
...
requireNoError(
  await supabase.from("profiles").upsert(
    { id: authUser.id, email, full_name: "AdBuilder E2E", is_operator: false },
    { onConflict: "id" },
  ),
  "Upsert profile",
);
...
requireNoError(
  await supabase.from("workspace_members").upsert(
    { workspace_id: ADBUILDER_E2E_WORKSPACE_ID, profile_id: authUser.id, role: "owner" },
...
requireNoError(
  await supabase.from("adbuilder_brand_kits").upsert(
    {
      id: "00000000-0000-4000-8000-0000000000e3",
      workspace_id: ADBUILDER_E2E_WORKSPACE_ID,
      source_type: "manual",
      source_url: "https://blockwise.sale",
      business_name: "AdBuilder E2E Realty",
      market_country: "AU",
      market_region: "WA",
      identity_json: { businessName: "AdBuilder E2E Realty", tradingName: "AdBuilder E2E Realty" },
      logos_json: {},
      colours_json: {
        primary: "#123E75",
        secondary: "#F1F5F9",
  ...
  "Upsert AdBuilder e2e Brand Pack",
);
...
const periodKey = `adbuilder-e2e-${periodStart.toISOString().slice(0, 7)}`;
requireNoError(
  await supabase.rpc("grant_workspace_credits", {
    p_workspace_id: ADBUILDER_E2E_WORKSPACE_ID,
    p_entitlement_type: "operator",
    p_period_key: periodKey,
    p_credits: 6,
    p_period_start: periodStart.toISOString(),
    p_period_end: periodEnd.toISOString(),
    p_mutation_key: `adbuilder-e2e:credit-grant:${periodStart.toISOString().slice(0, 7)}:v1`,
    p_source_reference: "adbuilder-e2e-fixture",
    p_metadata: { fixture: true, scope: "preview-real-loop" },
  }),
  "Grant AdBuilder e2e render credits",
);
...
if (quarantinedTemplateId) {
  ...
  const creationKey = `adbuilder-e2e:quarantined-canary:${quarantinedTemplateId}`;
  const existingAd = requireNoError(
    await supabase
      .from("ad_customer_ads")
      .select("id, template_id")
      .eq("workspace_id", ADBUILDER_E2E_WORKSPACE_ID)
...
console.log(`Seeded AdBuilder e2e fixture: ${email} → workspace ${ADBUILDER_E2E_WORKSPACE_ID}`);
if (quarantinedAdId) console.log(`ADBUILDER_E2E_AD_ID=${quarantinedAdId}`);
