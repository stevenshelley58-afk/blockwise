...
const server=createServer(async(req,res)=>{
  ...
  if(req.url.startsWith("/api/")){
    assert.equal(req.method,"POST");assert.equal(req.url,"/api/adbuilder/ads/original-ad/vue-copy?workspaceId=workspace-1");calls++;
...
try {
  ...
  await page.waitForFunction(()=>window.lastNavigation==="/ad-builder/ads/native-ad/canvas");
