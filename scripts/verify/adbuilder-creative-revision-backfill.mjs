...
const fixturePath = "scripts/verify/fixtures/adbuilder-creative-revisions-pre-migration.sql";
...
const seededCount = psql(["-A", "-t", "-c", "select count(*) from public.adbuilder_creatives;"]);
if (seededCount !== "427") {
...
const result = psql([
  ...
  "-c",
  `select
    (select count(*) from public.adbuilder_creatives),
    (select count(*) from public.adbuilder_creative_revisions where creation_operation = 'migration_backfill'),
    (select count(*) from public.adbuilder_creatives where active_revision_id is null),
