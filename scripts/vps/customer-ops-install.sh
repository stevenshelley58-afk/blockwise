...
usage() {
  cat >&2 <<'EOF'
usage: customer-ops-install.sh --env-file /etc/blockwise/customer-ops/customer-ops.env [--check|--apply]
...
secret_names=(postgres_owner_password chatwoot_db_password snagtime_db_password frank_projection_db_password mautic_db_root_password mautic_db_password chatwoot_secret_key_base google_client_secret smtp_password snagtime_auth_secret snagtime_token_encryption_key)
