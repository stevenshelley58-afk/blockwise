...
HERMES_DEFAULT_MODEL="${HERMES_DEFAULT_MODEL:-kimi-k2.6}"
...
if [ -z "$supabase_secret_key" ] && [ -z "$supabase_service_role_key" ]; then
  echo "missing SUPABASE_SECRET_KEY or SUPABASE_SERVICE_ROLE_KEY in /opt/blockwise/.env" >&2
  exit 1
fi
: "${MOONSHOT_API_KEY:?missing MOONSHOT_API_KEY in /opt/blockwise/.env}"
...
HERMES_DEFAULT_MODEL=${HERMES_DEFAULT_MODEL:-kimi-k2.6}
HERMES_MODELS_JSON=${HERMES_MODELS_JSON:-{}}
...
HERMES_SUPABASE_SECRET_KEY=$supabase_secret_key
...
HERMES_META_CAPTURE_PROXY_URL=${HERMES_META_CAPTURE_PROXY_URL:-}
