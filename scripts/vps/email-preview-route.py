#!/usr/bin/env python3
"""Install/remove only the isolated email preview route; never replace the app."""
import argparse
import copy
import datetime
import json
...
ROUTE_ID = "blockwise-email-preview"
...
def main():
    ...
    parser.add_argument("--upstream", default="blockwise-email-preview:3000")
    args = parser.parse_args()
    if not re.fullmatch(r"blockwise-email-preview(?:-[a-f0-9]{8,40})?:3000", args.upstream):
        raise SystemExit("Only the isolated email preview upstream is allowed")
    original = config()
    ...
    if not args.remove:
        routes.insert(0, {
            "@id": ROUTE_ID,
            "match": [{"host": ["blockwise.sale"], "path": ["/email-preview", "/email-preview/*"]}],
    ...
    backup = Path("/srv/blockwise/previews/email")
