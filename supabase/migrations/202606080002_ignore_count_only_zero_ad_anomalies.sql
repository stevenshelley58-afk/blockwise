-- Count-only Meta location-search shells should not create zero-ad anomaly noise.
-- They prove Meta reported a result count, but did not expose ad rows in the page payload.
...
begin
  for r in
    ...
    where afr.status in ('success', 'partial')
      ...
      and coalesce((afr.result_summary->'metadata'->>'count_only')::boolean, false) = false
