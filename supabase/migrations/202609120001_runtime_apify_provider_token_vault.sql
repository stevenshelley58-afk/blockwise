-- Allow the Hermes first-fill worker to fetch only the encrypted Apify token.
-- The token remains in private.provider_token_vault and access remains service_role-only.
...
alter table private.provider_token_vault
  add constraint provider_token_vault_scope_check check (
    ...
    (
      ...
      and runtime_provider in ('openai', 'google', 'apify')
    )
  );
...
as $$
  ...
  where v.runtime_provider = p_runtime_provider
    and p_runtime_provider in ('openai', 'google', 'apify');
$$;
...
as $$
begin
  if p_runtime_provider not in ('openai', 'google', 'apify') then
