-- Customer AdBuilder images live in their own private bucket.  The bucket
-- limits are enforced by Storage as well as by the application routes.
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values (
  'adbuilder-customer-images',
  'adbuilder-customer-images',
  false,
  10485760,
...
create table if not exists public.adbuilder_customer_image_uploads (
  ...
  check (object_path ~ ('^' || workspace_id::text || '/adbuilder/ads/' || ad_id::text || '/images/[a-f0-9]{64}\.(png|jpg|webp)$')),
...
begin
  alter table public.adbuilder_customer_image_uploads
    drop constraint if exists adbuilder_customer_image_uploads_status_check;
  alter table public.adbuilder_customer_image_uploads
    drop constraint if exists adbuilder_customer_image_uploads_status_finalized_at_check;
  if not exists (
  ...
  if not exists (
    select 1 from pg_constraint
     where conrelid = 'public.adbuilder_customer_image_uploads'::regclass
       and conname = 'adbuilder_customer_image_uploads_status_finalized_at_check'
  ) then
    alter table public.adbuilder_customer_image_uploads
      add constraint adbuilder_customer_image_uploads_status_finalized_at_check
      check ((status in ('pending', 'finalizing', 'deleting') and finalized_at is null) or (status = 'finalized' and finalized_at is not null));
  end if;
end;
$$;

create index if not exists adbuilder_customer_image_uploads_workspace_status_idx
  on public.adbuilder_customer_image_uploads (workspace_id, status, expires_at);

alter table public.adbuilder_customer_image_uploads enable row level security;
revoke all on table public.adbuilder_customer_image_uploads from public, anon, authenticated;
grant all on table public.adbuilder_customer_image_uploads to service_role;
drop policy if exists adbuilder_customer_image_uploads_service_role_all on public.adbuilder_customer_image_uploads;
create policy adbuilder_customer_image_uploads_service_role_all
  on public.adbuilder_customer_image_uploads
  as permissive for all to service_role
  using (true) with check (true);

create or replace function public.adbuilder_prepare_customer_image_upload(
  p_workspace_id uuid,
  p_ad_id uuid,
  p_object_path text,
  p_sha256 text,
...
begin
  ...
  if p_byte_size is null or p_byte_size <= 0 or p_byte_size > 10485760
     ...
     or p_object_path !~ ('^' || p_workspace_id::text || '/adbuilder/ads/' || p_ad_id::text || '/images/[a-f0-9]{64}\.(png|jpg|webp)$') then
  ...
  with claimed as (
    update public.adbuilder_customer_image_uploads
  ...
  select id, status, sha256, mime_type, byte_size
    ...
    from public.adbuilder_customer_image_uploads
  ...
  insert into public.adbuilder_customer_image_uploads (
...
drop function if exists public.adbuilder_finalize_customer_image_upload(uuid, uuid, text, text, text, bigint);
drop function if exists public.adbuilder_discard_customer_image_upload(uuid, uuid, text);

create or replace function public.adbuilder_claim_customer_image_finalize(
...
as $$
declare
  reservation public.adbuilder_customer_image_uploads%rowtype;
  claimed_id uuid;
begin
  if coalesce(auth.jwt() ->> 'role', '') <> 'service_role' then
    raise exception 'service role required' using errcode = '42501';
...
create or replace function public.adbuilder_finalize_customer_image_upload(
  p_reservation_id uuid,
  p_workspace_id uuid,
  p_ad_id uuid,
  p_object_path text,
...
create or replace function public.adbuilder_discard_customer_image_upload(
  p_reservation_id uuid,
  p_workspace_id uuid,
  p_ad_id uuid,
  p_object_path text
...
drop function if exists public.adbuilder_customer_image_stale_path_is_safe(uuid, text);

create or replace function public.adbuilder_complete_customer_image_stale_cleanup(
  p_reservation_id uuid,
  p_object_path text
)
returns boolean
...
begin
  ...
  delete from public.adbuilder_customer_image_uploads
   where id = p_reservation_id
     and object_path = p_object_path
     and status = 'deleting';
  return found;
...
revoke all on function public.adbuilder_prepare_customer_image_upload(uuid, uuid, text, text, text, bigint) from public, anon, authenticated;
revoke all on function public.adbuilder_claim_customer_image_finalize(uuid, uuid, uuid, text, text, text, bigint) from public, anon, authenticated;
revoke all on function public.adbuilder_finalize_customer_image_upload(uuid, uuid, uuid, text, text, text, bigint) from public, anon, authenticated;
revoke all on function public.adbuilder_discard_customer_image_upload(uuid, uuid, uuid, text) from public, anon, authenticated;
grant execute on function public.adbuilder_prepare_customer_image_upload(uuid, uuid, text, text, text, bigint) to service_role;
grant execute on function public.adbuilder_claim_customer_image_finalize(uuid, uuid, uuid, text, text, text, bigint) to service_role;
grant execute on function public.adbuilder_finalize_customer_image_upload(uuid, uuid, uuid, text, text, text, bigint) to service_role;
grant execute on function public.adbuilder_discard_customer_image_upload(uuid, uuid, uuid, text) to service_role;
revoke all on function public.adbuilder_complete_customer_image_stale_cleanup(uuid, text) from public, anon, authenticated;
grant execute on function public.adbuilder_complete_customer_image_stale_cleanup(uuid, text) to service_role;
