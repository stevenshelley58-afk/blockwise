...
alter table public.adbuilder_clone_candidate_audits
  ...
  add constraint adbuilder_clone_candidate_audits_qa_status_check
    check (qa_status in ('pending', 'passed', 'rejected', 'technical_failed', 'aborted')),
  add constraint adbuilder_clone_candidate_audits_accepted_matches_qa_status_check
    check (accepted = (qa_status = 'passed')),
  add constraint adbuilder_clone_candidate_audits_qa_error_length_check
    check (qa_error is null or char_length(qa_error) <= 1000),
  add constraint adbuilder_clone_candidate_audits_qa_state_shape_check
    check (
