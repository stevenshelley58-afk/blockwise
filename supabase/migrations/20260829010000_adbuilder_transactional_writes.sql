-- Ad Builder transactional writes and tenant isolation.
...
begin
  foreach relation_name in array array[
    ...
    execute format('drop policy if exists adbuilder_customer_select on public.%I', relation_name);
    execute format(
      'create policy adbuilder_customer_select on public.%I for select to authenticated using (private.adbuilder_has_workspace_access(workspace_id))',
      relation_name
    );
  end loop;
end;
...
drop policy if exists adbuilder_customer_insert on public.ad_customer_ads;
create policy adbuilder_customer_insert
  on public.ad_customer_ads for insert to authenticated
  with check (private.adbuilder_has_workspace_access(workspace_id));

drop policy if exists adbuilder_customer_update on public.ad_customer_ads;
create policy adbuilder_customer_update
  on public.ad_customer_ads for update to authenticated
  using (private.adbuilder_has_workspace_access(workspace_id))
  with check (private.adbuilder_has_workspace_access(workspace_id));

drop policy if exists adbuilder_form_insert on public.ad_instant_form_drafts;
create policy adbuilder_form_insert
  on public.ad_instant_form_drafts for insert to authenticated
  with check (private.adbuilder_has_workspace_access(workspace_id));

drop policy if exists adbuilder_form_update on public.ad_instant_form_drafts;
create policy adbuilder_form_update
  on public.ad_instant_form_drafts for update to authenticated
  using (private.adbuilder_has_workspace_access(workspace_id))
  with check (private.adbuilder_has_workspace_access(workspace_id));

drop policy if exists adbuilder_snapshot_insert on public.ad_publication_snapshots;
create policy adbuilder_snapshot_insert
...
begin
  if coalesce((select auth.role()), '') <> 'service_role'
     and not private.adbuilder_has_workspace_access(p_workspace_id) then
