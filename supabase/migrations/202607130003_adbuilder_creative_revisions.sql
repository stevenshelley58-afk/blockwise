...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.adbuilder_creatives'::regclass
      and conname = 'adbuilder_creatives_workspace_id_id_unique'
  ) then
    alter table public.adbuilder_creatives
      add constraint adbuilder_creatives_workspace_id_id_unique unique (workspace_id, id);
  end if;
end
$$;
...
create table public.adbuilder_creative_revisions (
  ...
  constraint adbuilder_creative_revisions_workspace_id_id_unique
    unique (workspace_id, id),
  constraint adbuilder_creative_revisions_workspace_creative_id_id_unique
    unique (workspace_id, creative_id, id),
  constraint adbuilder_creative_revisions_number_unique
    unique (workspace_id, creative_id, revision_number),
  constraint adbuilder_creative_revisions_mutation_unique
    unique (workspace_id, creative_id, mutation_id),
  constraint adbuilder_creative_revisions_creative_fk
    foreign key (workspace_id, creative_id)
    references public.adbuilder_creatives (workspace_id, id)
    on delete cascade,
  constraint adbuilder_creative_revisions_parent_fk
    foreign key (workspace_id, creative_id, parent_revision_id)
    references public.adbuilder_creative_revisions (workspace_id, creative_id, id)
    deferrable initially deferred
);

create table public.adbuilder_creative_revision_mutations (
  id uuid primary key,
  workspace_id uuid not null,
  creative_id uuid not null,
  ...
  constraint adbuilder_revision_mutations_workspace_creative_id_key
    unique (workspace_id, creative_id, id),
  constraint adbuilder_creative_revision_mutations_creative_fk
    foreign key (workspace_id, creative_id)
    references public.adbuilder_creatives (workspace_id, id),
  constraint adbuilder_creative_revision_mutations_base_fk
    foreign key (workspace_id, creative_id, base_revision_id)
    references public.adbuilder_creative_revisions (workspace_id, creative_id, id),
  constraint adbuilder_creative_revision_mutations_result_fk
    foreign key (workspace_id, creative_id, result_revision_id)
    references public.adbuilder_creative_revisions (workspace_id, creative_id, id)
);

create index adbuilder_creative_revisions_creative_created_idx
  on public.adbuilder_creative_revisions (workspace_id, creative_id, created_at desc);

lock table public.adbuilder_creatives in share row exclusive mode;

insert into public.adbuilder_creative_revisions (
  workspace_id,
  creative_id,
  revision_number,
  canvas_json,
...
select
  ...
  gen_random_uuid(),
  c.created_at
from public.adbuilder_creatives c;

update public.adbuilder_creatives c
set active_revision_id = r.id
from public.adbuilder_creative_revisions r
...
begin
  ...
  from public.adbuilder_creatives;

  select count(*) into revision_count
  from public.adbuilder_creative_revisions
  ...
  if unresolved <> 0 or revision_count <> creative_count then
    raise exception
      'AdBuilder revision backfill mismatch: creatives=%, revisions=%, unresolved=%',
      creative_count,
      revision_count,
      unresolved;
  end if;
  ...
  raise notice
    'AdBuilder revision backfill verified: creatives=%, revisions=%, unresolved=%',
    creative_count,
    revision_count,
    unresolved;
end
...
alter table public.adbuilder_creatives
  ...
  add constraint adbuilder_creatives_active_revision_fk
    foreign key (workspace_id, id, active_revision_id)
    references public.adbuilder_creative_revisions (workspace_id, creative_id, id)
    deferrable initially deferred;

alter table public.adbuilder_creatives
  add constraint adbuilder_creatives_pending_revision_mutation_fk
    foreign key (workspace_id, id, pending_revision_mutation_id)
    references public.adbuilder_creative_revision_mutations (workspace_id, creative_id, id)
    deferrable initially deferred;

create or replace function private.adbuilder_prepare_initial_creative_revision()
returns trigger
language plpgsql
security definer
set search_path = ''
...
create or replace function private.adbuilder_create_initial_creative_revision()
returns trigger
language plpgsql
security definer
set search_path = ''
...
create trigger adbuilder_creatives_prepare_initial_revision
before insert on public.adbuilder_creatives
for each row execute function private.adbuilder_prepare_initial_creative_revision();

create trigger adbuilder_creatives_create_initial_revision
after insert on public.adbuilder_creatives
for each row execute function private.adbuilder_create_initial_creative_revision();

create or replace function private.adbuilder_guard_creative_version_update()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if new.pending_revision_mutation_id is distinct from old.pending_revision_mutation_id then
    if old.pending_revision_mutation_id is not null then
      ...
      from public.adbuilder_creative_revision_mutations m
    ...
    raise exception using
      errcode = '55P03',
      message = 'ADBUILDER_EDIT_IN_PROGRESS';
  end if;
...
create trigger adbuilder_guard_creative_version_update
before update on public.adbuilder_creatives
for each row execute function private.adbuilder_guard_creative_version_update();

create or replace function private.adbuilder_preserve_creative_revision_history()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
begin
  if exists (
    select 1 from public.adbuilder_creative_revisions r
...
create trigger adbuilder_preserve_creative_revision_history
before delete on public.adbuilder_creatives
for each row execute function private.adbuilder_preserve_creative_revision_history();
...
alter table public.adbuilder_creative_revisions enable row level security;
alter table public.adbuilder_creative_revision_mutations enable row level security;

create policy adbuilder_creative_revisions_select
on public.adbuilder_creative_revisions
for select
to authenticated
using ((select private.adbuilder_has_workspace_access(workspace_id)));

create policy adbuilder_creative_revision_mutations_select
on public.adbuilder_creative_revision_mutations
for select
to authenticated
using ((select private.adbuilder_has_workspace_access(workspace_id)));

revoke insert, update, delete on public.adbuilder_creative_revisions
  from public, anon, authenticated, service_role;
grant select on public.adbuilder_creative_revisions to authenticated;
grant select on public.adbuilder_creative_revisions to service_role;

revoke insert, update, delete on public.adbuilder_creative_revision_mutations
  from public, anon, authenticated, service_role;
grant select on public.adbuilder_creative_revision_mutations to authenticated, service_role;

create or replace function public.adbuilder_claim_creative_revision_mutation(
  p_workspace_id uuid,
  p_creative_id uuid,
...
begin
  ...
  if found then
    ...
    if mutation_row.request_hash is distinct from p_request_hash then
      raise exception using errcode = '22023', message = 'ADBUILDER_MUTATION_CONTENT_MISMATCH';
    end if;
    ...
    if mutation_row.status = 'claimed' and mutation_row.claim_expires_at > now() then
      raise exception using errcode = '55P03', message = 'ADBUILDER_EDIT_IN_PROGRESS';
    end if;
  end if;
  ...
  if creative_row.pending_revision_mutation_id is not null
    ...
    update public.adbuilder_creative_revision_mutations
  ...
  if creative_row.active_revision_id is distinct from p_expected_active_revision_id then
    raise exception using errcode = '40001', message = 'ADBUILDER_STALE_REVISION';
  end if;

  insert into public.adbuilder_creative_revision_mutations (
  ...
  update public.adbuilder_creatives
...
create or replace function public.adbuilder_release_creative_revision_mutation(
  p_workspace_id uuid,
  p_creative_id uuid,
  p_mutation_id uuid
) returns void
...
create or replace function public.adbuilder_append_creative_revision(
  p_workspace_id uuid,
  p_creative_id uuid,
  p_expected_active_revision_id uuid,
  p_canvas_json jsonb,
...
begin
  ...
  if current_active_revision_id is distinct from p_expected_active_revision_id then
    raise exception using
      errcode = '40001',
      message = 'ADBUILDER_STALE_REVISION';
  end if;
...
revoke all on function public.adbuilder_claim_creative_revision_mutation(uuid, uuid, uuid, uuid, text)
  from public, anon;
grant execute on function public.adbuilder_claim_creative_revision_mutation(uuid, uuid, uuid, uuid, text)
  to authenticated, service_role;
revoke all on function public.adbuilder_release_creative_revision_mutation(uuid, uuid, uuid)
  from public, anon;
grant execute on function public.adbuilder_release_creative_revision_mutation(uuid, uuid, uuid)
  to authenticated, service_role;

revoke all on function public.adbuilder_append_creative_revision(
  uuid, uuid, uuid, jsonb, text, text, uuid, text
) from public, anon;
grant execute on function public.adbuilder_append_creative_revision(
  uuid, uuid, uuid, jsonb, text, text, uuid, text
) to authenticated, service_role;

comment on table public.adbuilder_creative_revisions is
  'Append-only creative snapshots. Targeted edits advance adbuilder_creatives.active_revision_id through a compare-and-swap RPC.';
