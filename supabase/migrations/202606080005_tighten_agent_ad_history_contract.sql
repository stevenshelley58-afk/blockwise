-- Customer-safe authenticated ad history. This keeps the broader
-- v_agent_ad_history view available for operator diagnostics while making the
-- customer/API research paths use the same gate as public ad-radar cards:
-- verified real-estate pages, displayable creatives, valid Meta library IDs,
-- and at least one explicit area attribution.
...
where ap.status in ('resolved_collectable', 'no_ads_confirmed')
  ...
  and case
    when jsonb_typeof(oa.raw_payload -> 'targeted_or_reached_countries') = 'array' then exists (
      select 1
  ...
  and case
    when jsonb_typeof(oa.raw_payload -> 'ad_reached_countries') = 'array' then exists (
      select 1
