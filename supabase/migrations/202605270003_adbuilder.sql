-- AdBuilder module: brand kits, campaign generation, editable creatives, compliance, and export state.

create extension if not exists pgcrypto;

create table if not exists public.adbuilder_brand_kits (
...
create table if not exists public.adbuilder_brand_assets (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces (id) on delete cascade,
  brand_kit_id uuid not null references public.adbuilder_brand_kits (id) on delete cascade,
...
create table if not exists public.adbuilder_offer_templates (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces (id) on delete cascade,
...
create table if not exists public.adbuilder_campaigns (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces (id) on delete cascade,
  brand_kit_id uuid not null references public.adbuilder_brand_kits (id) on delete restrict,
...
create table if not exists public.adbuilder_campaign_variants (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces (id) on delete cascade,
  campaign_id uuid not null references public.adbuilder_campaigns (id) on delete cascade,
  angle text not null,
...
create table if not exists public.adbuilder_creatives (
  ...
  workspace_id uuid not null references public.workspaces (id) on delete cascade,
  campaign_id uuid not null references public.adbuilder_campaigns (id) on delete cascade,
  variant_id uuid not null references public.adbuilder_campaign_variants (id) on delete cascade,
...
create table if not exists public.adbuilder_creative_objects (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces (id) on delete cascade,
  creative_id uuid not null references public.adbuilder_creatives (id) on delete cascade,
  object_type text not null,
  role text not null,
  content text,
  asset_id uuid references public.adbuilder_brand_assets (id) on delete set null,
...
create table if not exists public.adbuilder_platform_copy (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces (id) on delete cascade,
  campaign_id uuid not null references public.adbuilder_campaigns (id) on delete cascade,
  variant_id uuid references public.adbuilder_campaign_variants (id) on delete cascade,
...
create table if not exists public.adbuilder_exports (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces (id) on delete cascade,
  campaign_id uuid not null references public.adbuilder_campaigns (id) on delete cascade,
...
create table if not exists public.adbuilder_compliance_reports (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces (id) on delete cascade,
  campaign_id uuid references public.adbuilder_campaigns (id) on delete cascade,
  variant_id uuid references public.adbuilder_campaign_variants (id) on delete cascade,
...
create table if not exists public.adbuilder_provider_runs (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces (id) on delete cascade,
  job_id uuid,
...
create table if not exists public.adbuilder_job_runs (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces (id) on delete cascade,
...
create table if not exists public.adbuilder_performance_imports (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces (id) on delete cascade,
  campaign_id uuid references public.adbuilder_campaigns (id) on delete cascade,
...
alter table public.adbuilder_brand_kits enable row level security;
alter table public.adbuilder_brand_assets enable row level security;
alter table public.adbuilder_offer_templates enable row level security;
alter table public.adbuilder_campaigns enable row level security;
alter table public.adbuilder_campaign_variants enable row level security;
alter table public.adbuilder_creatives enable row level security;
alter table public.adbuilder_creative_objects enable row level security;
alter table public.adbuilder_platform_copy enable row level security;
alter table public.adbuilder_exports enable row level security;
alter table public.adbuilder_compliance_reports enable row level security;
alter table public.adbuilder_provider_runs enable row level security;
alter table public.adbuilder_job_runs enable row level security;
alter table public.adbuilder_performance_imports enable row level security;
