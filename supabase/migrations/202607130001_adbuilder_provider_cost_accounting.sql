...
alter table public.adbuilder_provider_runs
...
update public.adbuilder_provider_runs
...
with provider_rollup as (
  ...
  from public.adbuilder_provider_runs
...
begin
  ...
  from (select billing_status, count(*) row_count from public.adbuilder_provider_runs group by billing_status) counts;
  ...
  raise notice 'AdBuilder cost backfill classifications: provider_runs=%, ai_runs=%, ledger=%',
...
alter table public.adbuilder_provider_runs
  add constraint adbuilder_provider_runs_workspace_id_id_unique unique (workspace_id, id),
  add constraint adbuilder_provider_runs_workspace_mutation_unique unique (workspace_id, mutation_id);
...
begin
  ...
  if exists (
    select 1 from public.adbuilder_provider_runs provider_run
...
alter table public.adbuilder_provider_runs
  add constraint adbuilder_provider_runs_workspace_ai_run_fk
    foreign key (workspace_id, ai_run_id) references public.ai_runs (workspace_id, id) on delete set null (ai_run_id),
  add constraint adbuilder_provider_runs_workspace_ledger_fk
...
create table public.adbuilder_provider_run_attempts (
  ...
  constraint adbuilder_attempts_workspace_run_fk foreign key (workspace_id, provider_run_id)
    references public.adbuilder_provider_runs (workspace_id, id) on delete cascade
);
...
create table public.adbuilder_provider_attempt_outbox (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces (id) on delete cascade,
  ...
  constraint adbuilder_outbox_workspace_run_fk foreign key (workspace_id, provider_run_id)
    references public.adbuilder_provider_runs (workspace_id, id) on delete set null (provider_run_id)
);

create index adbuilder_provider_run_attempts_workspace_created_idx
  on public.adbuilder_provider_run_attempts (workspace_id, created_at desc);
create index adbuilder_provider_run_attempts_provider_request_idx
  on public.adbuilder_provider_run_attempts (provider_name, provider_request_id)
  where provider_request_id is not null;
create index adbuilder_provider_attempt_outbox_reconciliation_idx
  on public.adbuilder_provider_attempt_outbox (workspace_id, status, reserved_at)
  where status in ('reserved', 'submitted');

alter table public.adbuilder_provider_run_attempts enable row level security;
alter table public.adbuilder_provider_attempt_outbox enable row level security;

create policy adbuilder_provider_run_attempts_workspace_select
  on public.adbuilder_provider_run_attempts for select to authenticated
  using (private.adbuilder_has_workspace_access(workspace_id));
create policy adbuilder_provider_run_attempts_no_client_insert
  on public.adbuilder_provider_run_attempts for insert to authenticated with check (false);
create policy adbuilder_provider_run_attempts_no_client_update
  on public.adbuilder_provider_run_attempts for update to authenticated using (false) with check (false);
create policy adbuilder_provider_run_attempts_no_client_delete
  on public.adbuilder_provider_run_attempts for delete to authenticated using (false);

revoke all on public.adbuilder_provider_run_attempts from anon, authenticated;
revoke all on public.adbuilder_provider_attempt_outbox from anon, authenticated;
grant select on public.adbuilder_provider_run_attempts to authenticated;
grant all on public.adbuilder_provider_run_attempts to service_role;
grant all on public.adbuilder_provider_attempt_outbox to service_role;

create or replace function public.adbuilder_reserve_provider_attempt(
  p_workspace_id uuid,
  p_mutation_id text,
...
as $function$
declare
  reservation_row public.adbuilder_provider_attempt_outbox%rowtype;
begin
  ...
  insert into public.adbuilder_provider_attempt_outbox (
  ...
  from public.adbuilder_provider_attempt_outbox
...
create or replace function public.adbuilder_mark_provider_attempt_submitted(
  p_workspace_id uuid,
  p_mutation_id text,
  p_attempt_index integer
)
...
as $function$
begin
  update public.adbuilder_provider_attempt_outbox
...
create or replace function public.adbuilder_cancel_provider_attempt(
  p_workspace_id uuid,
  p_mutation_id text,
  p_attempt_index integer,
  p_reason text
...
create or replace function public.adbuilder_record_provider_run(
  p_workspace_id uuid,
  p_mutation_id text,
  p_payload_hash text,
  p_run jsonb,
...
as $function$
declare
  existing_run public.adbuilder_provider_runs%rowtype;
  v_ai_run_id uuid;
  v_ledger_id uuid;
...
begin
  ...
  v_attempts := p_attempts || coalesce((
    ...
    from public.adbuilder_provider_attempt_outbox outbox
  ...
  insert into public.adbuilder_provider_runs (
  ...
  loop
    insert into public.adbuilder_provider_run_attempts (
  ...
  update public.adbuilder_provider_attempt_outbox outbox
  set status = 'closed',
...
create or replace function public.adbuilder_recover_provider_run(
  p_workspace_id uuid,
  p_mutation_id text,
  p_payload_hash text,
  p_run jsonb,
...
begin
  ...
  if not exists (
    select 1 from public.adbuilder_provider_attempt_outbox
    where workspace_id = p_workspace_id
      and mutation_id = p_mutation_id
  ...
  return public.adbuilder_record_provider_run(
    p_workspace_id, p_mutation_id, p_payload_hash, recovered_run, '[]'::jsonb
  );
end;
$function$;

revoke all on function public.adbuilder_reserve_provider_attempt(uuid, text, integer, text, jsonb) from public, anon, authenticated;
revoke all on function public.adbuilder_mark_provider_attempt_submitted(uuid, text, integer) from public, anon, authenticated;
revoke all on function public.adbuilder_cancel_provider_attempt(uuid, text, integer, text) from public, anon, authenticated;
revoke all on function public.adbuilder_record_provider_run(uuid, text, text, jsonb, jsonb) from public, anon, authenticated;
revoke all on function public.adbuilder_recover_provider_run(uuid, text, text, jsonb, timestamptz) from public, anon, authenticated;
grant execute on function public.adbuilder_reserve_provider_attempt(uuid, text, integer, text, jsonb) to service_role;
grant execute on function public.adbuilder_mark_provider_attempt_submitted(uuid, text, integer) to service_role;
grant execute on function public.adbuilder_cancel_provider_attempt(uuid, text, integer, text) to service_role;
grant execute on function public.adbuilder_record_provider_run(uuid, text, text, jsonb, jsonb) to service_role;
grant execute on function public.adbuilder_recover_provider_run(uuid, text, text, jsonb, timestamptz) to service_role;
