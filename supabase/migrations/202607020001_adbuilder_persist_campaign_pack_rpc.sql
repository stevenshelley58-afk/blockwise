...
--
-- persistAdBuilderCampaignPack previously issued six sequential upserts from the
...
-- on every adbuilder_* table still apply unchanged.

create or replace function public.adbuilder_persist_campaign_pack(
  brand_kit jsonb,
  campaign jsonb,
  variants jsonb,
  creatives jsonb,
...
begin
  ...
  from jsonb_populate_record(null::public.adbuilder_brand_kits, brand_kit) as r
  ...
  from jsonb_populate_record(null::public.adbuilder_campaigns, campaign) as r
  on conflict (id) do update set
    workspace_id = excluded.workspace_id,
  ...
  from jsonb_populate_recordset(null::public.adbuilder_campaign_variants, variants) as r
  on conflict (id) do update set
    workspace_id = excluded.workspace_id,
  ...
  from jsonb_populate_recordset(null::public.adbuilder_creatives, creatives) as r
  on conflict (id) do update set
    workspace_id = excluded.workspace_id,
    campaign_id = excluded.campaign_id,
  ...
  insert into public.adbuilder_platform_copy (
  ...
  from jsonb_populate_recordset(null::public.adbuilder_platform_copy, copy_packs) as r
  on conflict (id) do update set
    workspace_id = excluded.workspace_id,
    campaign_id = excluded.campaign_id,
    variant_id = excluded.variant_id,
  ...
  insert into public.adbuilder_compliance_reports (
  ...
  from jsonb_populate_record(null::public.adbuilder_compliance_reports, compliance) as r
  on conflict (id) do update set
    workspace_id = excluded.workspace_id,
    campaign_id = excluded.campaign_id,
    status = excluded.status,
...
grant execute on function public.adbuilder_persist_campaign_pack(jsonb, jsonb, jsonb, jsonb, jsonb, jsonb) to authenticated;
grant execute on function public.adbuilder_persist_campaign_pack(jsonb, jsonb, jsonb, jsonb, jsonb, jsonb) to service_role;
