...
-- (scripts/adbuilder/apply-template-review-overrides.mjs) folds rows back into
-- git as the canonical source.
begin;

create table if not exists public.adbuilder_template_review_overrides (
...
alter table public.adbuilder_template_review_overrides enable row level security;

revoke all on table public.adbuilder_template_review_overrides from public, anon, authenticated;
