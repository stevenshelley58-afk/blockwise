...
begin
 ...
 if p_granted and not exists(select 1 from auth.users u where u.id=auth.uid() and coalesce(u.email_confirmed_at,u.confirmed_at) is not null) then raise exception 'verified email required' using errcode='42501'; end if;
