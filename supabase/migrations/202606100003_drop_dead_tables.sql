...
alter table if exists public.adbuilder_campaigns drop column if exists offer_template_id;
...
alter table if exists public.adbuilder_creative_objects set schema legacy_archive;
alter table if exists public.adbuilder_exports set schema legacy_archive;
alter table if exists public.adbuilder_offer_templates set schema legacy_archive;
...
-- Status columns filtered by Hermes matching and AdBuilder list queries.
...
create index if not exists adbuilder_campaigns_status_idx on public.adbuilder_campaigns (status);
