...
values ('library-status-active', '{"templateId":"library-status-active"}'::jsonb);
...
select is(
  (select library_status from public.ad_templates where template_id = 'library-status-active'),
  'active',
  'new direct templates default to active'
);
...
select is(
  (select count(*)::integer from public.ad_templates where template_id like 'library-status-%'),
  2,
  'customer omission preserves both template rows'
);
select is(
  (select count(*)::integer from public.ad_template_assets_direct where template_id like 'library-status-%'),
  2,
