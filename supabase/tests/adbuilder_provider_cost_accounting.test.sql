...
select has_table('public', 'adbuilder_provider_run_attempts', 'normalized attempt table exists');
select has_table('public', 'adbuilder_provider_attempt_outbox', 'pre-dispatch outbox exists');
select ok(
  not has_table_privilege('authenticated', 'public.adbuilder_provider_attempt_outbox', 'SELECT'),
  'authenticated users cannot read the service-only outbox'
);
select ok(
  has_table_privilege('authenticated', 'public.adbuilder_provider_run_attempts', 'SELECT'),
  'authenticated users can read normalized attempts through RLS'
);
select ok(
  not has_function_privilege(
    'authenticated',
    'public.adbuilder_reserve_provider_attempt(uuid,text,integer,text,jsonb)',
    'EXECUTE'
  ),
...
select ok(
  not has_function_privilege(
    'authenticated',
    'public.adbuilder_record_provider_run(uuid,text,text,jsonb,jsonb)',
    'EXECUTE'
  ),
...
select ok(
  not has_function_privilege(
    'authenticated',
    'public.adbuilder_mark_provider_attempt_submitted(uuid,text,integer)',
    'EXECUTE'
  ),
...
select ok(
  not has_function_privilege(
    'authenticated',
    'public.adbuilder_cancel_provider_attempt(uuid,text,integer,text)',
    'EXECUTE'
  ),
...
select ok(
  not has_function_privilege(
    'authenticated',
    'public.adbuilder_recover_provider_run(uuid,text,text,jsonb,timestamp with time zone)',
    'EXECUTE'
  ),
  'recovery RPC is service-role-only'
);
select ok(
  (select relrowsecurity from pg_class where oid = 'public.adbuilder_provider_run_attempts'::regclass),
  'attempt RLS is enabled'
);
select ok(
  (select relrowsecurity from pg_class where oid = 'public.adbuilder_provider_attempt_outbox'::regclass),
...
select ok(
  not exists (
    ...
    where table_schema = 'public'
      and table_name in ('ai_runs', 'ai_usage_ledger', 'adbuilder_provider_runs')
...
select ok(
  exists (
    ...
    where conrelid = 'public.adbuilder_provider_run_attempts'::regclass
      and conname = 'adbuilder_attempts_workspace_run_fk'
      and contype = 'f'
  ),
...
select ok(
  exists (
    select 1 from pg_constraint
    where conrelid = 'public.adbuilder_provider_attempt_outbox'::regclass
      and conname = 'adbuilder_outbox_workspace_run_fk'
      and contype = 'f'
  ),
...
select is(
  (public.adbuilder_reserve_provider_attempt(
...
select is(
  public.adbuilder_mark_provider_attempt_submitted(
...
select public.adbuilder_mark_provider_attempt_submitted(
...
returns jsonb language sql security definer set search_path = '' as $$
  select public.adbuilder_record_provider_run(
...
select is(
  (select count(*)::integer from public.adbuilder_provider_runs
...
select is(
  (select count(*)::integer from public.adbuilder_provider_run_attempts attempts
   join public.adbuilder_provider_runs runs on runs.id = attempts.provider_run_id
...
select is(
  (select status from public.adbuilder_provider_attempt_outbox
...
select is(
  (select estimated_cost_usd from public.adbuilder_provider_runs
...
select throws_ok(
  $$select public.adbuilder_recover_provider_run(
    'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
    'recover-submitted',
    'recover-run-hash',
...
select is(
  (select attempts.billing_status from public.adbuilder_provider_run_attempts attempts
   join public.adbuilder_provider_runs runs on runs.id = attempts.provider_run_id
...
select is(
  (select request_submitted from public.adbuilder_provider_run_attempts attempts
   join public.adbuilder_provider_runs runs on runs.id = attempts.provider_run_id
   where runs.workspace_id = 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa'
     and runs.mutation_id = 'recover-submitted'),
  true,
...
select is(
  (select count(*)::integer from public.adbuilder_provider_run_attempts
   where workspace_id = 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa'),
  0,
