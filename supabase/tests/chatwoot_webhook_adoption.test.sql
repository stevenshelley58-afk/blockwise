...
select is((select count(*)::int from private.ops_enquiry_messages where provider_message_id=encode(public.digest('202','sha256'),'hex')),1,'duplicate message does not duplicate storage');
select is(jsonb_array_length((select messages from jsonb_array_elements(public.resolve_ops_enquiry_threads()) x where (x->>'enquiry_id')=(select id::text from public.ops_enquiry_associations where source_id like 'conversation:%' and workspace_id is not null limit 1))),1,'thread resolver returns bounded message timeline');
select is((select count(*)::int from public.ops_enquiry_associations where source_system='chatwoot' and source_id='conversation:'||encode(public.digest('101','sha256'),'hex')),1,'concurrent-safe source binding is unique');
