...
async function executeChatwootAction(supabase: Supabase, action: Action, fetchImpl: typeof fetch): Promise<Record<string, unknown>> {
  ...
  if (state === "settled") {
    ...
    return safe && typeof safe === "object" ? safe as Record<string, unknown> : { provider: "chatwoot", status: "settled", replay: true };
  }
...
async function call(base: URL, path: string, method: string, token: string, key: string, body: unknown, fetchImpl: typeof fetch): Promise<{ body: Record<string, any> }> { const controller=new AbortController(); let timedOut=false; const timer=setTimeout(()=>{timedOut=true;controller.abort();},15_000); try { const response = await fetchImpl(new URL(path, base), { method, redirect: "error", headers: { api_access_token: token, "content-type": "application/json", "idempotency-key": key, "x-blockwise-correlation": key }, ...(body === undefined ? {} : { body: JSON.stringify(body) }), signal: controller.signal }); if (response.status===429 || response.status>=500) throw new ProviderActionError(`chatwoot temporary failure (${response.status})`,true); if (!response.ok) throw new ProviderActionError(`chatwoot provider rejected action (${response.status})`,false); const parsed = await response.json(); if (!parsed || typeof parsed !== "object") throw new ProviderActionError("chatwoot response was invalid",true); return { body: parsed as Record<string, any> }; } catch (error) { if (error instanceof ProviderActionError) throw error; throw new ProviderActionError(timedOut ? "chatwoot request timed out" : "chatwoot transport failure",true); } finally { clearTimeout(timer); } }
...
function secretFile(name: string): string { const path = env(name); if (!isAbsolute(path)) throw new Error(`${name} must be absolute`); const resolved=resolve(path); const file = lstatSync(resolved); if (!file.isFile() || file.isSymbolicLink()) throw new Error(`${name} is not a regular file`); if (process.platform !== "win32") { if ((file.mode & 0o777) !== 0o600 || (typeof process.getuid === "function" && file.uid !== process.getuid())) throw new Error(`${name} has unsafe ownership or permissions`); let part=dirname(resolved); while (part && part!==dirname(part)) { const stat=lstatSync(part); if (stat.isSymbolicLink() || (stat.mode & 0o022)) throw new Error(`${name} has unsafe path component`); part=dirname(part); } } const value=readFileSync(resolved, "utf8").trim(); if (!value) throw new Error(`${name} is empty`); return value; }
