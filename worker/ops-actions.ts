...
export async function runOpsActionOnce(supabase: Supabase, fetchImpl: typeof fetch = fetch): Promise<boolean> {
  ...
  try {
    ...
    await Promise.resolve(supabase.rpc("fail_ops_action", { p_id: action.id, p_lease_token: action.lease_token, p_error: redact(error instanceof Error ? error.message : String(error)), p_retryable: true }));
  }
  return true;
}
...
async function executeChatwootAction(supabase: Supabase, action: Action, fetchImpl: typeof fetch): Promise<Record<string, unknown>> {
  ...
  const lookup = await supabase.schema("private").from("ops_provider_operation_ledger").select("provider_conversation_id_ciphertext,provider_conversation_id_digest,source_version").eq("workspace_id", action.workspace_id).eq("provider", "chatwoot").eq("aggregate_type", "enquiry").eq("aggregate_id", action.target_id).order("source_version", { ascending: false }).limit(1).maybeSingle();
  if (lookup.error || !lookup.data) throw new Error("chatwoot conversation identity unavailable");
  const conversationId = decryptId(lookup.data.provider_conversation_id_ciphertext);
  if (!conversationId || lookup.data.provider_conversation_id_digest !== digest(conversationId)) throw new Error("chatwoot conversation identity invalid");
  ...
  if (begun.error) throw new Error("chatwoot action ledger unavailable");
  ...
  await step(supabase, operationKey, "conversation.reconciled", conversationId);
  ...
  if (action.action_type === "enquiry_reply") {
    ...
    if (!messageId) { const sent = await call(base, `/api/v1/accounts/${encodeURIComponent(account)}/conversations/${encodeURIComponent(conversationId)}/messages`, "POST", token, operationKey + ":message", { content: body, message_type: "outgoing", private: false, content_attributes: { blockwise_action_id: action.action_id } }, fetchImpl); messageId = safeId(sent.body?.id ?? sent.body?.payload?.id); if (!messageId) throw new Error("chatwoot reply receipt missing"); }
    await step(supabase, operationKey, "message.reply", messageId);
  } else {
    status = action.action_type === "enquiry_close" ? "resolved" : "open";
    await call(base, `/api/v1/accounts/${encodeURIComponent(account)}/conversations/${encodeURIComponent(conversationId)}`, "PATCH", token, operationKey + ":status", { status }, fetchImpl);
...
async function call(base: URL, path: string, method: string, token: string, key: string, body: unknown, fetchImpl: typeof fetch): Promise<{ body: Record<string, any> }> { const response = await fetchImpl(new URL(path, base), { method, redirect: "error", headers: { api_access_token: token, "content-type": "application/json", "idempotency-key": key, "x-blockwise-correlation": key }, ...(body === undefined ? {} : { body: JSON.stringify(body) }) }); if (!response.ok) throw new Error(`chatwoot provider rejected action (${response.status})`); const parsed = await response.json().catch(() => ({})); return { body: parsed && typeof parsed === "object" ? parsed : {} }; }
