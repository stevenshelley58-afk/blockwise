...
export const PROJECTIONS = ["customers", "email", "flows", "mautic", "enquiries", "bookings", "billing", "activity", "members"] as const;
...
export function publishOpsBundle(root: string, input: OpsBundle): { receiptId: string; sha256: string; generation: string } {
  ...
  durable(join(staging, "publication-receipt.json"), JSON.stringify(receipt, Object.keys(receipt).sort()) + "\n"); fsyncDir(staging); renameSync(staging, join(generations, generation)); fsyncDir(generations);
  const pointer = { schema: "schema://frank.ops-pointer/v1", version: 1, generation, publication_receipt_id: publicationReceiptId }; const pointerTemp = join(output, `.current.${randomUUID()}.tmp`); durable(pointerTemp, JSON.stringify(pointer, Object.keys(pointer).sort()) + "\n"); renameSync(pointerTemp, join(output, "current.json")); fsyncDir(output);
