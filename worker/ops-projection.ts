...
type ProviderResult = { providerId?: string; contactId?: string; conversationId?: string; providerRecordSuffix?: string; status?: string; stage?: string; subject?: string; channel?: string; deliveryStatus?: string; occurredAt?: string; lastActivityAt?: string; safeData?: Record<string, unknown> };
type Ledger = { state?: string; provider_contact_id_ciphertext?: string | null; provider_conversation_id_ciphertext?: string | null };
type StepRecorder = (step: string, providerId?: string | null, resource?: "contact" | "conversation") => Promise<void>;
...
export async function runOpsProjectionOnce(supabase: Supabase, fetchImpl: Fetcher = fetch): Promise<boolean> {
  ...
  } finally { clearInterval(heartbeat); if (heartbeatInFlight) await awaitHeartbeat(heartbeatInFlight); }
  return true;
}
...
async function publishFrankBundleIfConfigured(supabase: Supabase): Promise<void> {
  ...
  if (!root) return;
  const revision = process.env.BLOCKWISE_WORKER_REVISION?.trim();
...
export async function runGlobalProjectionOnce(supabase: Supabase, fetchImpl: Fetcher = fetch): Promise<boolean> {
  ...
  try {
    ...
    if (ledger.state !== "settled") {
      const priorContactId = decryptProviderId(ledger.provider_contact_id_ciphertext);
      const priorConversationId = decryptProviderId(ledger.provider_conversation_id_ciphertext);
      const recordStep: StepRecorder = async (step, providerId = null, resource = undefined) => {
        const recorded = await Promise.resolve(supabase.rpc("record_ops_provider_step", { p_operation_key: operationKey, p_step: step, p_resource: resource ?? null, p_provider_id_ciphertext: resource && providerId ? encryptProviderId(providerId) : null, p_provider_id_digest: providerId ? digestProviderId(providerId) : null }));
        if (recorded.error || recorded.data !== true) throw new Error(`global provider operation step could not be recorded: ${step}`);
      };
      const delivered = await deliverGlobalChatwoot(data, operationKey, priorContactId, priorConversationId, fetchImpl, recordStep);
      for (const [resource, value] of [["contact", delivered.contactId], ["conversation", delivered.conversationId]] as const) {
        const recorded = await Promise.resolve(supabase.rpc("record_ops_provider_identifier", { p_operation_key: operationKey, p_resource: resource, p_provider_id_ciphertext: encryptProviderId(value), p_provider_id_digest: digestProviderId(value), p_status: "remote_succeeded" }));
        if (recorded.error || recorded.data !== true) throw new Error(`global provider ${resource} could not be recorded`);
      }
...
async function processProjection(supabase: Supabase, row: Row, fetchImpl: Fetcher): Promise<void> {
  ...
  const recordStep: StepRecorder = async (step, providerId = null, resource = undefined) => {
    const recorded = await Promise.resolve(supabase.rpc("record_ops_provider_step", { p_operation_key: operationKey, p_step: step, p_resource: resource ?? null, p_provider_id_ciphertext: resource && providerId ? encryptProviderId(providerId) : null, p_provider_id_digest: providerId ? digestProviderId(providerId) : null }));
    if (recorded.error || recorded.data !== true) throw new Error(`provider operation step could not be recorded: ${step}`);
  };
  const result = await providerCall(row.provider, mapping, row.aggregate_type, operationKey, priorContactId, priorConversationId, fetchImpl, recordStep);
  if (!result.providerId) throw new Error("provider returned no durable identifier");
  for (const [resource, value] of [["contact", result.contactId], ["conversation", result.conversationId]] as const) {
    if (!value) continue;
    const recorded = await Promise.resolve(supabase.rpc("record_ops_provider_identifier", { p_operation_key: operationKey, p_resource: resource, p_provider_id_ciphertext: encryptProviderId(value), p_provider_id_digest: digestProviderId(value), p_status: "remote_succeeded" }));
    if (recorded.error || recorded.data !== true) throw new Error("provider operation identifier could not be recorded");
  }
...
async function providerCall(provider: Row["provider"], mapping: AdapterMapping, aggregateType: Row["aggregate_type"], operationKey: string, priorContactId: string | null, priorConversationId: string | null, fetchImpl: Fetcher, recordStep: StepRecorder): Promise<ProviderResult> { return provider === "mautic" ? mauticCall(mapping as Extract<AdapterMapping, { provider: "mautic" }>, operationKey, priorContactId, fetchImpl, recordStep) : chatwootCall(mapping as Extract<AdapterMapping, { provider: "chatwoot" }>, aggregateType, operationKey, priorContactId, priorConversationId, fetchImpl, recordStep); }

async function mauticCall(mapping: Extract<AdapterMapping, { provider: "mautic" }>, operationKey: string, priorId: string | null, fetchImpl: Fetcher, recordStep: StepRecorder): Promise<ProviderResult> {
  ...
  if (mapping.resource === "lifecycle") {
    ...
    await recordStep("contact.reconciled", contact, "contact");
    ...
    await request(base, `/api/segments/${encodeURIComponent(segmentId)}/contact/${encodeURIComponent(contact)}/add`, "POST", token, operationKey + ":segment", {}, fetchImpl); await recordStep("lifecycle.segment", contact, "contact");
    await request(base, `/api/contacts/${encodeURIComponent(contact)}/edit`, "PATCH", token, operationKey + ":tag", { tags: [requiredEnv("MAUTIC_LIFECYCLE_TAG")] }, fetchImpl); await recordStep("lifecycle.tag", contact, "contact");
    if (campaignId) { await request(base, `/api/campaigns/${encodeURIComponent(campaignId)}/contact/${encodeURIComponent(contact)}/add`, "POST", token, operationKey + ":campaign", {}, fetchImpl); await recordStep("lifecycle.campaign", contact, "contact"); }
    return { providerId: contact, contactId: contact, providerRecordSuffix: maskedSuffix(contact), stage, status: "active", safeData: { flow: "lifecycle", segment_status: "configured", campaign_status: campaignId ? "configured" : "not_configured" } };
  }
  ...
  await recordStep("contact.upsert", id, "contact");
  return { providerId: id, contactId: id, providerRecordSuffix: maskedSuffix(id), status: "active", stage: mapping.fields.activationStage ?? mapping.fields.lifecycle, deliveryStatus: "accepted", safeData: { flow: "contact" } };
}

async function chatwootCall(mapping: Extract<AdapterMapping, { provider: "chatwoot" }>, aggregateType: Row["aggregate_type"], operationKey: string, priorContactId: string | null, priorConversationId: string | null, fetchImpl: Fetcher, recordStep: StepRecorder): Promise<ProviderResult> {
  const base = requiredHttpsEnv("CHATWOOT_BASE_URL"); const token = readSecretFile("CHATWOOT_API_TOKEN_FILE"); const account = requiredEnv("CHATWOOT_ACCOUNT_ID"); const inboxName = aggregateType === "enquiry" ? "CHATWOOT_ENQUIRY_INBOX_ID" : "CHATWOOT_SUPPORT_INBOX_ID"; const inbox = numericId(inboxName); const sourceId = requiredEnv(aggregateType === "enquiry" ? "CHATWOOT_ENQUIRY_SOURCE_ID" : "CHATWOOT_SUPPORT_SOURCE_ID");
  ...
  let contactId = mapping.fields.contactId ?? priorContactId ?? await chatwootFindContact(base, account, mapping.fields.externalId, mapping.fields.requesterEmail, token, fetchImpl);
  ...
  await recordStep("contact.upsert", contactId, "contact");
  const conversations = await request(base, `/api/v1/accounts/${encodeURIComponent(account)}/conversations?inbox_id=${encodeURIComponent(inbox)}&contact_id=${encodeURIComponent(contactId)}`, "GET", token, operationKey + ":conversation-lookup", undefined, fetchImpl); await recordStep("conversation.lookup", priorConversationId, "conversation");
  const existing = findConversation(conversations.body, mapping.fields.externalId, priorConversationId ?? undefined);
  const conversation = existing ?? (await request(base, `/api/v1/accounts/${encodeURIComponent(account)}/conversations`, "POST", token, operationKey + ":conversation", { inbox_id: inbox, contact_id: numericValue(contactId, "Chatwoot contact id"), source_id: sourceId, status: chatStatus(mapping.fields.status), custom_attributes: { blockwise_external_id: mapping.fields.externalId, blockwise_aggregate_type: mapping.resource } }, fetchImpl)).body;
  ...
  await recordStep(existing ? "conversation.reconciled" : "conversation.created", conversationId, "conversation");
  if (mapping.fields.message) {
    ...
    if (!hasMessage) { const created = await request(base, `/api/v1/accounts/${encodeURIComponent(account)}/conversations/${encodeURIComponent(conversationId)}/messages`, "POST", token, operationKey + ":message", { content: mapping.fields.message, message_type: "incoming", private: false, content_attributes: { blockwise_operation_key: operationKey } }, fetchImpl); await recordStep("message.inbound", stringId(created.body?.id)); }
  }
  if (mapping.fields.reply) {
    ...
    if (!hasReply) { const created = await request(base, `/api/v1/accounts/${encodeURIComponent(account)}/conversations/${encodeURIComponent(conversationId)}/messages`, "POST", token, operationKey + ":reply", { content: mapping.fields.reply, message_type: "outgoing", private: false, content_attributes: { blockwise_operation_key: operationKey } }, fetchImpl); await recordStep("message.reply", stringId(created.body?.id)); }
  }
  if (mapping.fields.status) { await request(base, `/api/v1/accounts/${encodeURIComponent(account)}/conversations/${encodeURIComponent(conversationId)}`, "PATCH", token, operationKey + ":status", { status: chatStatus(mapping.fields.status) }, fetchImpl); await recordStep("conversation.status", conversationId, "conversation"); }
  const assignee = mapping.fields.assigneeId || process.env.CHATWOOT_ASSIGNEE_ID?.trim(); if (assignee) { await request(base, `/api/v1/accounts/${encodeURIComponent(account)}/conversations/${encodeURIComponent(conversationId)}/assignments`, "POST", token, operationKey + ":assignment", { assignee_id: numericValue(assignee, "CHATWOOT_ASSIGNEE_ID") }, fetchImpl); await recordStep("conversation.assignment", conversationId, "conversation"); }
  return { providerId: conversationId, contactId, conversationId, providerRecordSuffix: maskedSuffix(conversationId), status: chatStatus(mapping.fields.status), subject: mapping.fields.subject, channel: "web", lastActivityAt: new Date().toISOString(), safeData: { conversationStatus: chatStatus(mapping.fields.status) } };
}

async function deliverGlobalChatwoot(data: Record<string, unknown>, operationKey: string, priorContactId: string | null, priorConversationId: string | null, fetchImpl: Fetcher, recordStep: StepRecorder): Promise<{ contactId: string; conversationId: string }> {
  const base = requiredHttpsEnv("CHATWOOT_BASE_URL"); const token = readSecretFile("CHATWOOT_API_TOKEN_FILE"); const account = requiredEnv("CHATWOOT_GLOBAL_ACCOUNT_ID"); const inbox = numericId("CHATWOOT_GLOBAL_INBOX_ID"); const sourceId = requiredEnv("CHATWOOT_GLOBAL_SOURCE_ID"); const leadId = stringId(data.id); if (!leadId) throw new Error("global enquiry lacks exact lead id");
  const externalId = `blockwise:global:${leadId}`; const email = string(data.requester_email); if (!email) throw new Error("global Chatwoot reconciliation requires the source email");
  const known = await chatwootFindContact(base, account, externalId, email, token, fetchImpl);
  const contact = known ? await request(base, `/api/v1/accounts/${encodeURIComponent(account)}/contacts/${encodeURIComponent(known)}`, "PUT", token, operationKey + ":contact", { inbox_id: inbox, identifier: externalId, email, name: string(data.requester_name) || "Website enquiry", custom_attributes: { blockwise_global_lead_id: leadId } }, fetchImpl) : await request(base, `/api/v1/accounts/${encodeURIComponent(account)}/contacts`, "POST", token, operationKey + ":contact", { inbox_id: inbox, identifier: externalId, email, name: string(data.requester_name) || "Website enquiry", custom_attributes: { blockwise_global_lead_id: leadId } }, fetchImpl);
  const contactId = stringId(contact.body?.payload?.id ?? contact.body?.id ?? known); if (!contactId) throw new Error("global Chatwoot contact response has no id"); await recordStep("contact.upsert", contactId, "contact");
  const conversations = await request(base, `/api/v1/accounts/${encodeURIComponent(account)}/conversations?inbox_id=${encodeURIComponent(inbox)}&contact_id=${encodeURIComponent(contactId)}`, "GET", token, operationKey + ":conversation-lookup", undefined, fetchImpl); await recordStep("conversation.lookup", priorConversationId, "conversation");
  const existing = findConversation(conversations.body, externalId, priorConversationId ?? undefined); const conversation = existing ?? (await request(base, `/api/v1/accounts/${encodeURIComponent(account)}/conversations`, "POST", token, operationKey + ":conversation", { inbox_id: inbox, contact_id: numericValue(contactId, "Chatwoot global contact id"), source_id: sourceId, status: "open", custom_attributes: { blockwise_global_lead_id: leadId, blockwise_external_id: externalId } }, fetchImpl)).body;
  const conversationId = stringId(conversation.body?.id ?? conversation.id ?? conversation.payload?.id); if (!conversationId) throw new Error("global Chatwoot conversation response has no id"); await recordStep(existing ? "conversation.reconciled" : "conversation.created", conversationId, "conversation");
  const content = string(data.message) || string(data.subject) || "Website enquiry"; const messages = await request(base, `/api/v1/accounts/${encodeURIComponent(account)}/conversations/${encodeURIComponent(conversationId)}/messages`, "GET", token, operationKey + ":message-lookup", undefined, fetchImpl); await recordStep("message.lookup", conversationId); const hasMessage = Array.isArray(messages.body?.payload) && messages.body.payload.some((item: any) => item?.content === content && item?.content_attributes?.blockwise_operation_key === operationKey); if (!hasMessage) { const created = await request(base, `/api/v1/accounts/${encodeURIComponent(account)}/conversations/${encodeURIComponent(conversationId)}/messages`, "POST", token, operationKey + ":message", { content, message_type: "incoming", private: false, content_attributes: { blockwise_operation_key: operationKey } }, fetchImpl); await recordStep("message.inbound", stringId(created.body?.id)); }
  return { contactId, conversationId };
}

async function mauticFindContact(base: URL, token: string, externalId: string, fetchImpl: Fetcher): Promise<string | null> { const result = await request(base, `/api/contacts?search=${encodeURIComponent(`blockwise_external_id:${externalId}`)}`, "GET", token, "lookup", undefined, fetchImpl); const contacts = result.body?.contacts; if (!contacts || typeof contacts !== "object") return null; const first = Object.values(contacts as Record<string, any>).find((item: any) => item?.fields?.core?.blockwise_external_id?.value === externalId || item?.fields?.blockwise_external_id?.value === externalId || item?.fields?.core?.blockwise_external_id === externalId) as Record<string, unknown> | undefined; return stringId(first?.id); }
async function chatwootFindContact(base: URL, account: string, externalId: string, email: string | undefined, token: string, fetchImpl: Fetcher): Promise<string | null> { if (!email) throw new Error("Chatwoot contact reconciliation requires the source email"); const result = await request(base, `/api/v1/accounts/${encodeURIComponent(account)}/contacts/search?email=${encodeURIComponent(email)}`, "GET", token, "lookup", undefined, fetchImpl); const payload = result.body?.payload; const matches = Array.isArray(payload) ? payload.filter((item: any) => item?.identifier === externalId || item?.custom_attributes?.blockwise_external_id === externalId) : []; if (matches.length > 1) throw new Error("Chatwoot has duplicate contacts for the Blockwise external id"); return stringId((matches[0] as Record<string, unknown> | undefined)?.id); }
...
async function awaitHeartbeat(promise: Promise<unknown>): Promise<void> { await Promise.race([promise.catch(() => undefined), new Promise<void>((resolve) => setTimeout(resolve, 2_000))]); }
