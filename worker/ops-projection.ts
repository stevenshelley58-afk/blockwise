...
export async function runGlobalProjectionOnce(supabase: Supabase, fetchImpl: Fetcher = fetch): Promise<boolean> {
  ...
  try {
    ...
    if (ledger.state !== "settled") {
      const priorId = decryptProviderId(ledger.provider_conversation_id_ciphertext);
      const conversationId = await deliverGlobalChatwoot(data, operationKey, priorId, fetchImpl);
      const recorded = await Promise.resolve(supabase.rpc("record_ops_provider_identifier", { p_operation_key: operationKey, p_resource: "conversation", p_provider_id_ciphertext: encryptProviderId(conversationId), p_provider_id_digest: digestProviderId(conversationId), p_status: "remote_succeeded" }));
...
async function processProjection(supabase: Supabase, row: Row, fetchImpl: Fetcher): Promise<void> {
  ...
  const result = await providerCall(row.provider, mapping, row.aggregate_type, operationKey, priorContactId, priorConversationId, fetchImpl);
...
async function chatwootCall(mapping: Extract<AdapterMapping, { provider: "chatwoot" }>, aggregateType: Row["aggregate_type"], operationKey: string, priorContactId: string | null, priorConversationId: string | null, fetchImpl: Fetcher): Promise<ProviderResult> {
  ...
  let contactId = mapping.fields.contactId ?? priorContactId ?? await chatwootFindContact(base, account, mapping.fields.externalId, token, fetchImpl);
  ...
  const conversations = await request(base, `/api/v1/accounts/${encodeURIComponent(account)}/conversations?inbox_id=${encodeURIComponent(inbox)}`, "GET", token, operationKey + ":conversation-lookup", undefined, fetchImpl); const existing = findConversation(conversations.body, mapping.fields.externalId, priorConversationId ?? undefined);
...
async function chatwootFindContact(base: URL, account: string, externalId: string, token: string, fetchImpl: Fetcher): Promise<string | null> { const result = await request(base, `/api/v1/accounts/${encodeURIComponent(account)}/contacts/search?q=${encodeURIComponent(externalId)}`, "GET", token, "lookup", undefined, fetchImpl); const payload = result.body?.payload; const first = Array.isArray(payload) ? payload.find((item: any) => item?.identifier === externalId || item?.custom_attributes?.blockwise_external_id === externalId) : null; return stringId((first as Record<string, unknown> | null)?.id); }
