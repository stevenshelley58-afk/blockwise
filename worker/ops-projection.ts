...
export async function runGlobalProjectionOnce(supabase: Supabase, fetchImpl: Fetcher = fetch): Promise<boolean> {
  ...
  try {
    ...
    await deliverGlobalChatwoot(resolved.data as Record<string, unknown>, row.id, fetchImpl);
...
function findConversation(body: Record<string, any> | null, externalId: string): Record<string, any> | null { const values = Array.isArray(body?.data?.payload) ? body?.data?.payload : Array.isArray(body?.payload) ? body.payload : []; return (values as Record<string, any>[]).find((item) => item.custom_attributes?.blockwise_external_id === externalId) ?? null; }
