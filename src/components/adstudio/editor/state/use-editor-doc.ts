...
export function useEditorDoc({
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        setSaveError(cause instanceof Error ? cause.message : "The ad could not be saved.");
      } finally {
        setSaving(false);
      }
    }, saveDelayMs);
  ...
  return useMemo(
    ...
    [state.instance, state.denied, state.past.length, state.future.length, mode, undo, redo, edit, saving, dirty, saveError],
  );
}
