...
import { uploadAdStudioMedia } from "@/components/adstudio/media-upload";
import { uploadAdStudioVideo } from "@/components/adstudio/video/video-asset-upload";
import { VIDEO_HOOKS, VIDEO_PRESENTERS, VIDEO_RECIPES, buildVideoScenes, createVideoDraft } from "@/lib/adstudio/video/recipes";
...
type ApiBody = { project?: { id?: string; status?: string; version?: number; project?: Record<string, unknown>; plan?: Record<string, unknown> }; renderJob?: { status?: string }; scriptPlan?: VideoScriptPlan; renderUrl?: string | null; posterUrl?: string | null; captionsUrl?: string | null; job?: { status?: string }; error?: string };

export function VideoNewFlow({ workspaceId, brandKitId, brandAssets = [], brandSnapshot = {} }: { workspaceId: string; brandKitId?: string; brandAssets?: VideoAsset[]; brandSnapshot?: BrandSnapshot }) {
  ...
  const [testimonialSubject, setTestimonialSubject] = useState("");
  const [testimonialConsent, setTestimonialConsent] = useState(false);
  const scenes = useMemo(() => buildVideoScenes(draft.hook, "local_expertise"), [draft.hook]);
  const recipe = VIDEO_RECIPES[draft.recipeId];
  const availableAssets = [...brandAssets, ...uploadedAssets];
  const selectedAssets = availableAssets.filter((asset) => draft.selectedAssetIds.includes(asset.id));
  const missingRequiredAssets = recipe.requiredAssets.filter((kind) => !selectedAssets.some((asset) => asset.kind === kind));
  const needsTestimonialConsent = draft.recipeId === "testimonial_case_study" && selectedAssets.some((asset) => asset.kind === "testimonial");
  const canSave = draft.serviceArea.trim().length >= 2 && draft.offer.trim().length >= 2;
  const canGenerate = canSave && missingRequiredAssets.length === 0 && (!needsTestimonialConsent || (testimonialConsent && testimonialSubject.trim().length >= 2));
  ...
  useEffect(() => {
    ...
    if (!id) return;
    void request(`/api/adstudio/videos/${encodeURIComponent(id)}`, { method: "GET" }).then((body) => {
      acceptProject(body.project);
      if (body.renderUrl) setRenderUrl(body.renderUrl);
      if (body.posterUrl) setPosterUrl(body.posterUrl);
      const input = body.project?.project;
      const brief = input?.brief && typeof input.brief === "object" ? input.brief as Record<string, unknown> : {};
      if (input) setDraft((current) => ({ ...current, recipeId: input.recipeId as VideoRecipeId ?? current.recipeId, serviceArea: typeof brief.serviceArea === "string" ? brief.serviceArea : current.serviceArea, offer: typeof brief.offer === "string" ? brief.offer : current.offer, cta: typeof brief.cta === "string" ? brief.cta : current.cta }));
      const savedPlan = body.project?.plan?.scriptPlan;
      if (savedPlan && typeof savedPlan === "object") setScriptPlan(savedPlan as VideoScriptPlan);
    }).catch(() => undefined);
  }, []);
  ...
  const projectInput = {
    ...
    consentRecords: needsTestimonialConsent && testimonialConsent ? [{ id: `consent:${testimonialSubject.trim().toLowerCase().replace(/\s+/gu, "-")}`, assetId: selectedAssets.find((asset) => asset.kind === "testimonial")?.id ?? "", subject: testimonialSubject.trim(), scope: "Paid and organic advertising video", capturedAt: new Date().toISOString(), status: "approved" as const }] : [],
    captions: true,
  ...
  async function ensureVideo() {
    ...
    acceptProject(body.project);
    window.history.replaceState(null, "", `/ad-studio/video/new?videoId=${encodeURIComponent(body.project.id)}`);
    return body.project.id;
  }

  async function saveProject(id: string, plan?: VideoScriptPlan, assetsOverride?: VideoAsset[]) {
    const body = await request(`/api/adstudio/videos/${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify({ project: assetsOverride ? { ...projectInput, assets: assetsOverride } : projectInput, ...(plan ? { plan: { scriptPlan: plan } } : {}), expectedVersion: versionRef.current }) });
    acceptProject(body.project);
    return body;
  }
  ...
  return <div className="mx-auto w-full max-w-[1180px] px-4 py-6 pb-24 sm:px-6 lg:py-8">
    ...
    <div className="mt-7 grid gap-7 xl:grid-cols-[minmax(0,1fr)_360px]"><section aria-label="Video setup" className="min-w-0">
      ...
      <Card className="mt-4 rounded-(--r-panel) border-(--line) shadow-card"><CardHeader><CardTitle>{STEPS[step]}</CardTitle><CardDescription>{step === 0 ? "Choose a lead-generation recipe, audience and outcome." : step === 1 ? "Give the story enough local detail to sound like you." : step === 2 ? "Choose the amount of camera presence that feels right." : step === 3 ? "Pick one opening idea. The rest follows the brief." : step === 4 ? "Use approved Brand Pack assets. Required assets are called out." : scriptPlan ? "Edit the generated script before you queue the render." : "Generate a script to review the four scenes."}</CardDescription></CardHeader><CardContent>
        {step === 0 ? <><ChoiceGroup label="Lead-generation recipe" value={draft.recipeId} options={Object.values(VIDEO_RECIPES).map((item) => ({ value: item.id, label: item.name, detail: item.audience }))} onChange={(value) => update("recipeId", value as VideoRecipeId)} /><p className="mt-4 rounded-(--r-card) bg-muted/40 p-3 text-sm text-muted-foreground">Audience: <span className="font-semibold text-foreground">{recipe.audience}</span><br />Goal: <span className="font-semibold text-foreground">{recipe.name}</span></p></> : null}
        ...
        {step === 4 ? <AssetPicker assets={availableAssets} selectedIds={draft.selectedAssetIds} required={recipe.requiredAssets} optional={recipe.optionalAssets} testimonialSubject={testimonialSubject} testimonialConsent={testimonialConsent} onTestimonialSubject={setTestimonialSubject} onTestimonialConsent={setTestimonialConsent} onToggle={(id) => update("selectedAssetIds", draft.selectedAssetIds.includes(id) ? draft.selectedAssetIds.filter((item) => item !== id) : [...draft.selectedAssetIds, id])} onUpload={async (file, kind) => { setUploadStatus(`Uploading ${kind}…`); setError(null); try { const id = videoId ?? await ensureVideo(); const asset = file.type.startsWith("video/") ? await uploadAdStudioVideo({ file, workspaceId, projectId: id, onProgress: setUploadStatus }) : brandKitId ? { id: `upload:${Date.now()}`, kind, url: (await uploadAdStudioMedia({ file, workspaceId, brandKitId })).src } as VideoAsset : (() => { throw new Error("Set up your Brand Pack before uploading an image."); })(); setUploadedAssets((current) => [...current, asset]); update("selectedAssetIds", [...draft.selectedAssetIds, asset.id]); await saveProject(id, undefined, [...selectedAssets, asset]); setUploadStatus("Asset attached"); } catch (caught) { setError(caught instanceof Error ? caught.message : "Could not upload this asset."); setUploadStatus(null); } }} uploadStatus={uploadStatus} /> : null}
      ...
      {step === 5 && !canGenerate ? <div role="status" className="mt-4 rounded-(--r-card) border border-amber-500/30 bg-amber-500/5 p-3 text-sm"><p className="font-semibold">Finish setup before generating</p><p className="mt-1 text-muted-foreground">{missingRequiredAssets.length ? `Select required ${missingRequiredAssets.join(", ")} asset(s) in Assets.` : needsTestimonialConsent ? "Add the testimonial subject and confirm advertising permission." : "Add a service area and offer in the Brief step."}</p></div> : null}
      <div className="mt-4 flex flex-wrap justify-between gap-2"><Button type="button" variant="ghost-pill" onClick={() => setStep((current) => Math.max(0, current - 1))} disabled={step === 0 || busy} className="min-h-11"><ArrowLeft aria-hidden className="size-4" /> Back</Button>{step < STEPS.length - 1 ? <Button type="button" onClick={() => setStep((current) => Math.min(STEPS.length - 1, current + 1))} disabled={!canContinue || busy} className="min-h-11">Continue <ArrowRight aria-hidden className="size-4" /></Button> : scriptPlan ? <Button type="button" onClick={() => void queueRender()} disabled={busy || !canGenerate} className="min-h-11">{busy ? <><LoaderCircle aria-hidden className="size-4 animate-spin" /> Queueing…</> : "Queue render"}</Button> : <Button type="button" onClick={() => void generateScript()} disabled={busy || !canGenerate} className="min-h-11">{busy ? <><LoaderCircle aria-hidden className="size-4 animate-spin" /> Writing…</> : "Generate script"}</Button>}</div>
...
function AssetPicker({ assets, selectedIds, required, optional, onToggle, onUpload, uploadStatus, testimonialSubject, testimonialConsent, onTestimonialSubject, onTestimonialConsent }: { assets: VideoAsset[]; selectedIds: string[]; required: string[]; optional: string[]; onToggle: (id: string) => void; onUpload: (file: File, kind: VideoAsset["kind"]) => Promise<void>; uploadStatus: string | null; testimonialSubject: string; testimonialConsent: boolean; onTestimonialSubject: (value: string) => void; onTestimonialConsent: (value: boolean) => void }) {
  const [kind, setKind] = useState<VideoAsset["kind"]>("photo");
  return <div className="space-y-4"><div><p className="text-sm font-semibold">Assets for this recipe</p><p className="mt-1 text-xs text-muted-foreground">Required: {required.join(", ") || "none"} · Optional: {optional.join(", ") || "none"}</p></div><div className="rounded-(--r-card) border border-dashed border-border bg-muted/20 p-4"><div className="flex flex-wrap items-center gap-2"><label className="flex min-h-11 items-center gap-2 text-sm font-semibold">Upload as <select value={kind} onChange={(event) => setKind(event.target.value as VideoAsset["kind"])} className="min-h-11 rounded-(--r-control) border border-input bg-background px-2"><option value="photo">Photo</option><option value="video">Video</option><option value="proof">Proof</option><option value="testimonial">Testimonial</option></select></label><label className="inline-flex min-h-11 cursor-pointer items-center rounded-full bg-primary px-4 text-sm font-semibold text-primary-foreground focus-within:ring-2 focus-within:ring-ring">Choose file<input type="file" accept={kind === "video" ? "video/mp4,video/webm" : "image/png,image/jpeg,image/webp"} className="sr-only" disabled={Boolean(uploadStatus)} onChange={(event) => { const file = event.target.files?.[0]; event.currentTarget.value = ""; if (file) void onUpload(file, kind); }} /></label></div>{uploadStatus ? <p role="status" className="mt-2 text-xs text-muted-foreground">{uploadStatus}</p> : <p className="mt-2 text-xs text-muted-foreground">Images use your Brand Pack upload. Videos are verified before they are attached.</p>}</div>{assets.length ? <div className="grid gap-2 sm:grid-cols-2">{assets.map((asset) => <button key={asset.id} type="button" aria-pressed={selectedIds.includes(asset.id)} onClick={() => onToggle(asset.id)} className={`flex min-h-16 items-center gap-3 rounded-(--r-card) border p-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${selectedIds.includes(asset.id) ? "border-primary bg-primary/5" : "border-border bg-card hover:bg-muted/50"}`}>{asset.url.startsWith("http") ? <img src={asset.url} alt={asset.alt ?? ""} className="size-10 rounded-(--r-control) object-cover" /> : <span className="grid size-10 place-items-center rounded-(--r-control) bg-muted text-xs font-bold">{asset.kind.slice(0, 1).toUpperCase()}</span>}<span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{asset.alt ?? asset.kind}</span><span className="block text-xs text-muted-foreground">{asset.kind}{required.includes(asset.kind) ? " · required" : " · optional"}</span></span>{selectedIds.includes(asset.id) ? <Check aria-hidden className="size-4 text-primary" /> : null}</button>)}</div> : <div className="rounded-(--r-card) border border-dashed border-border p-4 text-sm text-muted-foreground">No approved assets are available yet. Upload one here or <Link className="font-semibold text-foreground underline underline-offset-2" href="/ad-studio/library?tab=assets">open Assets</Link>.</div>}</div>;
}
