...
import { validateInstantForm, type ValidationIssue } from "@/lib/adbuilder/instant-form-generator";
import {
  ACTION_TYPES,
  CONTACT_FIELD_TYPES,
  instantFormSchema,
  ...
  type InstantForm,
} from "@/lib/adbuilder/instant-form-types";
...
export function InstantFormEditor({ adId, workspaceId, onPinStateChange }: InstantFormEditorProps) {
  ...
  const basePath = `/api/adbuilder/ads/${encodeURIComponent(adId)}/instant-form?workspaceId=${encodeURIComponent(workspaceId)}`;
...
// ---------------------------------------------------------------------------
// Small form primitives — styled to match the AdBuilder design tokens.
// ---------------------------------------------------------------------------
