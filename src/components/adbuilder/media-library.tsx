...
import { SearchField, SearchFilterPanel, SearchFilterRow, filterChipClassName, sortControlClassName } from "@/components/adbuilder/search-filter-controls";
...
import { filterAndSortAssets } from "@/lib/adbuilder/library-contract";
import type { LibraryAssetModel } from "@/lib/adbuilder/library-read-model";
import { uploadAdBuilderMedia } from "./media-upload";
...
export function MediaLibrary({ workspaceId, brandKitId, assets, nextAssetCursor: initialCursor, embedded = false }: MediaLibraryProps) {
  ...
  async function processUploads(items: UploadItem[]) {
    for (const item of items) {
      ...
      try {
        const result = await uploadAdBuilderMedia({ file: item.file, workspaceId, brandKitId });
  ...
  async function loadMore() {
    ...
    try {
      ...
      const response = await fetch(`/api/adbuilder/bootstrap?${params}`, { cache: "no-store" });
  ...
  return (
    <>
      ...
      <header className={embedded ? "mb-4 flex justify-end" : "flex flex-wrap items-start justify-between gap-4"}>
        {!embedded ? <div><h1 className="font-display text-[27px] font-extrabold tracking-[-.02em]">Photos &amp; logos</h1><p className="mt-2 max-w-[62ch] text-sm leading-6 text-muted-foreground">Keep your media ready for the next ad.</p></div> : null}
        <Button type="button" size="pill" className="min-h-11" disabled={!brandKitId} onClick={openPicker}><Upload aria-hidden /> Upload photos &amp; logos</Button>
      </header>
      <input ref={inputRef} type="file" accept={AD_IMAGE_UPLOAD_TYPES.join(",")} multiple hidden aria-label="Upload photos and logos" onChange={(event) => chooseFiles(event.target.files ?? [])} />

      {allAssets.length > 0 ? <>
      <SearchFilterPanel className={embedded ? "mt-0" : "mt-8"} label="Photos and logos controls">
