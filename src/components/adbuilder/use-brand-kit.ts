...
import { resolveAdvertiserDomain } from "@/lib/adbuilder/advertiser-domain";
...
export function useBrandKit(brandKit: AdBuilderBrandKit) {
  ...
  async function saveKit() {
    ...
    try {
      const res = await fetch(`/api/adbuilder/brand-kits/${kitId}`, {
        method: "PATCH",
  ...
  async function approveKit() {
    ...
    setSaveError(null);
    try {
      const res = await fetch(`/api/adbuilder/brand-kits/${kitId}/approve`, {
        method: "POST",
      });
      if (!res.ok) {
        const json = (await res.json().catch(() => ({}))) as { error?: string };
