...
export async function uploadAdBuilderMedia(input: {
  file: File;
  workspaceId: string;
  brandKitId: string;
  ...
  const storagePath = `${input.workspaceId}/adbuilder/${input.brandKitId}/${crypto.randomUUID()}-${safeName}`;
  ...
  const response = await fetch(`/api/adbuilder/brand-kits/${input.brandKitId}/assets`, {
    method: "POST",
