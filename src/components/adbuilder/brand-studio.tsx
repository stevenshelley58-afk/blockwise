...
import type { AdBuilderBrandKit } from "@/lib/adbuilder";
import { mediaUrlForStoragePath } from "@/lib/adbuilder/assets";
import {
  LOGO_MAX_BYTES,
  LOGO_UPLOAD_TYPES,
  readFileAsDataUrl,
...
function requirePersistedBrandKit(payload: BrandKitResponse, response: Response, action: string): AdBuilderBrandKit {
...
export function needsLogoImportRecovery(src: string | null | undefined): boolean {
  ...
  try {
    ...
    if (parsed.pathname === "/api/adbuilder/media") return false;
...
export function BrandStudio({ brandKit: initialKit, returnTo = "/ad-builder" }: { brandKit: AdBuilderBrandKit | null; returnTo?: string }) {
  ...
  async function scanSite() {
    ...
    setBusy(true);
    try {
      const res = await fetch("/api/adbuilder/brand-kits/extract", {
        method: "POST",
...
function BrandStudioEditor({ brandKit: initialKit, returnTo }: { brandKit: AdBuilderBrandKit; returnTo: string }) {
  ...
  function setIdentity<K extends keyof AdBuilderBrandKit["identity"]>(key: K, value: AdBuilderBrandKit["identity"][K]) {
    setKit((current) => ({ ...current, identity: { ...current.identity, [key]: value } }));
  }

  function setContact<K extends keyof AdBuilderBrandKit["contact"]>(key: K, value: AdBuilderBrandKit["contact"][K]) {
    setKit((current) => ({ ...current, contact: { ...current.contact, [key]: value } }));
  }

  function setTone<K extends keyof AdBuilderBrandKit["tone"]>(key: K, value: AdBuilderBrandKit["tone"][K]) {
  ...
  async function uploadLogoAsset(file: File): Promise<string> {
    ...
    // first load of /ad-builder/*; this upload path is the only thing here that
    ...
    const response = await fetch(`/api/adbuilder/brand-kits/${kit.brandKitId}/assets`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        assetType: "logo",
  ...
  async function approveKit() {
    ...
    try {
      ...
      const submittedKit: AdBuilderBrandKit = {
        ...kit,
        source: { ...kit.source, url: sourceUrl },
        logos: nextLogos,
      };
      const res = await fetch(`/api/adbuilder/brand-kits/${kit.brandKitId}/approve`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="tw min-h-full bg-background font-sans text-foreground" aria-label="Brand Pack">
      <div className="flex min-h-20 flex-wrap items-center gap-3 border-b border-border bg-card px-4 py-4 md:px-6">
        ...
        <div className="grid gap-0.5"><p className="font-mono text-[10px] font-medium uppercase tracking-[0.12em] text-muted-foreground">Ad Builder</p><h1 className="font-display text-2xl font-extrabold tracking-[-0.02em] md:text-[27px]">Brand Pack</h1></div>
