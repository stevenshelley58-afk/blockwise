...
import { templateAssetProxyUrl } from "../../../lib/adbuilder/pack-gallery.ts";
...
export function nativeTemplateFonts(pack: AdTemplate, adId: string): Array<{ family: string; url: string }> {
  return pack.fonts.map(font => {
    ...
    const url = asset ? templateAssetProxyUrl(pack.templateId, asset[0], adId) : "/fonts/adbuilder/" + encodeURIComponent(file);
