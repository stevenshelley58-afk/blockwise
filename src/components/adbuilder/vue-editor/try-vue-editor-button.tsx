...
export function TryVueEditorButton({ adId, workspaceId, automatic = false }: { adId: string; workspaceId: string; automatic?: boolean }) {
  ...
  const begin = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch("/api/adbuilder/ads/" + encodeURIComponent(adId) + "/vue-copy?workspaceId=" + encodeURIComponent(workspaceId), {
        method: "POST",
      ...
      router.replace("/ad-builder/ads/" + encodeURIComponent(body.ad.adId) + "/canvas");
    } catch (cause) {
  ...
  if (automatic) return <div className="grid min-h-64 place-items-center p-6">
    <div className="space-y-4 text-center">
      ...
      {error && <div className="flex items-center justify-center gap-3">
        ...
        <Button variant="ghost" asChild><Link href="/ad-builder/ads">Back to ads</Link></Button>
      </div>}
    </div>
  </div>;
