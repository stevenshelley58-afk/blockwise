...
import type { TemplateLeadType, TemplateSummary } from "@/lib/adbuilder/pack-gallery";
  ...
  if (!hasAvailableTemplates) {
    return (
      <section aria-labelledby="template-review-heading" className="rounded-(--r-panel) border border-dashed border-(--line-heavy) bg-(--surface-subtle)/50 px-6 py-12 text-center sm:py-16">
        ...
        <p className="mx-auto mt-2 max-w-[48ch] text-sm leading-6 text-muted-foreground">New ads start only from a reviewed Feed and Story design. Finish your Brand Pack or return to Ad Builder while reviewed templates are unavailable.</p>
        <div className="mt-5 flex flex-wrap justify-center gap-2">
          <Button asChild><Link href="/ad-builder/brand">Review Brand Pack</Link></Button>
          <Button asChild variant="outline"><Link href="/ad-builder">Return to Ad Builder</Link></Button>
        </div>
      </section>
    );
  }
