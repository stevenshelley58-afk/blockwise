...
export async function proxy(request: NextRequest) {
  ...
  if (process.env.BLOCKWISE_EMAIL_PREVIEW === "true") {
    ...
    if (pathname === "/email-design" || pathname === "/email-library" || pathname === "/email-notifications" || /^\/email-assets\/sample-(seller-appraisal|buyer-demand)\.jpg$/.test(pathname) || pathname.startsWith("/_next/")) {
