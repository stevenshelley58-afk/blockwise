...
export type V2GenerationInput = {
  workspaceId: string;
  userId: string;
  /** Stable per customer mutation so retries resolve to the same DB rows. */
...
async function resolveImageBytes(
  ...
  try {
    let bytes: Buffer;
    const inline = decodeInlineAdDocImageBytes(ref);
    if (inline) {
      bytes = inline;
    } else {
      // Customer image inputs normally arrive as the workspace media proxy,
      // not a raw bucket path. Parse that canonical form first; raw paths are
      // still accepted for trusted callers and subjected to the same scope
      // and traversal checks below.
      const storagePath = storagePathFromMediaSrc(ref) ?? (ref.startsWith("/") ? null : ref.trim());
      if (!storagePath || storagePath.includes("..") || !storagePath.startsWith(`${input.workspaceId}/`)) {
        throw new V2GenerationError("That image is not in this workspace.");
      }
      const { data, error } = await input.supabase.storage.from("workspace-artifacts").download(storagePath);
      if (error || !data) throw new V2GenerationError("That image could not be loaded. Upload it again and retry.");
      bytes = Buffer.from(await data.arrayBuffer());
    }
    validateAdDocSlotImageBytes(bytes);
    const { width, height } = await validateAdDocSlotImageDimensions(bytes);
    return { bytes, width, height };
  } catch (cause) {
    if (cause instanceof V2GenerationError) throw cause;
    if (cause instanceof AdDocSlotMediaError) throw new V2GenerationError(cause.message);
    throw cause;
  }
}
...
export async function generateV2Campaign(input: V2GenerationInput): Promise<V2GenerationResult> {
  ...
  const campaignId = deterministicUuid(
    `adstudio-v2:campaign:${input.workspaceId}:${input.generationKey}`,
  );
  ...
  const copyPack: AdStudioPlatformCopyPack = {
    copyPackId: deterministicUuid(`adstudio-v2:copy:${campaignId}:${variantId}`),
    campaignId,
    variantId,
    meta: metaCopy,
  ...
  const pack: AdStudioCampaignPack = {
    ...
    copyPacks: [copyPack],
    compliance: {
      reportId: deterministicUuid(`adstudio-v2:compliance:${campaignId}`),
      campaignId,
      status: "needs_review",
      issues: [],
