...
export function validateGalleryTemplate(raw: AdStudioTemplate): AdStudioTemplate {
  ...
  if (
    !isRecord(raw?.sourceAd)
    || !hasExactKeys(raw.sourceAd, ["contentHash", "provenance"])
    || raw.sourceAd.provenance !== "frank_factory"
  ) {
    errors.push("sourceAd must contain only the Frank factory provenance marker and SHA-256 hash");
  }
