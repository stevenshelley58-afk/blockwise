...
export async function resolveTemplateAssetValues(
  ...
  if (adError || !ad?.template_id) throw new Error("ad_not_found");
  ...
  if (assetError) throw new Error("template_asset_load_failed");
  const rows = (assets ?? []) as StoredTemplateAsset[];
  if (rows.length !== declarations.length) throw new Error("template_asset_missing");
  ...
  for (const [assetKey, declaration] of declarations) {
    ...
    if (!asset || asset.file_name !== declaration.fileName || asset.mime_type !== declaration.mimeType || asset.storage_path !== expectedPath) throw new Error("template_asset_missing");
    const { data, error } = await service.storage.from("workspace-artifacts").download(expectedPath);
    if (error || !data) throw new Error("template_asset_missing");
  ...
  for (const input of template.imageInputs) {
    if (input.defaultAssetKey) {
      ...
      if (!defaultBytes) throw new Error("template_asset_missing");
