...
export function parseVideoProjectInput(value: unknown): VideoProjectInput {
  ...
  if (input.brief.verifiedProof && (!input.brief.proofSource || !input.brief.proofDate)) {
...
export function validateVideoScriptPlan(value: unknown, input?: Pick<VideoProjectInput, "durationSeconds" | "brief">): VideoScriptPlan {
  ...
  if (input) {
    ...
    if (!plan.hookVariants[0]?.text.toLocaleLowerCase().includes(area) || !plan.scenes[0]?.narration.toLocaleLowerCase().includes(area)) issues.push("Local service area must be the first intent in the script.");
  }
