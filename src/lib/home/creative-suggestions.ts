import type { TemplateSummary } from "@/lib/adbuilder/pack-gallery";
...
export function buildHomeCreativeSuggestions(input: { templates: readonly TemplateSummary[]; usedTemplateIds: ReadonlySet<string>; hasCreatedAds: boolean; usageReadSucceeded: boolean }): HomeCreativeSuggestions {
  ...
  return { audience: firstAd ? "first_ad" : "returning", status: "ready", items: available.slice(0, 3).map((template) => ({ templateId: template.templateId, name: template.name, previewUrl: template.gallerySampleUrl, href: `/ad-builder/templates/${encodeURIComponent(template.templateId)}` })) };
}
