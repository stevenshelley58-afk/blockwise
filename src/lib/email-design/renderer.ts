...
const contentCopy = (message: EmailMessage, type: "sans" | "serif" = "sans") => `<strong class="ink"${style(`font-weight:600;color:#16181d`)}>${escapeHtml(message.greeting)}</strong><br><span${style(type === "serif" ? "display:inline-block;height:10px" : "")}></span>${escapeHtml(message.intro)}`;
...
function cta(message: EmailMessage, type: "inline" | "letter" | "full") {
  ...
  return `<a href="${escapeHtml(safeHref(message.action.href))}" class="action ${type === "letter" ? "letter-action" : ""}"${style(styles)}>${escapeHtml(message.action.label)}${type === "letter" ? " &nbsp;→" : ""}</a>`;
}
