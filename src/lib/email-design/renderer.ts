...
function dailyVisual(message: EmailMessage): string {
  ...
  if (!chart && !ad) return "";
  return `<tr><td class="body-cell" style="padding:24px 28px 0"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" class="daily-visual" style="width:100%"><tr>${chart ? `<td class="visual-column" valign="top" style="width:58%;padding:0 18px 0 0;vertical-align:top">${chart}${ad ? dailyActivity(message) : ""}</td>` : ""}${ad ? `<td class="visual-column creative-column" valign="top" style="width:42%;padding:0;vertical-align:top">${ad}</td>` : ""}</tr></table></td></tr>`;
}
