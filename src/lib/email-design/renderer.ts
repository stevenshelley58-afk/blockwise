...
function footer(message: EmailMessage) {
  ...
  const link = (label: string, url: string) => `<a class="muted" href="${escapeHtml(safeHref(url))}" style="display:inline-block;padding:10px 4px;color:#545a66;text-decoration:underline">${escapeHtml(label)}</a>`;
...
function chartMarkup(chart: EmailChart | undefined): string {
  if (!chart || chart.kind !== "bars" || !chart.values.length) return "";
  const max = Math.max(...chart.values.map(item => item.value), 0);
  const plotHeight = 148;
  const bars = chart.values.map(item => {
    const height = max > 0 ? Math.round((Math.max(0, item.value) / max) * plotHeight) : 0;
    return `<td class="chart-bar-cell" valign="bottom" style="width:${(100 / chart.values.length).toFixed(2)}%;padding:0 3px;vertical-align:bottom"><div class="chart-count ink" style="height:20px;font:700 12px/20px ${FONT_BODY};color:${INK};text-align:center">${item.value}</div><table role="presentation" width="100%" height="${plotHeight}" cellpadding="0" cellspacing="0" border="0" class="chart-track" style="height:${plotHeight}px;background:${DATA_TRACK};border-radius:6px 6px 2px 2px"><tr><td valign="bottom" height="${plotHeight}" style="height:${plotHeight}px;padding:0;vertical-align:bottom"><div class="chart-bar" style="height:${height}px;background:${DATA_BLUE};border-radius:6px 6px 2px 2px;font-size:0;line-height:0">&nbsp;</div></td></tr></table><div class="chart-label muted" style="height:34px;padding-top:7px;font:600 11px/14px ${FONT_BODY};color:${MUTED};text-align:center;word-break:break-word">${escapeHtml(item.label)}</div></td>`;
  }).join("");
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" class="chart-table" style="width:100%;border-top:1px solid ${LINE}"><tr><td colspan="${chart.values.length}" style="padding:18px 0 4px;font:800 15px/20px ${FONT_HEAD};color:${INK}">${escapeHtml(chart.title)}</td></tr><tr>${bars}</tr><tr><td colspan="${chart.values.length}" class="muted" style="padding:0 0 2px;font:400 11px/16px ${FONT_BODY};color:${MUTED}">${escapeHtml(chart.unit)}</td></tr></table>`;
}
...
function adMarkup(previews: ReadonlyArray<EmailAdPreview> | undefined, heading = "Creative context"): string {
  ...
  const cards = previews.slice(0, 2).map(item => { const intrinsicWidth = item.width ?? AD_WIDTH; const intrinsicHeight = item.height ?? AD_HEIGHT; return `<td class="creative-column" valign="top" style="width:50%;padding:0 8px 0 0;vertical-align:top"><img src="${escapeHtml(safeHref(item.src))}" width="220" height="${Math.round(220 * intrinsicHeight / intrinsicWidth)}" alt="${escapeHtml(item.alt)}" style="display:block;width:100%;max-width:220px;height:auto;border-radius:10px;background:#f1f2f4"><p class="ink ad-label" style="margin:8px 0 0;font:800 12px/16px ${FONT_HEAD};color:${INK}">${escapeHtml(item.label)}</p>${item.detail ? `<p class="muted ad-detail" style="margin:3px 0 0;font:400 11px/16px ${FONT_BODY};color:${MUTED}">${escapeHtml(item.detail)}</p>` : ""}</td>`; }).join("");
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" class="creative-grid" style="width:100%;border-top:1px solid ${LINE}"><tr><td colspan="${Math.min(previews.length, 2)}" class="ink" style="padding:18px 0 10px;font:800 15px/20px ${FONT_HEAD};color:${INK}">${escapeHtml(heading)}</td></tr><tr>${cards}</tr></table>`;
}
...
function dailyVisual(message: EmailMessage): string {
  const chart = chartMarkup(message.visual?.chart); const ad = adMarkup(message.visual?.adPreviews?.slice(0, 1), "Featured creative");
  if (!chart && !ad) return "";
  return `<tr><td class="body-cell" style="padding:24px 28px 0"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" class="daily-visual" style="width:100%"><tr>${chart ? `<td class="visual-column" valign="top" style="width:58%;padding:0 18px 0 0;vertical-align:top">${chart}</td>` : ""}${ad ? `<td class="visual-column creative-column" valign="top" style="width:42%;padding:0;vertical-align:top">${ad}</td>` : ""}</tr></table></td></tr>`;
}
...
function notificationCard(message: EmailMessage): string {
  ...
  const visual = isNewLead ? newLeadVisual(message) : message.kind === "daily-digest" ? dailyVisual(message) : chartBlock(message.visual?.chart) + adBlock(message.visual?.adPreviews);
  ...
  const sections = isNewLead ? sectionContent(message) : (message.kind === "weekly-performance" && message.visual?.adPreviews?.length ? sectionContent({ ...message, sections: (message.sections ?? []).filter(section => section.layout !== "list-item") }) : sectionContent(message));
  return `<tr><td align="center" style="padding:34px 20px 18px">${logo()}</td></tr><tr><td style="padding:0 20px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" class="card notification-card" style="width:100%;border:1px solid ${LINE};border-radius:16px;background:#ffffff"><tbody><tr><td class="body-cell" style="padding:28px 28px 0"><h1 class="ink" style="margin:0;font:800 28px/32px ${FONT_HEAD};letter-spacing:-.6px;color:${INK}">${escapeHtml(message.heading)}</h1><p class="muted period-intro" style="margin:10px 0 0;font:400 13.5px/20px ${FONT_BODY};color:${MUTED}">${intro}</p></td></tr>${strip}${isNewLead ? action : ""}${visual}${isNewLead ? "" : action}${isNewLead ? "" : detailsRow}${sections}${note(message, "padding:18px 28px 0")}${signOff(message, "padding:20px 28px 30px")}</tbody></table></td></tr>`;
}
function quietCard(message: EmailMessage) {
  ...
  return `<tr><td align="center" style="padding:34px 20px 18px">${logo()}</td></tr><tr><td style="padding:0 20px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" class="card" style="width:100%;border:1px solid #e9ebef;border-radius:20px;background:#ffffff"><tbody><tr><td class="body-cell muted" style="padding:28px 28px 12px;font:600 10px/14px ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.12em;color:#545a66">${escapeHtml(message.eyebrow)}</td></tr><tr><td class="body-cell" style="padding:0 28px"><h1 class="ink" style="margin:0;font:700 27px/32px ${FONT_BODY};letter-spacing:-.5px;color:#16181d">${escapeHtml(message.heading)}</h1></td></tr><tr><td class="body-cell muted" style="padding:16px 28px 0;white-space:pre-line;font:400 15px/23px ${FONT_BODY};color:#545a66">${contentCopy(message)}</td></tr>${summary(message)}${optionalDetails}${sectionContent(message)}${optionalAction}${note(message, "padding:18px 28px 0")}${signOff(message, "padding:20px 28px 30px")}</tbody></table></td></tr>`;
}
