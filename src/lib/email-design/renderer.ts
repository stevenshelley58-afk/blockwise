...
function footer(message: EmailMessage) {
  ...
  const preferences = message.transactional ? "" : `<br>${link("Manage preferences", info?.preferencesUrl ?? "#")} · ${link("Unsubscribe", info?.unsubscribeUrl ?? "#")}`;
