import type { AdBuilderCopyFields } from "./copy-generation.ts";
import { runAdBuilderComplianceReview } from "./compliance.ts";
...
export function applyProvidedCopyToCampaignPack(
  ...
  const compliance = runAdBuilderComplianceReview({ campaign: next.campaign, copyPacks: next.copyPacks });
  return {
    ...next,
    campaign: {
      ...next.campaign,
...
function updateCopyPack(copyPack: AdBuilderPlatformCopyPack, copy: AdBuilderCopyFields): AdBuilderPlatformCopyPack {
