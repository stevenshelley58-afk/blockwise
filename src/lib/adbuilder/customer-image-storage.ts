...
export async function storeCustomerImageBytes(input: {
  ...
  try {
    ...
    const kind = error instanceof CustomerImageValidationError || /invalid|mismatch/i.test(error instanceof Error ? error.message : "")
      ? "invalid"
      : "storage";
    console.error("Ad Builder customer image upload failed", { reason });
...
async function removeCustomerImageObject(supabase: SupabaseClient, path: string): Promise<void> {
  try {
    ...
    if (result.error) console.error("Ad Builder customer image cleanup failed", { message: result.error.message });
  } catch (error) {
    console.error("Ad Builder customer image cleanup failed", { reason: error instanceof Error ? error.message : "unknown" });
  }
}
...
export async function resolveCustomerImageValues(
  ...
  for (const [inputKey, value] of Object.entries(sharedImageValues)) {
    try {
      ...
      console.error("Ad Builder customer image storage failed", { inputKey, reason });
