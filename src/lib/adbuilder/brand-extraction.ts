...
import type { AdBuilderBrandKit, AdBuilderReviewStatus } from "./types.ts";
...
export function extractBrandKitFromWebsite(input: ExtractBrandKitInput): AdBuilderBrandKit {
...
export function mergeBrandKitReview(
  existing: AdBuilderBrandKit,
  next: AdBuilderBrandKit,
  patch: BrandKitReviewPatch = {},
): AdBuilderBrandKit {
  const lockedFields = patch.lockedFields ?? existing.lockedFields;
  const merged = structuredClone(next) as AdBuilderBrandKit;
