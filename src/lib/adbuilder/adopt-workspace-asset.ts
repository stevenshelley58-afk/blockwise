...
export async function adoptWorkspaceAsset(input: {
  ...
  const prepared = await input.serviceSupabase.rpc("adbuilder_prepare_customer_image_upload", {
  ...
  const claimed = await input.serviceSupabase.rpc("adbuilder_claim_customer_image_finalize", { p_reservation_id: result.reservationId, p_workspace_id: input.workspaceId, p_ad_id: input.adId, p_object_path: parsed.path, p_sha256: sha256, p_mime_type: mime, p_byte_size: bytes.length });
  ...
  const finalized = await input.serviceSupabase.rpc("adbuilder_finalize_customer_image_upload", { p_reservation_id: result.reservationId, p_workspace_id: input.workspaceId, p_ad_id: input.adId, p_object_path: parsed.path, p_sha256: sha256, p_mime_type: mime, p_byte_size: bytes.length });
...
export async function discardCustomerImageUpload(input: { serviceSupabase: SupabaseClient }, reservationId: string, workspaceId: string, adId: string, path: string) {
  const claimed = await input.serviceSupabase.rpc("adbuilder_discard_customer_image_upload", { p_reservation_id: reservationId, p_workspace_id: workspaceId, p_ad_id: adId, p_object_path: path });
  if (claimed.error || claimed.data !== true) return;
  await input.serviceSupabase.storage.from(CUSTOMER_IMAGE_BUCKET).remove([path]);
  await input.serviceSupabase.rpc("adbuilder_complete_customer_image_stale_cleanup", { p_reservation_id: reservationId, p_object_path: path });
}
...
async function removeStale(supabase: SupabaseClient, entries: Array<{ id: string; path: string }> | undefined) {
  for (const entry of entries ?? []) {
    ...
    await supabase.rpc("adbuilder_complete_customer_image_stale_cleanup", { p_reservation_id: entry.id, p_object_path: entry.path });
  }
}
