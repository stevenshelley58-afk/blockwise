...
export function useVideoUpload(input: { workspaceId: string; projectId: string }) {
  ...
  const upload = useCallback(
    async (file: File) => {
      ...
      try {
        const started = await fetch("/api/adbuilder/videos/uploads", {
          method: "POST",
          ...
          for (let attempt = 1; attempt <= MAX_ATTEMPTS_PER_CHUNK && !sent; attempt += 1) {
            ...
            try {
              const response = await fetch(
                `/api/adbuilder/videos/uploads/${uploadId}?workspaceId=${encodeURIComponent(input.workspaceId)}&offset=${offset}`,
                {
                  method: "PUT",
        ...
        const finalised = await fetch(
          `/api/adbuilder/videos/uploads/${uploadId}?workspaceId=${encodeURIComponent(input.workspaceId)}`,
          {
            method: "POST",
            headers: { "content-type": "application/json" },
            signal: controller.signal,
