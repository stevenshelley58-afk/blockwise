...
import {
  ADBUILDER_EMBEDDED_ASSET_LIMIT,
  applyBrandAssetRows,
  loadAdBuilderBrandAssetRows,
} from "./assets.ts";
import type {
  AdBuilderBrandKit,
  AdBuilderCampaign,
...
export function isExampleBrandKit(brandKit: Pick<AdBuilderBrandKit, "source">): boolean {
...
export async function persistAdBuilderBrandKit(
  supabase: SupabaseServerClient,
  brandKit: AdBuilderBrandKit,
  userId: string,
  ...
  const result = await supabase.from("adbuilder_brand_kits").upsert(
    {
...
export async function persistAdBuilderCampaignPack(
  supabase: SupabaseServerClient,
  ...
  // One transactional RPC (adbuilder_persist_campaign_pack, SECURITY INVOKER so
  // RLS still applies): a failure in any table rolls back the whole pack —
  // partially written campaigns are impossible.
  const result = await supabase.rpc("adbuilder_persist_campaign_pack", {
    brand_kit: {
...
export function compactAdBuilderCampaignPackForTransport(pack: AdBuilderCampaignPack): AdBuilderCampaignPack {
  return {
    ...pack,
...
function compactCreativesForPersistence(creatives: AdBuilderCreative[]): AdBuilderCreative[] {
...
function compactCreativeCanvas(
  creativeCanvas: AdBuilderCreative["canvas"],
  keepPrimaryImage: boolean,
): AdBuilderCreative["canvas"] {
  return {
    ...creativeCanvas,
...
function primaryImageSource(creative: AdBuilderCreative): string | undefined {
  const image = creative.canvas.objects.find((object) => object.role === "primary_image");
  return image?.content || image?.assetId;
}

export function rowToBrandKit(row: Record<string, unknown>): AdBuilderBrandKit {
  return {
    ...
    identity: row.identity_json as AdBuilderBrandKit["identity"],
    logos: row.logos_json as AdBuilderBrandKit["logos"],
    colours: row.colours_json as AdBuilderBrandKit["colours"],
    typography: row.typography_json as AdBuilderBrandKit["typography"],
    tone: row.tone_json as AdBuilderBrandKit["tone"],
    visualStyle: row.visual_style_json as AdBuilderBrandKit["visualStyle"],
    assets: {
      headshots: [],
      officeImages: [],
      listingImages: [],
    ...
    contact: row.contact_json as AdBuilderBrandKit["contact"],
    compliance: row.compliance_json as AdBuilderBrandKit["compliance"],
    reviewStatus:
...
function rowToVariant(row: Record<string, unknown>): AdBuilderCampaignVariant {
  return {
    ...
    cta: String(row.cta ?? ""),
    score: row.score_json as AdBuilderCampaignVariant["score"],
    status: row.status === "approved" ? "approved" : "draft",
    lockedFields: asStringArray(row.locked_fields_json),
  };
}

export function rowToCreative(row: Record<string, unknown>): AdBuilderCreative {
  const rawCanvas = (row.canvas_json as AdBuilderCreative["canvas"] | null) ?? {
  ...
  return {
    ...
    format: row.format as AdBuilderCreative["format"],
    canvas: rawCanvas,
    safeZones: {
...
function rowToCopyPack(row: Record<string, unknown>): AdBuilderPlatformCopyPack {
  return {
    copyPackId: String(row.id),
    campaignId: String(row.campaign_id),
    variantId: String(row.variant_id),
    meta: row.meta_json as AdBuilderPlatformCopyPack["meta"],
    googleSearch: row.google_search_json as AdBuilderPlatformCopyPack["googleSearch"],
    googlePmax: row.google_pmax_json as AdBuilderPlatformCopyPack["googlePmax"],
    googleDemandGen: row.google_demand_gen_json as AdBuilderPlatformCopyPack["googleDemandGen"],
    landingPage: row.landing_page_json as AdBuilderPlatformCopyPack["landingPage"],
    followUp: row.followup_json as AdBuilderPlatformCopyPack["followUp"],
    lockedFields: asStringArray(row.locked_fields_json),
  };
}

function rowToCompliance(row: Record<string, unknown> | null, campaignId: string): AdBuilderComplianceReport {
  if (!row) {
    return {
  ...
  return {
    ...
    issues: Array.isArray(row.issues_json)
      ? (row.issues_json as AdBuilderComplianceReport["issues"])
      : [],
...
export function rowToCampaignPack(input: {
  ...
  const campaign: AdBuilderCampaign = {
    campaignId,
    ...
    goal: input.campaign.goal as AdBuilderCampaign["goal"],
    market: input.campaign.market_json as AdBuilderCampaign["market"],
    ...
    platforms: (input.campaign.platforms_json as AdBuilderCampaign["platforms"]) ?? [],
    creativeFormats: (input.campaign.creative_formats_json as AdBuilderCampaign["creativeFormats"]) ?? [],
    status: (input.campaign.status as AdBuilderCampaign["status"]) ?? "ready",
  };
...
export async function loadAdBuilderCampaignPack(
  supabase: SupabaseServerClient,
  workspaceId: string,
  campaignId: string,
): Promise<AdBuilderCampaignPack | null> {
  ...
  const [brandKitRow, variants, creatives, copy, compliance] = await Promise.all([
    supabase
      ...
      .maybeSingle(),
    supabase.from("adbuilder_campaign_variants").select("*").eq("workspace_id", workspaceId).eq("campaign_id", campaignId),
    supabase.from("adbuilder_creatives").select("*").eq("workspace_id", workspaceId).eq("campaign_id", campaignId),
    supabase.from("adbuilder_platform_copy").select("*").eq("workspace_id", workspaceId).eq("campaign_id", campaignId),
    supabase
      .from("adbuilder_compliance_reports")
      .select("*")
      .eq("workspace_id", workspaceId)
  ...
  const brandKit = applyBrandAssetRows(
    ...
    await loadAdBuilderBrandAssetRows(
      supabase,
      workspaceId,
...
function templateSource(value: unknown): AdBuilderCampaign["templateSource"] {
