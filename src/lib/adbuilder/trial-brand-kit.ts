...
import { isExampleBrandKitSourceUrl, persistAdBuilderBrandKit, rowToBrandKit } from "./persistence.ts";
...
export async function resolveAdBuilderGenerationBrandKit(input: ResolveGenerationBrandKitInput): Promise<{
  ok: true;
  ...
  const fallbackBrandKit = buildAdBuilderFallbackBrandKit({
    workspaceId: input.workspaceId,
    workspaceName: input.workspaceName,
    region: input.region,
  });
  const persisted = await persistAdBuilderBrandKit(input.supabase, fallbackBrandKit, input.userId);
...
export function buildAdBuilderFallbackBrandKit(input: {
  workspaceId: string;
  workspaceName?: string;
  region?: string;
}): AdBuilderBrandKit {
  const businessName = cleanWorkspaceName(input.workspaceName);
  const fallback: AdBuilderBrandKit = {
