import type { AdBuilderBrandKit, AdBuilderPlatformCopyPack } from "./types.ts";
...
type ResolveAdvertiserDomainInput = {
  ...
  copyPacks?: Pick<AdBuilderPlatformCopyPack, "googleSearch" | "googlePmax" | "googleDemandGen">[];
};
...
export function resolveAdvertiserBaseUrl(brandKit: AdBuilderBrandKit): string {
  return resolveAdvertiserDomain({ brandKit }).baseUrl;
}

export function resolveLeadFormPrivacyPolicyUrl(brandKit: AdBuilderBrandKit): string {
