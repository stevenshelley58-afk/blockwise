...
export const ADBUILDER_MEDIA_URL_LIMIT = 100;
export const ADBUILDER_MEDIA_URL_TTL_SECONDS = 60 * 60;

export type AdBuilderMediaProfiles = {
  path: string;
  grid: string;
  preview: string;
  full: string;
...
export async function createAdBuilderMediaUrls(input: {
  supabase: StorageClient;
  workspaceId: string;
  paths: string[];
  now?: Date;
}): Promise<Record<string, AdBuilderMediaProfiles>> {
  const paths = [...new Set(input.paths.map((path) => path.trim()))];
  if (paths.length > ADBUILDER_MEDIA_URL_LIMIT) {
    throw new Error(`At most ${ADBUILDER_MEDIA_URL_LIMIT} media paths can be signed at once.`);
  }
  ...
  const expiresAt = new Date(
    (input.now ?? new Date()).getTime() + ADBUILDER_MEDIA_URL_TTL_SECONDS * 1000,
  ).toISOString();
  const entries = await Promise.all(
    paths.map(async (path) => {
      const [grid, preview, full] = await Promise.all([
        bucket.createSignedUrl(path, ADBUILDER_MEDIA_URL_TTL_SECONDS, {
