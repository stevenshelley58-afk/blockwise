export const CUSTOMER_IMAGE_PREFIX = "/api/adbuilder/customer-media?";
export const CUSTOMER_IMAGE_BUCKET = "adbuilder-customer-images";
...
export function buildCustomerImageRef(workspaceId: string, adId: string, sha256: string, mime: CustomerImageMime): string {
  const path = `${workspaceId}/adbuilder/ads/${adId}/images/${sha256}.${MIME_EXTENSIONS[mime]}`;
...
export function parseCustomerImageRef(value: string, workspaceId: string, adId: string): { path: string; sha256: string; mime: CustomerImageMime } | null {
  ...
  const prefix = `${workspaceId}/adbuilder/ads/${adId}/images/`;
