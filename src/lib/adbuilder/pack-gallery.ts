 ...
 * (gallery, Ad Builder home, home dashboard, audit picker). Anything that needs
...
export function gallerySampleProxyUrl(
  ...
  return `/api/adbuilder/templates/${encodeURIComponent(templateId)}/sample?${params}`;
}
...
export function templateAssetProxyUrl(templateId: string, assetKey: string, existingAdId?: string): string | null {
  ...
  const path = `/api/adbuilder/templates/${encodeURIComponent(templateId)}/assets/${encodeURIComponent(assetKey)}`;
 ...
 * summaries the caller then throws away. The Ad Builder home page only asks
