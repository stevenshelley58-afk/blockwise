import { assetUrlForRow, type AdBuilderBrandAssetRow } from "./assets.ts";
...
type QueryClient = {
  ...
  storage: Parameters<typeof createAdBuilderMediaUrls>[0]["supabase"]["storage"];
};

type Cursor = { orderAt: string; id: string };

export async function loadAdBuilderLibraryPage(input: {
  ...
  let query = input.supabase
    .from(input.kind === "assets" ? "adbuilder_brand_assets" : "ad_customer_ads")
    .select("*")
  ...
  if (input.kind === "ads") {
    ...
    // identity through adbuilder_campaign_id. Read it workspace-scoped and use
    // only an applied activation/pause mutation as state evidence.
    try {
      const { data: plans, error: planError } = pageRows.length
        ? await input.supabase.from("meta_publish_plans").select("id,adbuilder_campaign_id,status,updated_at").eq("workspace_id", input.workspaceId).in("adbuilder_campaign_id", pageRows.map((row) => String(row.id))).order("updated_at", { ascending: false })
        : { data: [], error: null };
      if (planError) throw new Error(planError.message);
      for (const plan of (plans ?? []) as Array<Record<string, unknown>>) {
        const adId = String(plan.adbuilder_campaign_id);
  ...
  if (input.kind === "assets") {
    for (const row of pageRows as AdBuilderBrandAssetRow[]) {
  ...
  const signed =
    paths.size > 0
      ? await createAdBuilderMediaUrls({
...
function assetMetadata(row: AdBuilderBrandAssetRow): Pick<LibraryAssetModel, "width" | "height" | "dimensionsLabel" | "createdAt" | "lastUsedAt" | "usageCount"> {
...
function metadataForRow(row: AdBuilderBrandAssetRow): Record<string, unknown> {
