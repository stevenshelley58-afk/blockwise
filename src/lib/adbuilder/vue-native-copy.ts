...
async function finalizeCopiedImageLedger(input: {
  ...
  const prepared = await input.serviceSupabase.rpc("adbuilder_prepare_customer_image_upload", metadata);
  ...
  const claimed = await input.serviceSupabase.rpc("adbuilder_claim_customer_image_finalize", {
  ...
  const finalized = await input.serviceSupabase.rpc("adbuilder_finalize_customer_image_upload", {
    p_reservation_id: reservationId,
    ...metadata,
  });
