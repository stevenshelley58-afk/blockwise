/**
 * The one contract for a customer-supplied image source in Ad Builder.
 *
 ...
 * - `workspace-media` `/api/adbuilder/media?path=<workspace storage path>` —
 *                     the durable, full-resolution, auth-gated proxy. This is
 *                     the preferred form for anything we store: it never
 *                     expires and `resolveAdBuilderImageForModel` streams the
 ...
 *                     `resolveAdBuilderImageForModel` passes these to the model
...
export type AdBuilderImageSrcKind = "inline" | "workspace-media" | "builtin" | "remote";

export const ADBUILDER_MEDIA_SRC_PREFIX = "/api/adbuilder/media?";

const BUILTIN_PREFIXES = ["/adbuilder-samples/", "/ads/"] as const;
...
export function workspaceMediaSrc(
  ...
  return `${ADBUILDER_MEDIA_SRC_PREFIX}path=${encodeURIComponent(path)}`;
}
...
export function storagePathFromMediaSrc(src: string | null | undefined): string | null {
  ...
  if (!value?.startsWith(ADBUILDER_MEDIA_SRC_PREFIX)) return null;
  const query = value.slice(value.indexOf("?") + 1);
  const path = new URLSearchParams(query).get("path")?.trim();
  return path || null;
}

export function classifyAdBuilderImageSrc(
  value: string | null | undefined,
): AdBuilderImageSrcKind | null {
  const src = value?.trim();
  if (!src) return null;
  if (src.startsWith("data:image/")) return "inline";
  if (src.startsWith(ADBUILDER_MEDIA_SRC_PREFIX)) {
...
export function isAdBuilderImageSrc(value: string | null | undefined): boolean {
  return classifyAdBuilderImageSrc(value) !== null;
}
