...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!["adjust_credits", "prepare_booking", "complete_onboarding", "approve_managed_scope"].includes(action)) {
  ...
  try {
    const result = await runOperatorCustomerAction({
      ...
      action: action as "adjust_credits" | "prepare_booking" | "complete_onboarding" | "approve_managed_scope",
      mutationId,
      reason,
