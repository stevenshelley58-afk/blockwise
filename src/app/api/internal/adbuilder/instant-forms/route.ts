...
import { formGenerationInputSchema } from "@/lib/adbuilder/instant-form-types";
import { generateInstantForm, validateInstantForm } from "@/lib/adbuilder/instant-form-generator";

/**
 * POST /api/internal/adbuilder/instant-forms/generate
 *
...
export async function POST(request: Request) {
  ...
  const auth = await verifyInternalRequest(request, "adbuilder.instant-forms", { body: rawBody });
  if (!auth.ok) {
    return NextResponse.json({ error: auth.error }, { status: auth.status });
  }
  const rateLimit = await checkRateLimit(null, "internal:adbuilder.instant-forms", {
    windowSeconds: 60,
    maxRequests: 120,
    bucket: "internal-api",
    failClosed: true,
...
/**
 * PUT /api/internal/adbuilder/instant-forms/validate
 *
...
export async function PUT(request: Request) {
  ...
  const { instantFormSchema } = await import("@/lib/adbuilder/instant-form-types");
