...
import { ingestTemplateArtifact } from "@/lib/adbuilder/ingest-artifact";
...
export async function POST(request: NextRequest) {
  ...
  const auth = await verifyInternalRequest(request, "adbuilder.templates", { body: rawBody });
  if (!auth.ok) {
    return NextResponse.json({ error: auth.error }, { status: auth.status });
  }
  const rateLimit = await checkRateLimit(null, "internal:adbuilder.templates", {
    windowSeconds: 60,
    maxRequests: 120,
    bucket: "internal-api",
    failClosed: true,
