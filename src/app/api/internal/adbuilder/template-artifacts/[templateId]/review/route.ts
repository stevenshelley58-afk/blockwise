...
import { z } from "zod";
import { activateTemplate, discardTemplate, smokeTestTemplate } from "@/lib/adbuilder/template-review";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ templateId: string }> }) {
  ...
  const auth = await verifyInternalRequest(request, "adbuilder.templates.review", { body: rawBody });
  if (!auth.ok) return NextResponse.json({ error: auth.error }, { status: auth.status });
  const rateLimit = await checkRateLimit(null, "internal:adbuilder.templates.review", { windowSeconds: 60, maxRequests: 120, bucket: "internal-api", failClosed: true });
