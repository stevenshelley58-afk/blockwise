...
import { loadPublishState, validatePublishState, freezePublicationSnapshot } from "@/lib/adbuilder/publish-adapter";
...
/**
 * GET /api/internal/adbuilder/publish/state?adId=...&workspaceId=...
 *
 ...
 * (scope "adbuilder.publish").
 */
export async function GET(request: Request) {
  const auth = await verifyInternalRequest(request, "adbuilder.publish");
  if (!auth.ok) {
  ...
  const rateLimit = await checkRateLimit(null, "internal:adbuilder.publish", {
    windowSeconds: 60,
    maxRequests: 120,
    bucket: "internal-api",
    failClosed: true,
...
/**
 * POST /api/internal/adbuilder/publish/freeze
 *
...
export async function POST(request: Request) {
  ...
  const auth = await verifyInternalRequest(request, "adbuilder.publish", { body: rawBody });
  if (!auth.ok) {
    return NextResponse.json({ error: auth.error }, { status: auth.status });
  }
