...
export async function POST(request: Request) {
  ...
  try {
    const receipt = await importTemplatePack(supabase, {
      ...
      runId: input.runId == null ? undefined : String(input.runId),
      releaseId: input.releaseId == null ? undefined : String(input.releaseId),
      traceRef: input.traceRef == null ? undefined : String(input.traceRef),
      qaReceiptRef: input.qaReceiptRef == null ? undefined : String(input.qaReceiptRef),
      approvalReceiptRef: input.approvalReceiptRef == null ? undefined : String(input.approvalReceiptRef),
      sanitizationReceiptRef: input.sanitizationReceiptRef == null ? undefined : String(input.sanitizationReceiptRef),
