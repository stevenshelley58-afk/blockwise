...
const ACTIONS = new Set(["team_invite", "team_resend", "team_cancel", "team_role_change", "team_suspend", "team_reactivate", "session_revoke", "enquiry_assign", "billing_reconcile"]);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (actionType === "team_invite" || actionType === "team_resend") {
    ...
    } else if (actionType === "team_role_change" || actionType === "team_suspend" || actionType === "team_reactivate") {
      ...
      if (actionType === "team_suspend") {
        // Suspension must also end live access: revoke sessions and ban the
        // auth user so a suspended member cannot ride an existing JWT.
        await requireRpc(service, "revoke_user_sessions", { p_user_id: targetId });
        const banned = await service.auth.admin.updateUserById(targetId, { ban_duration: "876000h" });
        if (banned.error) throw new ActionExecutionError("member_ban_failed", 502);
      } else if (actionType === "team_reactivate") {
        const unbanned = await service.auth.admin.updateUserById(targetId, { ban_duration: "none" });
        if (unbanned.error) throw new ActionExecutionError("member_unban_failed", 502);
      }
