...
export async function GET(request: NextRequest) {
  ...
  const limit = Math.max(1, Math.min(14, Number(params.get("limit") ?? "6") || 6));
  ...
  const errors: Array<{ brief_id: string; error: string }> = [];
  ...
  for (const brief of briefs) {
    ...
    try {
      ...
      generated.push(brief.brief_id);
    } catch (caught) {
      errors.push({ brief_id: brief.brief_id, error: caught instanceof Error ? caught.message : "failed" });
    }
  }
