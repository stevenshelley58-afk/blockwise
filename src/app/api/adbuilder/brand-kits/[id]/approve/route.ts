...
import { approveAdBuilderBrandKitForUse, buildAdBuilderLiveResult, type AdBuilderBrandKit } from "@/lib/adbuilder";
import { errorResponse, readJsonBody, requireAdBuilderRequest } from "@/lib/adbuilder/http";
import { isExampleBrandKit, isExampleBrandKitSourceUrl, persistAdBuilderBrandKit, rowToBrandKit } from "@/lib/adbuilder/persistence";
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const body = await readJsonBody<{ brandKit?: AdBuilderBrandKit }>(request);
  ...
  if (body.brandKit) {
    ...
    const approvedBrandKit = approveAdBuilderBrandKitForUse({
      ...body.brandKit,
      brandKitId: id,
      workspaceId: access.access.workspaceId,
    });
    const persisted = await persistAdBuilderBrandKit(access.supabase, approvedBrandKit, access.access.userId);
    if (!persisted.error) {
      await recordBrandPackApproval(access.access.workspaceId, id);
    }
    const liveResult = buildAdBuilderLiveResult({
  ...
  await recordBrandPackApproval(access.access.workspaceId, id);
  const approvedBrandKit = approveAdBuilderBrandKitForUse(rowToBrandKit(data));
  const liveResult = buildAdBuilderLiveResult({ data: approvedBrandKit });
