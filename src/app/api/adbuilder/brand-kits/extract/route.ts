...
import { buildAdBuilderLiveResult, extractBrandKitFromWebsite } from "@/lib/adbuilder";
import { storeBrandKitLogoAssets } from "@/lib/adbuilder/brand-logo-assets.server";
import { normalizeAndValidateExtractionUrl } from "@/lib/adbuilder/extraction-url";
import { isExampleBrandKitSourceUrl, persistAdBuilderBrandKit } from "@/lib/adbuilder/persistence";
...
export async function POST(request: NextRequest) {
  ...
  const rateLimit = await checkRateLimit(context.access.workspaceId, context.access.userId, {
    windowSeconds: 300,
    maxRequests: 10,
    bucket: "adbuilder-brand-extract",
  });
  if (!rateLimit.ok) {
  ...
  try {
    ...
    let persisted = await persistAdBuilderBrandKit(context.supabase, brandKit, context.access.userId);
    ...
    if (!persisted.error) {
      ...
      persisted = await persistAdBuilderBrandKit(context.supabase, brandKit, context.access.userId);
    }
    if (!persisted.error) {
