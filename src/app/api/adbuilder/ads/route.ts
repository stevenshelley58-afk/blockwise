...
import { requireAdBuilderRequest, readJsonBody } from "@/lib/adbuilder/http";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error("[adbuilder] ad pack reservation failed", { workspaceId: access.access.workspaceId, error });
  ...
  try {
    ...
    console.error("[adbuilder] customer ad creation failed", { workspaceId: access.access.workspaceId, templateId });
    return NextResponse.json({ error: "Could not create ad. Please try again." }, { status: 500 });
  }
}
