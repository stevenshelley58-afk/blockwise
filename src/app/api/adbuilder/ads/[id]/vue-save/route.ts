...
import { NativeSaveError, saveNativeAd } from "@/lib/adbuilder/vue-native-save";
import { NativeEditorValidationError, type NativePngExport } from "@/lib/adbuilder/vue-native-validation";
import { SaveError } from "@/lib/adbuilder/save-ad";
      ...
      const status = error.code === "ad_not_found" || error.code === "template_not_found"
        ...
        : error.code === "stale_revision" || error.code === "template_contract_mismatch"
          ...
          : error.code === "revision_commit_failed" || error.code === "render_upload_failed" || error.code === "active_revision_invalid"
            ? 500
            : 400;
      if (status >= 500) console.error("Native Ad Builder save failed", { code: error.code, message: error.message });
