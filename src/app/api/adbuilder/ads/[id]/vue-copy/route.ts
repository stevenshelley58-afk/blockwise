...
import { errorResponse, requireAdBuilderRequest } from "@/lib/adbuilder/http";
import { copyAdToNativeTrial, NativeCopyError } from "@/lib/adbuilder/vue-native-copy";
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    if (error instanceof NativeCopyError) {
      ...
      if (status >= 500) console.error("Native Ad Builder copy failed", { code: error.code, message: error.message });
