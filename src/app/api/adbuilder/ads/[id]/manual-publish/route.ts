...
import { ManualPublishError, createOrLoadManualPublishRequest, getManualPublishRequest } from "@/lib/adbuilder/manual-publish";
...
export async function GET(request: NextRequest, context: Context) {
  ...
  const guard = await requireApiWorkspace(request, "adbuilder", bodyWorkspace);
...
export async function POST(request: NextRequest, context: Context) {
  const body = (await request.json().catch(() => ({}))) as { workspaceId?: unknown; notes?: unknown; mutationId?: unknown; publishSummary?: unknown; controls?: unknown; revisionId?: unknown; documentHash?: unknown };
  const requestedWorkspace = typeof body.workspaceId === "string" ? body.workspaceId : null;
  const guard = await requireApiWorkspace(request, "adbuilder", requestedWorkspace);
  if (!guard.ok) return guard.response;
  if (!guard.access.isOperator && !["owner", "admin"].includes(guard.access.role)) return NextResponse.json({ error: "Only a workspace owner or admin can request manual publishing." }, { status: 403 });
  const { id } = await Promise.resolve(context.params);
  try {
    const result = await createOrLoadManualPublishRequest({ serviceSupabase: createSupabaseServiceClient(), workspaceId: guard.access.workspaceId, adId: id, mutationId: typeof body.mutationId === "string" ? body.mutationId : "", notes: typeof body.notes === "string" ? body.notes : null, publishSummary: body.publishSummary, controls: body.controls, revisionId: typeof body.revisionId === "string" ? body.revisionId : "", documentHash: typeof body.documentHash === "string" ? body.documentHash : "", actorProfileId: guard.access.userId });
