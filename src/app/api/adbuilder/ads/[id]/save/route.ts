...
import { containsInlineImageData, } from "@/lib/adbuilder/persisted-document";
import { CustomerImageStorageError } from "@/lib/adbuilder/customer-image-storage";
import { resolveImageValues as resolveImageValuesShared, resolveTemplateAssetValues as resolveTemplateAssetValuesShared } from "@/lib/adbuilder/render-assets";
import { metaCopyLimitIssues } from "@/lib/adbuilder/types";
...
/**
 * POST /api/adbuilder/ads/[id]/save?workspaceId=...
 *
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    if (err instanceof SaveError) {
      ...
      if (storageFailure) console.error("Ad Builder asset storage failure", { code: err.code, message: err.message });
      const status =
