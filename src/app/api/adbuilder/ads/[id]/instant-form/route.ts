...
import type { AdBuilderBrandKit } from "@/lib/adbuilder/types";
import { deriveFormGenerationInput, generateInstantForm, validateInstantForm } from "@/lib/adbuilder/instant-form-generator";
import { instantFormSchema, type InstantForm } from "@/lib/adbuilder/instant-form-types";
import { sha256Hex } from "@/lib/adbuilder/document-token";
...
/** GET /api/adbuilder/ads/[id]/instant-form?workspaceId=... */
...
/** POST /api/adbuilder/ads/[id]/instant-form?workspaceId=... — draft only, no persistence. */
...
/** PUT /api/adbuilder/ads/[id]/instant-form?workspaceId=... — pin (save) the draft. */
...
async function loadBrandKit(
  supabase: Awaited<ReturnType<typeof import("@/lib/supabase/server").createSupabaseServerClient>>,
  workspaceId: string,
): Promise<AdBuilderBrandKit | null> {
  const { data } = await supabase
 ...
 */
function usablePrivacyUrl(brandKit: AdBuilderBrandKit | null): string {
  if (!brandKit) return "";
