...
import { buildCustomerImageRef, CUSTOMER_IMAGE_BUCKET, parseCustomerImageRef, type CustomerImageMime } from "@/lib/adbuilder/customer-image-ref";
import { imageSha256 } from "@/lib/adbuilder/customer-image-hash.server";
import { CUSTOMER_IMAGE_MAX_BYTES, CustomerImageValidationError, validateCustomerImageBytes } from "@/lib/adbuilder/image-validation";
import { checkRateLimit } from "@/lib/rate-limit";
import { createSupabaseServiceClient } from "@/lib/supabase/service";
import { adoptWorkspaceAsset, AdoptAssetError } from "@/lib/adbuilder/adopt-workspace-asset";
...
/**
 * POST /api/adbuilder/ads/[id]/media?workspaceId=...
 *
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  if (body.operation === "adopt") {
    ...
    const rateLimit = await checkRateLimit(access.access.workspaceId, access.access.userId, { windowSeconds: 60 * 60, maxRequests: 120, bucket: "adbuilder-media-upload" });
    ...
    try {
      ...
      console.error("Ad Builder workspace asset adoption failed", { code: error instanceof AdoptAssetError ? error.code : "storage" });
  ...
  const { data: ad, error: adError } = await access.supabase
    ...
    .maybeSingle();
  if (adError) {
    console.error("Ad Builder media ownership check failed", { message: adError.message });
  ...
  if (operation === "prepare") {
    const ledger = await service.rpc("adbuilder_prepare_customer_image_upload", {
    ...
    if (ledger.error) {
      console.error("Ad Builder customer image ledger prepare failed", { message: ledger.error.message });
      return NextResponse.json({ error: "We could not prepare this image upload." }, { status: 500 });
    }
    ...
    if (upload.error || !upload.data?.token) {
      ...
      console.error("Ad Builder media signed upload failed", { message: upload.error?.message ?? "missing token" });
      return NextResponse.json({ error: "We could not prepare this image upload." }, { status: 500 });
    }
  ...
  try {
    ...
    console.error("Ad Builder media finalization failed", { kind });
    return NextResponse.json(
...
async function removeUploadedObject(supabase: SupabaseClient, path: string): Promise<boolean> {
  try {
    ...
    if (result.error) {
      console.error("Ad Builder media cleanup failed", { message: result.error.message });
      return false;
    }
    return true;
  } catch (error) {
    console.error("Ad Builder media cleanup failed", { reason: error instanceof Error ? error.message : "unknown" });
    return false;
  }
}
...
async function discardAndCleanup(supabase: SupabaseClient, reservationId: string | null | undefined, workspaceId: string, adId: string, path: string): Promise<boolean> {
  ...
  const completed = await supabase.rpc("adbuilder_complete_customer_image_stale_cleanup", {
    p_reservation_id: reservationId,
    p_object_path: path,
  });
  if (completed.error || completed.data !== true) {
    console.error("Ad Builder customer image tombstone completion failed", { message: completed.error?.message ?? "claim no longer exists" });
    return false;
  }
  return true;
}
...
async function removeStaleObjects(supabase: SupabaseClient, entries: Array<{ id: string; path: string }> | undefined): Promise<void> {
  ...
  for (const entry of entries) {
    try {
      ...
      if (result.error) {
        console.error("Ad Builder stale customer image cleanup failed", { message: result.error.message });
        continue;
      }
      const completed = await supabase.rpc("adbuilder_complete_customer_image_stale_cleanup", {
      ...
      if (completed.error || completed.data !== true) {
        console.error("Ad Builder stale customer image tombstone completion failed", { message: completed.error?.message ?? "claim no longer exists" });
      }
    } catch (error) {
      console.error("Ad Builder stale customer image cleanup failed", { reason: error instanceof Error ? error.message : "unknown" });
    }
  }
}
...
async function discardUpload(supabase: SupabaseClient, reservationId: string | null | undefined, workspaceId: string, adId: string, path: string): Promise<boolean> {
  ...
  const result = await supabase.rpc("adbuilder_discard_customer_image_upload", {
    p_reservation_id: reservationId,
    p_workspace_id: workspaceId,
    p_ad_id: adId,
    p_object_path: path,
  });
  if (result.error) console.error("Ad Builder customer image ledger cleanup failed", { message: result.error.message });
