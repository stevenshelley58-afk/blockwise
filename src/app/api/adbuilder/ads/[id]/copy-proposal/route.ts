...
import { generateAdBuilderTemplateCopy, normalizeAdBuilderAiWritingGuidance } from "@/lib/adbuilder/copy-generation";
import { buildDeterministicCopyProposal } from "@/lib/adbuilder/copy-proposal";
import { errorResponse, readJsonBody, requireAdBuilderRequest } from "@/lib/adbuilder/http";
import { checkRateLimit } from "@/lib/rate-limit";
import { adTemplateSchema } from "@/lib/adbuilder/ingest-artifact";
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const guidance = normalizeAdBuilderAiWritingGuidance(metadata.aiWritingGuidance);
  const brand = await loadAdBuilderBrandDefaults(access.supabase, access.access.workspaceId);
  ...
  try {
    const result = await generateAdBuilderTemplateCopy({
