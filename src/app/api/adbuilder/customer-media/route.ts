...
import { CUSTOMER_IMAGE_BUCKET, parseCustomerImageRef } from "@/lib/adbuilder/customer-image-ref";
import { imageSha256 } from "@/lib/adbuilder/customer-image-hash.server";
import { CUSTOMER_IMAGE_MAX_BYTES, validateCustomerImageBytes } from "@/lib/adbuilder/image-validation";
...
export async function GET(request: NextRequest) {
  ...
  const canonical = new RegExp(`^(${escapeRegExp(workspaceId)})/adbuilder/ads/([A-Za-z0-9_-]+)/images/([a-f0-9]{64})\\.(png|jpg|webp)$`, "i").exec(path);
  ...
  const { data: finalizedUpload, error: finalizedUploadError } = await service
    .from("adbuilder_customer_image_uploads")
    .select("id")
    .eq("workspace_id", workspaceId)
