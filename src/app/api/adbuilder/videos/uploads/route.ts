...
import { VIDEO_UPLOAD_CHUNK_BYTES } from "@/lib/adbuilder/video-limits";
import { VIDEO_MIME_EXTENSIONS, type VideoMime } from "@/lib/adbuilder/video-refs";
import { isAcceptedSourceMime } from "@/lib/adbuilder/video-media-inspect";
import { initiateUpload, VideoUploadError } from "@/lib/adbuilder/video-upload-ledger";
import { receivedBytes } from "@/lib/adbuilder/video-upload-scratch";
...
export async function POST(request: NextRequest) {
  ...
  const guard = await requireApiWorkspace(request, "adbuilder", body.workspaceId ?? undefined);
