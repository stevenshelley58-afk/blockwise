...
function RecoveryScreen({ adId, revisionId, issues }: { adId: string; revisionId: string | null; issues: string[] }) {
  return <div className="grid min-h-screen place-items-center p-6"><div role="alert" className="max-w-lg rounded-(--r-card) border border-(--ui-error)/25 bg-(--ui-error-soft) p-6 text-(--ui-error)"><h1 className="font-display text-[17px] font-extrabold">We couldn&apos;t open this saved ad</h1><p className="mt-2 text-sm">The saved revision is preserved and needs recovery. Contact support and provide ad ID <code>{adId}</code> and revision ID <code>{revisionId ?? "unknown"}</code>.</p>{issues.length > 0 ? <ul className="mt-3 list-disc pl-5 text-sm">{issues.map((issue) => <li key={issue}>{issue}</li>)}</ul> : null}</div></div>;
}
