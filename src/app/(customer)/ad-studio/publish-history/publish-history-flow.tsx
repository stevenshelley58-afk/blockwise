          ...
          <ol className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
            {[
              ...
              <li key={step} aria-current={activeStage === Number(step) ? "step" : undefined}>
                <Button variant="ghost" type="button" onClick={() => { setActiveStage(Number(step)); window.setTimeout(() => document.getElementById(String(target))?.focus({ preventScroll: false }), 0); }} className={`flex min-h-11 w-full items-center rounded-(--r-ctl) border border-border px-3 text-left font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${activeStage === Number(step) ? "border-primary bg-primary/5" : ""}`}>
                  {step}. {label}
                </Button>
              </li>
            ))}
