...
export default async function TemplatesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1120px] px-4 pt-6 pb-28 md:px-6 md:pt-8 md:pb-16">
      ...
      <header className="mt-5 max-w-[700px]"><p className="font-mono text-[9.5px] uppercase tracking-[.12em] text-muted-foreground">Starting points</p><h1 className="mt-2 font-display text-[clamp(24px,4vw,32px)] font-extrabold tracking-[-.025em]">Choose a template for your next ad.</h1><p className="mt-2 text-sm leading-6 text-muted-foreground">Search by intent, compare the finished Feed and Story treatments, then use the pack when it fits your brief.</p></header>
      <form action="/ad-studio/templates" method="get" role="search" className="mt-7 rounded-(--r-panel) border border-border bg-card p-4 shadow-card md:p-5"><div className="flex min-h-11 items-center gap-3 rounded-(--r-card) border border-input bg-background px-3 focus-within:border-(--ink) focus-within:ring-2 focus-within:ring-(--ink)/10"><Search className="size-4 shrink-0 text-muted-foreground" aria-hidden /><label htmlFor="template-search" className="sr-only">Search templates</label><input id="template-search" name="q" defaultValue={query} placeholder="Search templates by name or intent…" className="min-w-0 flex-1 bg-transparent py-2 text-base outline-none placeholder:text-muted-foreground/75" /><button type="submit" className="inline-flex min-h-11 shrink-0 items-center rounded-full bg-primary px-4 text-[12.5px] font-bold text-primary-foreground hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Search</button></div><div className="mt-3 flex flex-wrap items-center gap-2" aria-label="Template filters"><FilterLink href={filterHref(query, "all")} label="All templates" active={filter === "all"} /><FilterLink href={filterHref(query, "image")} label="Uses image inputs" active={filter === "image"} /><FilterLink href={filterHref(query, "copy")} label="Uses text inputs" active={filter === "copy"} /></div></form>
      <div className="mt-8"><TemplateGallery templates={filtered} query={query} filter={filter} createAction={createAdAction} /></div>
      {templates.length > 0 ? <p className="mt-8 text-center text-xs text-muted-foreground">All {templates.length} available templates are retained here. Metadata reflects the imported pack.</p> : <div className="mt-8 rounded-(--r-card) border border-dashed border-(--line-heavy) p-8 text-center text-sm text-muted-foreground">No templates have been imported yet. Check again when packs are ready.</div>}
    </div>
  );
}
