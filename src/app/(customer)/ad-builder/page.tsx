import { HomeCommand } from "@/components/adbuilder/home-command";
import { loadAdBuilderLibraryPage, type LibraryAdModel } from "@/lib/adbuilder/library-read-model";
import { hasActiveTemplates } from "@/lib/adbuilder/pack-gallery";
import { requirePageSurfaceAccess } from "@/lib/auth/page-guards";

export const dynamic = "force-dynamic";

export default async function AdBuilderPage() {
  const { supabase, access, auth } = await requirePageSurfaceAccess("adbuilder");
  ...
  const [adsResult, templatesResult] = await Promise.allSettled([
    loadAdBuilderLibraryPage({ supabase, workspaceId: access.workspaceId, kind: "ads", limit: 3 }),
