...
import { EditorShell } from "@/components/adbuilder/editor/editor-shell";
import { TryVueEditorButton } from "@/components/adbuilder/vue-editor/try-vue-editor-button";
import { readVueNativeEditor } from "@/components/adbuilder/vue-editor/fabric-scene";
import { loadCustomerAd, InvalidActiveRevisionError, CustomerAdNotFoundError } from "@/lib/adbuilder/create-customer-ad";
import { getTemplateForExistingCustomerAd } from "@/lib/adbuilder/pack-gallery";
import { requirePageSurfaceAccess } from "@/lib/auth/page-guards";
import { loadAdBuilderBrandDefaults } from "@/lib/adbuilder/brand-defaults";
import { loadAdBuilderWorkspaceLibraryAssets } from "@/lib/adbuilder/assets";
import { createSupabaseServiceClient } from "@/lib/supabase/service";

import { loadNativeTrialIdentity } from "@/lib/adbuilder/vue-native-copy";
...
export default async function CustomerAdEditorPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ editor?: string }> }) {
  ...
  if (readVueNativeEditor(ad.initialDocument)) redirect(`/ad-builder/ads/${encodeURIComponent(ad.adId)}/canvas`);
  const nativeIdentity = await loadNativeTrialIdentity(supabase, access.workspaceId, ad.adId);
  if (nativeIdentity) redirect("/ad-builder/ads/" + encodeURIComponent(ad.adId) + "/canvas");
  ...
  const [brand, libraryAssets] = await Promise.all([
    loadAdBuilderBrandDefaults(supabase, access.workspaceId),
    loadAdBuilderWorkspaceLibraryAssets(supabase, access.workspaceId),
  ]);
