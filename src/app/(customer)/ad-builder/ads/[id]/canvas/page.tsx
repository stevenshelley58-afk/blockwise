...
import { VueEditorShell } from "@/components/adbuilder/vue-editor/vue-editor-shell";
import { readVueNativeEditor } from "@/components/adbuilder/vue-editor/fabric-scene";
import { CustomerAdNotFoundError, InvalidActiveRevisionError, loadCustomerAd } from "@/lib/adbuilder/create-customer-ad";
import { loadAdBuilderBrandDefaults } from "@/lib/adbuilder/brand-defaults";
import { loadAdBuilderWorkspaceAssetRows, mediaLibraryAssetForRow } from "@/lib/adbuilder/assets";
...
export default async function NativeCanvasPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!trial && !native) redirect(`/ad-builder/ads/${encodeURIComponent(ad.adId)}`);
  ...
  const [brand, rows] = await Promise.all([
    ...
    loadAdBuilderWorkspaceAssetRows(supabase, access.workspaceId),
  ]);
