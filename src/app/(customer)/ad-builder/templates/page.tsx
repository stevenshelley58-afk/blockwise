...
import { SearchField, SearchFilterPanel, SearchFilterRow, filterChipClassName } from "@/components/adbuilder/search-filter-controls";
import { TemplateGallery } from "@/components/adbuilder/template-gallery";
import { Button } from "@/components/ui/button";
import { getTemplate, listTemplates, type TemplateLeadType } from "@/lib/adbuilder/pack-gallery";
import { createCustomerAd } from "@/lib/adbuilder/create-customer-ad";
...
export default async function TemplatesPage({ searchParams }: { searchParams: SearchParams }) {
  const { supabase } = await requirePageSurfaceAccess("adbuilder");
  ...
  return (
    <div className="mx-auto w-full max-w-[1120px] px-4 pt-6 pb-28 md:px-6 md:pt-8 md:pb-16">
      <Link href="/ad-builder" className="inline-flex min-h-11 items-center gap-2 text-[12.5px] font-semibold text-muted-foreground hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><ArrowLeft className="size-4" aria-hidden />Ad Builder home</Link>
...
function filterHref(query: string, lead: TemplateLeadType | "all") {
  ...
  return value ? `/ad-builder/templates?${value}` : "/ad-builder/templates";
}
...
function TemplateReadError() {
  return <div className="mx-auto w-full max-w-[720px] px-4 pt-8 pb-28 md:px-6 md:pt-12 md:pb-16"><Link href="/ad-builder" className="inline-flex min-h-11 items-center gap-2 text-[12.5px] font-semibold text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" aria-hidden />Ad Builder home</Link><div className="mt-6 rounded-(--r-panel) border border-(--ui-error)/25 bg-(--ui-error-soft) p-6"><h1 className="font-display text-[20px] font-extrabold">Templates are temporarily unavailable.</h1><p className="mt-2 text-sm leading-6 text-muted-foreground">Refresh to try again. Your saved ads and workspace remain available.</p></div></div>;
}
...
async function createAdAction(formData: FormData) {
  ...
  redirect(`/ad-builder/ads/${encodeURIComponent(ad.adId)}`);
}
