...
export function PublishFlow({
  ...
  const refreshReceipt = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/adbuilder/ads/" + encodeURIComponent(adId) + "/publish?workspaceId=" + encodeURIComponent(workspaceId), { cache: "no-store" });
  ...
  useEffect(() => {
    ...
    let current = true;
    void fetch("/api/adbuilder/ads/" + encodeURIComponent(adId) + "/manual-publish?workspaceId=" + encodeURIComponent(workspaceId), { cache: "no-store" })
  ...
  const loadOptions = useCallback(async () => {
    setOptionsBusy(true);
    try {
      const response = await fetch("/api/adbuilder/publish-options?workspaceId=" + encodeURIComponent(workspaceId), { cache: "no-store" });
  ...
  useEffect(() => {
    ...
    let current = true;
    void fetch("/api/adbuilder/publish-options?workspaceId=" + encodeURIComponent(workspaceId) + "&campaignId=" + encodeURIComponent(campaignId) + (adSetId ? "&adSetId=" + encodeURIComponent(adSetId) : ""), { cache: "no-store" })
  ...
  async function searchLocations() {
    ...
    try {
      const response = await fetch("/api/adbuilder/publish-options?workspaceId=" + encodeURIComponent(workspaceId) + "&q=" + encodeURIComponent(locationQuery.trim()) + "&country=" + encodeURIComponent(country));
      const body = await response.json();
  ...
  async function publish() {
    ...
    try {
      if (!metaConnectionConnected) {
        const response = await fetch("/api/adbuilder/ads/" + encodeURIComponent(adId) + "/manual-publish", {
        ...
        const response = await fetch("/api/adbuilder/ads/" + encodeURIComponent(adId) + "/publish?workspaceId=" + encodeURIComponent(workspaceId), {
          method: "POST", headers: { "Content-Type": "application/json" },
  ...
  if (notSaved) return <div className="p-6"><h1 className="text-xl font-semibold">Save before publishing</h1><p className="mt-2 text-sm text-muted-foreground">Return to the editor and save the version you want to publish.</p><Button asChild className="mt-4"><Link href={"/ad-builder/ads/" + encodeURIComponent(adId)}>Back to editor</Link></Button></div>;
  ...
  return <div className="flex h-full min-h-0 flex-col bg-background">
        ...
        <div className="grid min-w-0 gap-8 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
          <aside className={`min-w-0 lg:sticky lg:top-0 lg:self-start ${stage !== 2 ? "hidden lg:block" : ""}`} aria-label="Saved ad preview">
            ...
            <div className="overflow-hidden rounded-(--r-card) border border-border bg-card">
              ...
              {previewPath ? <img src={"/api/adbuilder/media?path=" + encodeURIComponent(previewPath)} alt={`Saved ${format} creative`} className="max-h-[52dvh] w-full object-contain" /> : <p className="p-6 text-sm text-muted-foreground">Save this format in the editor to preview it.</p>}
            ...
            <div className="mt-2 flex items-center justify-between gap-3"><p className="text-xs text-muted-foreground">Saved creative. Appearance varies by placement.</p><Button asChild variant="link" size="sm"><Link href={"/ad-builder/ads/" + encodeURIComponent(adId)}>Edit ad</Link></Button></div>
          </aside>
    ...
    <Dialog open={modalOpen} onOpenChange={setModalOpen}>
      <DialogContent className="max-h-[85dvh] overflow-y-auto">
        ...
        <DialogFooter className="gap-2"><Button asChild variant="outline"><Link href="/ad-builder/templates">Create another ad</Link></Button><Button asChild><Link href={performanceUrl}>View in Performance</Link></Button></DialogFooter>
