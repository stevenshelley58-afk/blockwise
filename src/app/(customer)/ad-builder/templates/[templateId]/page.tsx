...
import { TemplateDetailPreview } from "@/components/adbuilder/template-detail-preview";
...
export default async function TemplateDetailPage({ params }: { params: Promise<{ templateId: string }> }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1120px] px-4 pt-6 pb-28 md:px-6 md:pt-8 md:pb-16">
      <Link href="/ad-builder/templates" className="inline-flex min-h-11 items-center gap-2 text-[12.5px] font-semibold text-muted-foreground hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><ArrowLeft className="size-4" aria-hidden />All templates</Link>
      <div className="mt-5 grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start">
        <section aria-labelledby="template-title">
          ...
          <TemplateDetailPreview template={pack} />
...
function formatImportedAt(value: string) {
  ...
  return Number.isNaN(date.getTime()) ? "Imported template" : `Imported ${new Intl.DateTimeFormat("en", { dateStyle: "medium" }).format(date)}`;
}
