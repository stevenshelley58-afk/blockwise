...
The customer app is niche-agnostic by construction: every niche noun, industry phrase, nav label, feature flag, and the data hue lives in `src/config/niche/`. Components read `niche.*`. Cloning = writing a sibling config folder and flipping one export.
...
2. **Assemble the config** in `src/config/niche/<vertical>.ts` (mirror `blockwise.ts`):
   ...
   export const <vertical>: NicheConfig = {
     ...
     theme: {
       data: "#2a78d6",                          // the ONE vivid data hue
...
- **`theme.data`** — the one vivid hue, for charts/meters/sparklines **only** (the Data Hue Rule in DESIGN.md). Never UI chrome or text. `dataSoft`/`dataTrack` are its fill/track transparencies.
...
## 4. How the data hue actually ships

Do not hardcode the hue anywhere. The flow:

1. `niche.theme.{data,dataSoft,dataTrack}` (your config)
2. → inline CSS vars on the `.tw` root: `src/components/self-serve-shell.tsx` sets `--ui-data`, `--ui-data-soft`, `--ui-data-track` on `<SidebarProvider className="tw" style={…}>`
3. → token bridge in `src/app/tailwind.css`: `--color-data: var(--ui-data)` (plus `-soft`/`-track`)
4. → Tailwind utilities: `text-data`, `bg-data-soft`, `stroke-data`, …

The defaults in `tailwind.css` are only a fallback for surfaces outside the customer shell.

## 5. Do NOT touch during a clone

- **Components and routes** — they consume `niche.*`; editing them reintroduces niche nouns and defeats the clone.
- **`src/app/tailwind.css` and `src/app/globals.css`** — tokens are vertical-agnostic.
- **Ad Studio** (`src/app/(customer)/ad-studio/**`, `src/components/adstudio/**`) — separate design system, out of scope.
...
- [ ] Charts/meters render in the new `theme.data` hue; no stale hex codes in chart components.
