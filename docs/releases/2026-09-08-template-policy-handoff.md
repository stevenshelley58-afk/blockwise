...
section met the 9.5 gate across comparator and two distinct reviewers; all four reusable
...
The pass gate is 9.5: the review policy literal is
`section-95-font-exempt-no-obvious-errors-v1` with 9.5 section/reviewer
minimums. The contract accepts legacy 9.5+ likeness thresholds on read; new
artifacts must declare the section-95 policy. The history above describes the
era before the gate moved to 9.5.
