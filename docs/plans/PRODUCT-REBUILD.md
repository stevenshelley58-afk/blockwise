...
| Booking / leads / performance / self-serve / settings / onboarding / connect-meta | `src/app/(customer)/booking`, `(customer)/leads`, `(customer)/performance`, `(customer)/self-serve`, `(customer)/settings`, `(customer)/onboarding`, `(customer)/connect-meta`, `src/app/api/leads/**` etc. |
