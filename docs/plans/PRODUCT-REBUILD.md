...
| Booking / leads / performance / ad-studio / settings / onboarding / connect-meta | `src/app/(customer)/booking`, `(customer)/leads`, `(customer)/performance`, `(customer)/ad-studio`, `(customer)/settings`, `(customer)/onboarding`, `(customer)/connect-meta`, `src/app/api/leads/**` etc. |
