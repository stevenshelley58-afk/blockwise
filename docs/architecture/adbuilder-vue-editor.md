# Native Ad Builder editor trial
...
\nNative template text is fitted once after its actual fonts load, with word wrapping rather than character wrapping, the pack line limit and declared truncate policy. Fabric leading includes its internal 1.13 multiplier, so pack em leading is converted. Transient fitting metadata is removed before serialization; saved native user sizing is never reflowed on reopen.\n