...
**Generated:** 2026-06-10 · Full e2e review (security, user flow, UI/UX, ads readiness, ops)
**Audience:** A code agent executing tasks one at a time, in order.

## How to use this document

Work through phases in order. Each task has an ID, files, the problem, the fix, and acceptance criteria. Do not skip Phase 0. Obey `AGENTS.md` at all times — tasks that require an exception (new dependency, auth-behaviour change) are tagged `[OWNER-AUTHORIZED]`, meaning the repo owner has explicitly requested this work as part of the launch plan. Never print or commit secret values. After each task: run `npm run typecheck` and `npm test`; commit with the task ID in the message.

---

## Phase 0 — Emergency (do these first, ~30 minutes)

### P0-1: Fix corrupted .gitignore — SSH private key is one `git add .` away from being committed 🔴 CRITICAL ✅ DONE 2026-06-10
- **File:** `.gitignore` (repo root)
- **Problem:** The file is truncated — the final entry reads `.s` instead of `.secrets/`. The `.secrets/` directory contains `vps_key` (a private SSH key to the production VPS) and is currently UNIGNORED.
...
### P0-2: Remove `ignoreBuildErrors: true` 🔴 BLOCKER ✅ DONE 2026-06-10
...
## Phase 1 — Security hardening (before any public traffic)
...
### S-1: Remove hardcoded operator email fallback 🔴 HIGH ✅ DONE 2026-06-10
- **File:** `src/lib/operator/auth.ts` line 28
- **Problem:** `process.env.OPERATOR_EMAILS ?? "stevenshelley58@gmail.com"` — personal email hardcoded as superuser fallback; grants operator access if the env var is unset.
...
### S-2: Lock down `/api/health` and `/api/health/research` 🔴 HIGH ✅ DONE 2026-06-10
...
### S-3: Authenticate `/api/model-profiles` GET 🔴 HIGH ✅ DONE 2026-06-10
- **File:** `src/app/api/model-profiles/route.ts` lines 9–13
...
### S-4: Fix PostgREST filter injection in ad search 🔴 HIGH ✅ DONE 2026-06-10
- **File:** `src/app/api/research/ads/search/route.ts` lines 28–46
- **Problem:** User-controlled `q` interpolated raw into `.or(...)` filter string — commas/parens inject extra predicates.
...
### S-5: Close SSRF gaps in brand-kit URL extraction 🔴 HIGH ✅ DONE 2026-06-10
- **File:** `src/lib/adstudio/extraction-url.ts` (`isBlockedHost`, lines 34–47)
...
### S-6: Authenticate the Google Places proxy 🟠 MEDIUM ✅ DONE 2026-06-10
...
- **Accept:** Unauthenticated request → 401; logged-in flow still autocompletes.

### S-7: Add Content-Security-Policy header 🟠 MEDIUM ✅ DONE 2026-06-10
- **File:** `vercel.json`
...
### S-8: Rate-limit expensive endpoints 🟠 MEDIUM ✅ DONE 2026-06-10
...
### S-9: Validate `sourceUrl` on brand asset creation 🟠 MEDIUM ✅ DONE 2026-06-10
- **File:** `src/app/api/adstudio/brand-kits/[id]/assets/route.ts` lines 41–57
...
- **Accept:** Posting `http://169.254.169.254/` as sourceUrl → 400.

### S-10: Stop returning raw prompt/error JSON to all workspace members 🟠 MEDIUM ✅ DONE 2026-06-10
...
### S-11: Small auth hardening items 🟡 LOW ✅ DONE 2026-06-10
- `src/app/auth/confirm/route.ts` line 50: redirect base → `new URL(next, process.env.NEXT_PUBLIC_APP_URL ?? request.url)`.
...
- **Accept:** typecheck + tests pass; manual smoke of login → ad-studio flow works.

---

## Phase 2 — Conversion funnel & ads instrumentation (before spending ad money)

### A-1: Cookie consent gate for Meta Pixel 🔴 BLOCKER ✅ DONE 2026-06-10
- **Files:** `src/app/layout.tsx` lines 81–102, `src/lib/analytics/pixel.ts`
- **Problem:** `fbq('init')` + PageView fire unconditionally for every visitor — GDPR/PECR exposure for any EU/UK traffic.
...
### A-2: Fire `CompleteRegistration` on signup confirmation 🔴 HIGH ✅ DONE 2026-06-10
- **Files:** post-confirm landing page (see UX-1 — `/self-serve`), `src/lib/analytics/pixel.ts`
...
### A-3: Add Google Ads tag 🔴 HIGH `[OWNER-AUTHORIZED]` (script tag, not an npm dep) ✅ DONE 2026-06-10
...
### A-4: Create a `/pricing` page 🔴 HIGH ✅ DONE 2026-06-10
- **Files:** new `src/app/pricing/page.tsx`; nav/footer links in `src/app/page.tsx`
- **Problem:** Ads → trial signup with no visible price anywhere = trust/conversion leak; FAQ promises a plan choice that doesn't exist.
- **Fix:** Single clear plan card (owner to supply price; use placeholder copy "$X/mo" flagged with TODO if not provided), what's included, trial terms (7 days / 10 ad packs), CTA → `/signup`. Add "Pricing" to landing nav + footer + sitemap.ts.
- **Accept:** `/pricing` renders, linked from landing nav, present in sitemap.

### A-5: Turnstile in production 🔴 HIGH ✅ DONE 2026-06-10
...
### A-6: Configure demo-request notifications 🟠 MEDIUM (config-only)
...
### A-7: Landing page performance + trust polish 🟠 MEDIUM ✅ DONE 2026-06-10
- **Files:** `src/app/page.tsx`
- **Fix:** (a) Replace hero `<img>` (lines ~129–133) with `next/image` + `priority` + `sizes` (keep mobile/desktop variants). (b) Footer social icons (lines ~577–589): until real profiles exist, add `pointer-events: none` so they aren't dead tap targets; when profiles exist, swap spans for real `<a>` links. (c) Verify `opengraph-image.png` and `twitter-image.png` are 1200×630; regenerate if not. (d) Standardize "10 ad packs" vs "10 campaigns" copy everywhere (landing, signup checkbox, trial pill, FAQ) — pick **"10 free ad packs"**.
- **Accept:** Lighthouse LCP improves; OG validator (metatags.io) renders correctly; grep shows one consistent trial phrase.

---

## Phase 3 — User flow & UI/UX fixes

### UX-1: Kill the dead `/start` route and unify "home" 🔴 HIGH ✅ DONE 2026-06-10
...
- **Fix:** Canonical home = `/self-serve`. Change signup `emailRedirectTo` next param to `/self-serve` (append `?confirmed=1` for A-2). Delete `start/page.tsx` and its unreachable components. Topbar href → `/self-serve`. Fix stale comment in `home.ts`. Keep `/home` redirect for old links.
- **Accept:** Fresh signup → confirm email → lands on `/self-serve` in one hop; no route renders dead code.

### UX-2: Fix error/404 pages 🔴 HIGH ✅ DONE 2026-06-10
- **Files:** `src/app/error.tsx` (lines 17, 22), `src/app/not-found.tsx` (line 12)
- **Fix:** Buttons link to `/self-serve` labeled "Go to dashboard" (not "/operator"). 404 copy → "Page not found / We couldn't find that page." In production, never render raw `error.message` — show the generic fallback (gate on `process.env.NODE_ENV`).
- **Accept:** Crashing page and unknown URL both show customer-appropriate copy and a working dashboard link.

### UX-3: Login form input attributes 🔴 HIGH (tiny) ✅ DONE 2026-06-10
- **File:** `src/components/login-form.tsx` lines 71–85
- **Fix:** Email input: add `type="email"`, `id="login-email"`, label `htmlFor="login-email"`. Password: `id="login-password"` + `htmlFor`. (Signup form is already correct — match it.)
- **Accept:** Password managers autofill; mobile shows email keyboard.

### UX-4: De-jargon Settings 🔴 HIGH ✅ DONE 2026-06-10
- **File:** `src/app/(customer)/settings/settings-view.tsx`
- **Fix:** (a) Line ~365: remove the "Monthly AI budget" card (internal cost cap) — show plan name + trial usage instead. (b) Line ~662: "Instagram actor" → "Instagram account (optional)". (c) Line ~482: replace `status.replace(/_/g," ")` with display map `{connected:"Connected", needs_attention:"Needs attention", revoked:"Disconnected", not_connected:"Not connected"}`; disconnect toast uses `prov.label` not raw key. (d) Region selects (also `onboarding-wizard.tsx` lines ~336–342): show full country names (`AU: "Australia"`, etc.).
- **Accept:** No raw enums, internal terms, or country codes visible on Settings/Onboarding.

### UX-5: Reset-password expired-link fallback 🟠 MEDIUM ✅ DONE 2026-06-10
- **File:** `src/app/reset-password/page.tsx` lines 63–66
...
### UX-6: Brand gate keeps navigation 🟠 MEDIUM ✅ DONE 2026-06-10
- **File:** `src/app/(customer)/ad-studio/page.tsx` lines 112–127
...
### UX-7: Empty/loading state fixes 🟠 MEDIUM ✅ DONE 2026-06-10
...
- `src/app/(customer)/leads/page.tsx` lines 106–126: only render the "Duplicate matches" panel when `duplicateCount > 0` or rows exist; normalize pill copy to "Possible duplicate" in both table and mobile views.
...
### UX-8: Small polish batch 🟡 LOW ✅ DONE 2026-06-10
...
- **File:** `src/app/api/settings/team/invite/route.ts` lines 45–46
...
## Phase 5 — Owner actions (not code — Steven's checklist)
...
1. `npm run check` (NUL check + tests + typecheck) and `npm run build` — all green.
2. Fresh-user e2e on production: signup → Turnstile renders → confirm email → land on `/self-serve` (one hop) → onboarding wizard → Ad Studio sample campaign → Settings shows no jargon/raw enums.
