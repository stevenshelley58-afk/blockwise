...
### P0-1: Fix corrupted .gitignore — SSH private key is one `git add .` away from being committed 🔴 CRITICAL
...
### P0-2: Remove `ignoreBuildErrors: true` 🔴 BLOCKER
...
### S-1: Remove hardcoded operator email fallback 🔴 HIGH
...
### S-2: Lock down `/api/health` and `/api/health/research` 🔴 HIGH
...
### S-3: Authenticate `/api/model-profiles` GET 🔴 HIGH
...
### S-4: Fix PostgREST filter injection in ad search 🔴 HIGH
...
### S-5: Close SSRF gaps in brand-kit URL extraction 🔴 HIGH
...
### S-6: Authenticate the Google Places proxy 🟠 MEDIUM
...
### S-7: Add Content-Security-Policy header 🟠 MEDIUM
- **File:** `vercel.json`
...
### S-8: Rate-limit expensive endpoints 🟠 MEDIUM
...
### S-9: Validate `sourceUrl` on brand asset creation 🟠 MEDIUM
...
### S-10: Stop returning raw prompt/error JSON to all workspace members 🟠 MEDIUM
...
### S-11: Small auth hardening items 🟡 LOW
...
### A-1: Cookie consent gate for Meta Pixel 🔴 BLOCKER
...
### A-2: Fire `CompleteRegistration` on signup confirmation 🔴 HIGH
...
### A-3: Add Google Ads tag 🔴 HIGH `[OWNER-AUTHORIZED]` (script tag, not an npm dep)
...
### A-4: Create a `/pricing` page 🔴 HIGH
...
### A-5: Turnstile in production 🔴 HIGH
...
### A-7: Landing page performance + trust polish 🟠 MEDIUM
...
### UX-1: Kill the dead `/start` route and unify "home" 🔴 HIGH
...
### UX-3: Login form input attributes 🔴 HIGH (tiny)
...
### UX-5: Reset-password expired-link fallback 🟠 MEDIUM
...
### UX-6: Brand gate keeps navigation 🟠 MEDIUM
...
### UX-7: Empty/loading state fixes 🟠 MEDIUM
...
### O-1: Error monitoring (Sentry) 🔴 HIGH `[OWNER-AUTHORIZED — new dependency]`
...
### O-3: Team invite actually invites 🟠 MEDIUM
