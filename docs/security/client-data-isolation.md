...
- `src/modules/ai-workforce/runtime-policy.ts` enforces allowed actions, data classes, destinations, outbound domains, row limits, and workspace scope.
- `src/modules/model-control/model-registry.ts` resolves approved model profiles, including OpenRouter selections saved by operators.
