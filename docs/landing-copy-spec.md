...
- **Stitch deviations:** hero CTAs say "Create your first campaign" / "See example campaign" (stitch's "Create ad" broke the settled campaign terminology); remote placeholder images replaced with `/ads/*.jpg`; radar now renders real scraped Meta Ad Library ads using a best-guess area from IP via `/api/research/locations/guess` and longest-running coverage as fallback, so no invented ads are attributed to anyone; radar "Change" location link is decorative pending a real picker.
